...
> **상태**: standalone Track 2 스펙 + PR-B1~B5 구현 박제. Track 1(in-pool 30-reranking shadow)은 CONVERGED, Track 2는 별도 blast radius로 분리된 자매 스펙이며 현재 feature branch에서 PR-B1~B5가 코드 작성·검증 완료(main 미머지, production apply/flag는 USER-only). PR-B5 구현 ✅ Claude↔omxy CONVERGED(feat commit `d3ca1d4`; design spec `0d19b46`; `scripts/shadow_eval.py` sibling evaluator + 112 tests + `pg_smoke_0039_prb5.sh`; dormant default-OFF `--shadow-eval`; 0039 apply + first verdict RUN은 USER-gated).
...
> **검증 기준**: 최초 설계 인용은 2026-06-20 KST repo 실측이며, 인용 라인번호는 리뷰 시점 기준 `≈`이라 drift한다. PR-B1~B5 구현 후 현재 사실은 §10 박제를 우선한다: migration 0039 파일은 feature branch에 존재하나 Supabase production apply는 USER-only이고, PR-B5는 구현 완료(feat `d3ca1d4`, dormant default-OFF)이나 first verdict RUN은 USER-gated이며, §11 체크리스트로 재확인한다.
...
## §5 Migration 0039 설계/구현안 — shadow 150 table + universe snapshot (production 0028 무변경)

> 0038은 Track 1 `shadow_arm_log`가 예약. Track 2는 **0039**를 사용한다. `0039_tier0_candidates_150_shadow.{sql,rollback.sql}`는 feature branch에 구현되어 있으나 Supabase production apply는 USER-only다.
...
- `feedback_pg_skip_locked_claim_anti_pattern`: shadow write는 insert/upsert-only(UPDATE...SKIP LOCKED 미사용)이므로 직접 해당 없음. 단 reconcile(PR-B4)이 concurrent selector와 race 가능하면 MATERIALIZED CTE 패턴. **(PR-B4 구현 ✅: reconcile = read-only REPORT — backfill executor 폐기. MATERIALIZED CTE는 `RECONCILE_GAP_SQL`의 owner-run race-stable read에만 적용[no SKIP LOCKED/UPDATE]; single-statement 1 MVCC snapshot.)**
...
stage 0은 **생성·로깅만**. PASS/FAIL 산출·claim unblock 금지. 아래는 PR-B5 evaluator spec이 실행 가능하게 구체화한 검정 요구조건이며, forward n_periods floor 미달 시 어떤 verdict도 금지한다. **(→ PR-B5 spec = `docs/superpowers/specs/2026-06-22-pathA-track2-prb5-forward-recall-evaluator.md`, 구현 ✅ Claude↔omxy CONVERGED 2026-06-22 [feat `d3ca1d4`; as-built §14]. 그 spec이 아래 §6.3/§6.5/§6.6을 정련한다: Option-3 secondary_selected injection 폐기→post-process sibling MODULE `scripts/shadow_eval.py` `aggregate_shadow_harvest`; §6.5 seeded null→analytic N/M 유지[no MC]; §6.6 horizon은 track당 1 primary horizon + n≥6=data-floor·pre-registered `power_floor_n`=verdict-floor. dormant default-OFF; 0039 apply + 첫 verdict run = USER-gated.)**
...
# PR-B5 refinement: binding lift is the pre-registered primary horizon h*, NOT .overall.
primary_metric = shadow_report.per_horizon[h_star] - prod_report.per_horizon[h_star]   # paired recall lift, CI90 over non-overlapping matured periods
```
...
- **PR-B5 refinement (CONVERGED)**: `aggregate_harvest` / `process_month` / `harvest_pit_months`는 frozen B+C provenance라 byte-identical 유지. `secondary_selected` injection(Option 3)과 `aggregate_harvest` 내부 shadow branch는 폐기. NEW sibling `harvest_shadow_periods` + `aggregate_shadow_harvest`(post-process Option 1)만 사용한다.
...
- **PR-B5 refinement (CONVERGED)**: same-count random null은 seeded Monte-Carlo draw가 아니다. baseline은 analytic `N/M`(selected count / full-universe size)이며 same-count no-replacement null의 exact mean이라 seed·random row 재현을 만들지 않는다(no MC, p-hack seed 제거). Count는 각 arm의 ACTUAL realized selected count per primary horizon: production-mirror/soft-tilt는 구성상 50, hard-gate는 절단 후 sleeve를 못 채우면 `INCOMPLETE_RUN`으로 제외(50 가정 금지). midlong-combined-100은 frozen `HORIZON_DAYS`에 combined key가 없으므로 `NOT_APPLICABLE_NO_COMBINED_HORIZON`으로 박제한다.
- analytic N/M baseline은 descriptive co-metric일 뿐 PRIMARY binding lift가 아니다(PRIMARY는 arm-vs-mirror paired lift). Any future eligible-universe-scoped alternative baseline requires a new freeze tag.
- verdict abort = INCOMPLETE_RUN: `n_periods` 미달 또는 `power_floor_n` 미달 / 검정 arm 누락 / forward 데이터 부족(`insufficient`) / `run_date`·`sector asOf` stale / `universe_hash` 불일치 / complete snapshot 부재 / immutable hypothesis mismatch / **(SECONDARY only)** `production-mirror`↔persisted production selection-identity parity 실패·production row 부재·★#1 미적용 / **tilt/gate arm with `sector_view.source==absent`** → plumbing-only, recall-lift verdict에서 제외(lift가 구조적으로 0이며, 이는 "섹터 효과 없음"의 증거가 **아님**).
...
|---|---|---|
| A **shadow-150 recall lift** | **PRIMARY (always forward-valid, ★#1 무관)**: `recall(shadow soft-tilt/hard-gate) − recall(production-mirror)` on **SAME forward winners**, CI90 lower>0 + `power_floor_n` satisfied — 같은 run의 두 B++ arm 비교. **SECONDARY (monthly-only, ★#1 게이트)**: vs persisted production-150, production-mirror가 selection-identity parity match일 때만; ★#1(B++=production scorer) 미적용 시 INCOMPLETE_RUN | PRIMARY: CI lower≤0 / data or power floor 미달 / universe_hash 불일치 / snapshot·hypothesis 부재 → FAIL/INCOMPLETE/DIRECTIONAL. SECONDARY: production parity 실패·row 부재·★#1 미적용 → INCOMPLETE |
...
| FWER | PR-B5 refinement: verdict endpoint는 track당 1 pre-registered primary horizon; binding family `m=(non-mirror arms)×(tracks)`(예: 2×2=4) Bonferroni. non-primary horizons는 descriptive | 단일 arm/period pass는 PASS 아님; `n≥6`은 data-floor, pre-registered `power_floor_n`이 verdict-floor |
...
> **구현 상태 (2026-06-22)**: **PR-B1 ✅ CONVERGED** (commit `42f5dcc`, `scripts/shadow_gen_core.py` + 50 tests). **PR-B2 ✅ Claude↔omxy CONVERGED** (omxy R1~R4 + Claude ce-* 3-패널, branch `tier0-bpp-multiregime` 미머지): `0039_tier0_candidates_150_shadow.{sql,rollback.sql}` + `scripts/pg_smoke_0039.sh`(docker-free 로컬 PG16 67 assertions LOCAL SMOKE PASS). PR-B2 구현 시 §5.2 sketch 대비 추가 확정: (a) **`register_shadow_hypothesis`** append-only RPC 신설(finalize는 pre-existing hypothesis 요구하나 table write grant 0이라 유일 write 경로; service_role+authenticated EXECUTE; 같은 (period_key,source,hash)에 다른 content면 `hypothesis_hash_content_mismatch`), (b) per-arm merge = **`v_arm || run_level`**(run-level wins, §5.2 정정 반영), (c) **symmetric fail-closed**: 모든 `::` cast(uuid/int/numeric/timestamptz/date)가 regex-선행 또는 begin/exception-wrap → 전부 typed error(`bad_hypothesis_id`/`bad_universe_size`/`bad_gate_eligible_size`/`bad_month`/`bad_run_date`/`bad_selection_as_of`/`row_invalid`/`snapshot_row_invalid`), 단 per-row format guard는 logged-전용이 아니라 전 status(INSERT cast가 전 status 적용), (d) anon RESTRICTIVE deny 정책 추가(0034 house pattern), (e) `snapshot_incomplete_after_write`는 `is distinct from`(3VL no-op 회피). **PR-B3 ✅ Claude↔omxy CONVERGED (2026-06-21, omxy R1~R2 + Claude ce-* 3-패널)**: `scripts/shadow_gen_runner.py`(pure payload builders) + `scripts/screen_shortlist_tier0.py`(`--shadow-sector` argparse + `run_shadow_bpp_generation_path` orchestrator, **+191/−0 ADDITIVE → OFF byte-identical**, `--shadow-sector`+`SHADOW_GENERATOR_ENABLED=true` 이중 게이트) + `scripts/test_shadow_gen_runner.py`(111 unittest) + 연결 검증 `scripts/_prb3_smoke_fixture.py`+`scripts/pg_smoke_0039_prb3.sh`(**PR-B1 generator→PR-B3 envelope→applied-0039 RPC end-to-end PASS**). PR-B3 핵심 확정: (a) F1 — manual은 USER 사전등록 SHADOW_HYPOTHESIS_ID 사용·self-register 금지(absent만 auto-register, idempotent); (b) F2 — mirror SleeveShortfall=run abort, 그 외 arm=incomplete_run; (c) **F3-NEW(0039 amend)** — register content-recheck에서 selection_as_of 제외(hash가 제외하므로 동일-period 재실행 idempotent; append-only 최초값 유지); (d) leading_sectors canonical-order 정합(Python `_canonicalize_leading_sectors` == SQL register CASE == canonical-14); (e) ce-* HIGH/P1 — `--shadow-sector`는 `--scoring bpp` 필수(argparse fail-closed, legacy fall-through로 production money-path write 금지). **PR-B4 ✅ Claude↔omxy CONVERGED (2026-06-22, commit `3d91823` feat, branch `tier0-bpp-multiregime` 미머지)**: Claude dynamic-workflow 드래프트(5 parallel ground + synth + 3 adversarial critique → report-only 설계) → omxy(fresh pane) native review(gpt-5.5 + gpt-5.4-mini 2 lane) → Claude ce-* 6-에이전트 적대 패널(correctness/data-integrity/security/testing/adversarial/simplicity) → omxy R2 catch(2 MED) → **omxy direct-fix(R3)** → Claude ce-* delta 적대검토(2 LOW) → Claude fix → omxy R4 **SIGNAL CONVERGED**. 산출 = `scripts/shadow_reconcile.py`(pure `classify_coverage_row` [complete|missing|anomaly] single-source + `RECONCILE_GAP_SQL` 3-CTE[prod/mirror/snap] MATERIALIZED owner gap query[§259, race-stable single-statement] + thin `main`[--coverage-json/--print-sql/--print-sql-inline]) + `scripts/test_shadow_reconcile.py`(stdlib unittest, fail-closed symmetric 전 boundary field + main I/O) + `scripts/pg_smoke_0039_prb4.sh`(**docker-free 로컬 PG16: 실 gap SQL→실 classifier→applied-0039 end-to-end PASS** — complete/missing + date_trunc 정규화 + universe_hash/snapshot-size/candidate tamper anomaly). **핵심 결정 = 백필 EXECUTOR 폐기(report-only)**: forward-only(§9) — past period 재실행은 today's current-only 섹터맵 stamping = look-ahead foot-gun(`--month`은 period_key label만 바꾸고 universe/price/sector는 today). 유일 정직 backfill(current period)은 idempotent PR-B3 경로로 이미 free → PR-B4는 gap을 REPORT + remediation을 DOCUMENT(현재기간만 runnable command, **KST 기준 mechanical guard**: build_reconcile_report가 now_iso→UTC+9→current month 계산, past는 command 미발급). `complete` = candidates(150/1-run/logged) AND same-run universe snapshot(distinct==universe_size·uniform min==max·**matching universe_hash** = PR-B5 recall 분모 무결성)까지 검증(candidate-only readiness 거부). `anomaly` = committed-but-partial(F2+atomic finalize상 불가 → corruption/tamper/out-of-contract direct RPC). shadow read = **owner psql**(T2-I-6 no service_role SELECT — production 경로 봉쇄), production write 0 · 마이그 0 · default no-op(production/PR-B1/PR-B3/0039 byte-unchanged) · §11 isolation. Gates(Claude 독립 재실행 + omxy 교차): 211 unittest + 3 PG smoke PASS. **USER-only 잔여 = 없음(PR-B4는 CLAUDE-verifiable; production reconcile run은 USER owner psql=외부 상태)**. **PR-B5 design spec(역사) ✅ Claude↔omxy CONVERGED (2026-06-22, commit `0d19b46`, `docs/superpowers/specs/2026-06-22-pathA-track2-prb5-forward-recall-evaluator.md`)**: Claude dynamic-workflow 드래프트(6 ground+synth+4 적대 critic) → omxy review+direct-fix → Claude ce-* 5-에이전트 doc-review(adversarial-document/feasibility/coherence/scope-guardian/spec-flow) → Claude fix → omxy re-review+fix → CONVERGED. 코드 0(당시 설계 spec이 산출물; 이후 feat `d3ca1d4`로 구현 완료); 첫 verdict run = USER-gated. **PR-B5 구현 ✅ Claude↔omxy CONVERGED (2026-06-22, feat commit `d3ca1d4`, branch `tier0-bpp-multiregime` 미머지)**: Claude dynamic-workflow 드래프트 → omxy direct-fix R1(FWER tracks_cfg·`_track_config` fail-closed·survivorship two-market·thin-volume·worst-track rollup) → Claude ce-* R1 4-패널(correctness/adversarial/testing/spec-flow) + adjudicate → omxy R2(§8 hard-gate counterfactual·§5.2.3 panel·§7.5b recall_shadow/mirror/regime_stratified·run/asof stale·markdown·alpha_corr_advisory) → Claude ce-* delta R2 2-패널 + adjudicate → omxy R3(stale/panel 런레벨→per-cell·panel RATE gate PANEL_ABSENT_MAX=0.10·fail-closed JSON loader·NaN/Inf→null·weekly graceful·non-str guard) **SIGNAL CONVERGED** → Claude 독립 최종검증(전 게이트 + 각 fix 기능 재확인). 산출 = `scripts/shadow_eval.py`(sibling MODULE evaluator, NOT in-file — frozen byte-identical 감사 용이) + `scripts/test_shadow_eval.py`(**112 unittest**) + `scripts/pg_smoke_0039_prb5.sh`(docker-free PG16 connection smoke: 0039 RPC seed→owner Query 2→FIX-J·2×150·snapshot==universe_size·service_role-denied) + `validate_tier0_ic.py` `--shadow-eval` dispatch(+80, frozen funcs byte-clean) + `probe_pit_survivorship.py` `--emit-artifact`. Gates(Claude 독립 재실행): 112 shadow + 123 frozen regression + prb5 smoke PASS · migration 0 · frozen byte-clean. as-built deltas = PR-B5 spec §14. **다음 = USER 게이트(0039 apply + `SHADOW_GENERATOR_ENABLED=true` + 첫 forward logging sign-off) 또는 PR-B6+**. **(PR-B1~B5) USER-only 잔여 = 0039 apply + real-conn smoke B1/B2/B3(§5.4) + `SHADOW_GENERATOR_ENABLED=true` + (non-mirror 시) manual hypothesis 사전등록·sign-off + (verdict RUN) owner-psql Query 2 extraction + §12 D4 timeline/power sign-off**.
...
| PR-B4 ✅ | Reconcile **REPORT** (read-only gap classification: complete/missing/anomaly per production monthly period). **Backfill EXECUTOR 폐기** — forward-only §9: past-period 재실행 = today's current-only 섹터맵 look-ahead. 유일 정직 backfill(current period)은 idempotent PR-B3 경로로 free → gap REPORT + remediation DOCUMENT(현재기간만 runnable command, KST mechanical guard). MATERIALIZED-CTE owner gap SQL → raw facts; classify는 candidates(150/1-run/logged) AND same-run universe snapshot(distinct==universe_size·uniform size·matching universe_hash = PR-B5 분모)까지 검증 | `scripts/shadow_reconcile.py` + `test_shadow_reconcile.py` + `pg_smoke_0039_prb4.sh` | shadow table only · read-only · no migration · default no-op · blocking before Stage 1 harvest/kill |
| PR-B5 ✅ | Stage 1 forward recall evaluator — **구현 ✅ Claude↔omxy CONVERGED** (feat commit `d3ca1d4`; design spec `docs/superpowers/specs/2026-06-22-pathA-track2-prb5-forward-recall-evaluator.md` §14 as-built, `0d19b46`). post-process **sibling MODULE `scripts/shadow_eval.py`** (NOT in-file, NOT `secondary_selected` injection — Option-1, frozen `aggregate_harvest`/`process_month`/`harvest_pit_months` byte-identical); PRIMARY = paired arm-vs-mirror lift @ 1 primary horizon/track (`gate_a_pass_selective_largemid` = PASS-predicate-only); analytic N/M baseline (no seeded MC); n≥6 data-floor + pre-registered `power_floor_n` verdict-floor (PASS structurally unreachable Stage-1 → DIRECTIONAL_GATE_B_NA); §8 hard-gate counterfactual; §5.2.3 panel-survivorship RATE gate (PANEL_ABSENT_MAX=0.10); FWER via `power_floor_n` (alpha_corr advisory, `_ci90` LOCKED 5/95); survivorship probe wired (+`--emit-artifact`); owner-psql read; NaN/Inf→null verdict JSON + markdown | `scripts/shadow_eval.py` + `test_shadow_eval.py` (112) + `pg_smoke_0039_prb5.sh`; `validate_tier0_ic.py` dispatch +80 (frozen byte-clean) | production effect 0; dormant default-OFF `--shadow-eval`; 0039 apply + 첫 verdict run = USER-gated |
