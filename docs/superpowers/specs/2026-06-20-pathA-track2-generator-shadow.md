...
> **상태**: standalone Track 2 스펙 + PR-B1~B4 구현 박제 + PR-B5 design spec 박제. Track 1(in-pool 30-reranking shadow)은 CONVERGED, Track 2는 별도 blast radius로 분리된 자매 스펙이며 현재 feature branch에서 PR-B1~B4가 코드 작성·검증 완료(main 미머지, production apply/flag는 USER-only), PR-B5는 design spec만 CONVERGED(commit `0d19b46`, 구현/first verdict USER-gated).
...
> **검증 기준**: 최초 설계 인용은 2026-06-20 KST repo 실측이며, 인용 라인번호는 리뷰 시점 기준 `≈`이라 drift한다. PR-B1~B4 구현 후 현재 사실은 §10 박제를 우선한다: migration 0039 파일은 feature branch에 존재하나 Supabase production apply는 USER-only이고, PR-B5는 설계 spec만 완료(코드 구현 아님)이며, §11 체크리스트로 재확인한다.
...
stage 0은 **생성·로깅만**. PASS/FAIL 산출·claim unblock 금지. 아래는 PR-B5 design spec이 실행 가능하게 구체화한 검정 요구조건이며, forward n_periods floor 미달 시 어떤 verdict도 금지한다. **(→ PR-B5 spec = `docs/superpowers/specs/2026-06-22-pathA-track2-prb5-forward-recall-evaluator.md`, design ✅ Claude↔omxy CONVERGED 2026-06-22. 그 spec이 아래 §6.3/§6.5/§6.6을 정련한다: Option-3 secondary_selected injection 폐기→post-process sibling `aggregate_shadow_harvest`; §6.5 seeded null→analytic N/M 유지[no MC]; §6.6 horizon은 track당 1 primary horizon + n≥6=data-floor·pre-registered `power_floor_n`=verdict-floor. 구현+첫 verdict run = USER-gated.)**
...
| PR-B5 (design ✅) | Stage 1 forward recall evaluator — **design spec ✅ Claude↔omxy CONVERGED** (`docs/superpowers/specs/2026-06-22-pathA-track2-prb5-forward-recall-evaluator.md`, commit `0d19b46`). 정련: post-process **sibling** `aggregate_shadow_harvest` (NOT `secondary_selected` injection — Option-1, frozen `aggregate_harvest`/`process_month` byte-identical); PRIMARY = paired arm-vs-mirror lift @ 1 primary horizon/track (`gate_a_pass_selective_largemid` = PASS-predicate-only); analytic N/M baseline (no seeded MC); n≥6 data-floor + pre-registered `power_floor_n` verdict-floor; survivorship probe wired (+`--emit-artifact`); owner-psql read | **spec only** (`validate_tier0_ic.py` sibling extension is future implementation) | production effect 0; 구현+첫 verdict run = USER-gated |
