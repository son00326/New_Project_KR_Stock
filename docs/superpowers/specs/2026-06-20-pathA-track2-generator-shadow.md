...
def compute_shadow_selections(
    ...
    no DB, no LLM, no mutation. raises invalid_input on:
      - hypothesis.source not in {absent, manual_pre_registered}
      - leadingSectors에 non-canonical-14
      - manual_pre_registered인데 leadingSectors 빈 배열 또는 asOf 누락/stale/selection_as_of 이후
      - sector-soft-tilt인데 tilt_version/params 미등록
      - arm unknown
    """
```
...
create table if not exists public.tier0_candidates_150_shadow (
  ...
  -- post-cut selectable count (hard-gate arm only); used for hard-gate under-fill INCOMPLETE_RUN check (§4). nullable.
...
- [ ] `tier0_shadow_sector_hypothesis`가 immutable append-only이고, `sector-soft-tilt` formula/params(`tilt_version`, multiplier/bonus 등)가 period 시작 전 pre-register되어 `hypothesis_id`로 참조됨. params 누락/사후변경/asOf>=selection_as_of는 `INVALID_INPUT`.
...
- [ ] RPC pre-validation(period_key regex/month-first date/period-month mismatch/arm enum/sector source enum/canonical-sector/asOf/selection_as_of/150 contract/bucket50/rank/duplicate/full-snapshot count/hash) 동작.
...
1. **arm 집합(stage 0 시작)**: `production-mirror`만으로 plumbing+production-150 baseline 먼저 안정화 vs 처음부터 `sector-soft-tilt`+`sector-hard-gate` 포함. **권고 = production-mirror 먼저(PR-B1~B3 안정화) → forward 데이터 흐름 확인 후 tilt/gate arm 확장(PR-B5 직전).**
