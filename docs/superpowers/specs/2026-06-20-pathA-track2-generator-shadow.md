...
> **상태**: standalone Track 2 스펙 + PR-B1~B4 구현 박제. Track 1(in-pool 30-reranking shadow)은 CONVERGED, Track 2는 별도 blast radius로 분리된 자매 스펙이며 현재 feature branch에서 PR-B1~B4가 코드 작성·검증 완료(main 미머지, production apply/flag는 USER-only).
...
> **검증 기준**: 최초 설계 인용은 2026-06-20 KST repo 실측이며, 인용 라인번호는 리뷰 시점 기준 `≈`이라 drift한다. PR-B1~B4 구현 후 현재 사실은 §10 박제를 우선한다: migration 0039 파일은 feature branch에 존재하나 Supabase production apply는 USER-only이고, §11 체크리스트로 재확인한다.
