...
- **가동·검증**: **S9 운용 검증 기간(어드민 3인 1개월+)** 동안 실제로 돌려 학습 루프가 작동하는지 검증한다. 출시 시점엔 **단기/주간 트랙 기준 "이미 작동하는 자가학습"**이 된다(중장기 월간 트랙은 §4 타이밍 — S9 길이·첫 선정일에 따라 데이터 발생 시점이 다르며, 데이터 미발생 시 no-op fail-soft).
...
2. **데이터 의존성은 S9 창이 충족한다.** Reflection의 가치는 "직전 기간 실현 수익률 → 다음 기간 prompt"라서 실 운용 데이터가 누적돼야 의미가 생긴다. 출시 직전 **S9 운용 검증(1개월+ 실 선정 운용)** 이 바로 그 데이터 창을 제공한다 → 출시 시점엔 **(단기/주간 트랙) 1+ 사이클 회고가 검증된 상태**(중장기 월간은 §4 타이밍 caveat — S9가 짧으면 1회고 미발생 가능, fail-soft).
...
… S7c → S7d → (Pre-launch 섹터 비교 메뉴 + Tier2 배선 + D4 freeze)
       ...
       → 🎉 출시 (단기/주간 트랙 자가학습 작동; 중장기 월간은 §4 타이밍·fail-soft)
       → [출시 후] S8 자동매매
```
