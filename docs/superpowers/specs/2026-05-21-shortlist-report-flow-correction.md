...
status: SUPERSEDED-IN-PART by 55차 §2 (PR3b scope 분리: writer-only + PR3c 신설) · **65차 supersede 포인터 (2026-06-04): §1 lock-in 설계 어휘는 65차 7결정으로 SUPERSEDED — 현 운영 SoT = HANDOFF.md 65차**

> **⚠️ 65차 supersede 포인터 (2026-06-04) — 본 spec doc의 선정·포트·AI·MVP 설계 어휘는 65차 7결정으로 SUPERSEDED. 현 운영 SoT = `Document/Process/HANDOFF.md` 65차 (⭐ MVP 엔진 섹션). 본 문서는 historical reference (53차 §5 정정 박제) — 본문 미수정, 아래는 다음 진입자 오독 방지 포인터.**
> 충돌 항목: (1) §1.1 선정흐름 = **65차 Q1 supersede** — 단기 주1회 / 중장기 월1회 split (기존 '월1회 monthly-batch 단일 배치로 단·중·장 30 동시 선정' stale). (2) §1.2/§4 PR3b·PR5 풀리포트 = **65차 W2** rolling composite writer + 빌드순서 W0→W2→W1→W3로 superseded. (3) §1.1/§1.7/§4 PR2 'Core 11 병렬 독립 채점 + 합의' = **65차 Q4** 실시간 멀티라운드 AI 반박 토론 loop로 확장(합의 점수로 선택). (4) **65차 Q2** AI 자율 포트(운용여부·편입개수·종목·단중장분배·비중·현금0~30% 전부 AI 자율) = 본 문서 미반영 신규(기존 '단/중/장 top 10 = 30 항상 전체' 변경). (5) **65차 Q3** 모델/프로바이더 추상화(모델 하드코딩 제거+설정화, Claude+GPT 멀티프로바이더 auto-detect, 역할별 모델 차등, registry) = 본 문서 'Anthropic 단독' 전제 변경. (6) hardcap = **50만원**(§4 PR5 'δ1 기존 40만원 hardcap' / 'δ2 100만원 = scope creep 폐기' 어휘는 active SoT 아님 — 본 historical 수치는 보존, 값 정정은 active SoT에서만). (7) §1.8 'PR-G ⓑ 단발 30선정' + canonical 5-PR 순서 = 65차 W0~W3로 superseded(코드자산 재사용). 상세: HANDOFF.md ⭐ 65차 MVP 엔진 섹션.
