# PR-B5 spec — Track 2 generator-shadow Stage 1 forward recall evaluator

**Status**: IMPLEMENTED ✅ Claude↔omxy CONVERGED (2026-06-22, feat commit `d3ca1d4`, branch `tier0-bpp-multiregime`, main 미머지). Dormant default-OFF (`--shadow-eval`); migration 0039 apply + the first verdict RUN remain USER-gated (§12). Design body below is preserved as authored; **as-built deltas vs this design are recorded in §14** (the SoT for what shipped).
...
**Target code (AS BUILT, §14)**: NEW sibling module `scripts/shadow_eval.py` (the evaluator) + NEW `scripts/test_shadow_eval.py` (112 unit tests) + NEW `scripts/pg_smoke_0039_prb5.sh` (connection smoke) + `scripts/validate_tier0_ic.py` carries ONLY the `--shadow-eval` argparse flags + a default-OFF early-return dispatch (+80 lines; frozen funcs byte-identical) + `scripts/probe_pit_survivorship.py` additive `--emit-artifact PATH` flag (§5.1). (The design said "additive extension of `validate_tier0_ic.py`" — shipped as a sibling module for trivial byte-identical auditability; see §14.1.)
...
## §14 — As-built implementation deltas (2026-06-22, feat `d3ca1d4`) — SoT for what shipped

The design body (§0–§13) is preserved as authored. The implementation matches it except for the deltas below, each settled in the Claude↔omxy convergence loop. Gates at landing: **112 `test_shadow_eval` + 123 frozen `test_validate_tier0_ic` regression + `pg_smoke_0039_prb5.sh` PASS · migration diff 0 · `process_month`/`aggregate_harvest`/`harvest_pit_months` byte-clean** (all independently re-run by Claude).

**14.1 Sibling module, not in-file** — the evaluator lives in a NEW `scripts/shadow_eval.py` importing `validate_tier0_ic as V` for leaf reuse, instead of the §3.3/§10.1 "same-file additive functions". `validate_tier0_ic.py` carries only the `--shadow-eval` flags + a default-OFF early-return dispatch (+80 lines). Rationale: the frozen-path byte-identical guarantee becomes trivially auditable (the working diff to the frozen file is flags+dispatch only, zero function-body edits). Supersedes the § header / §3.3 / §10.1 / §13 "same file" wording.

**14.2 FWER carried by `power_floor_n`, NOT CI-widening** — `_ci90` stays LOCKED 5/95; the report emits `alpha_corr_advisory = 0.10/m_fwer` + `fwer_via_power_floor_n: true` (advisory, not applied to the CI). This resolves the §4.5 internal tension: the line "widen each CI accordingly" is **superseded** — FWER is absorbed into the pre-registered `power_floor_n` (the USER fixes it under the Bonferroni'd α at §12 D4), consistent with the §4.2 LOCK and §13 "reject a second CI method".

**14.3 Panel-survivorship = RATE gate, not binary** — §5.2.3 ships as `panel_survivorship_unverified` iff `absent_fraction > PANEL_ABSENT_MAX` (a documented structural constant = 0.10, an order of magnitude above any plausible delisted fraction ~0.002–0.004; NOT tuned to a result — the probe remains the binding survivorship gate per §5.2.4). A binary `absent>0` would perpetually block every universe-wide period on benign entry-window data gaps (which FIX-I already excludes from the winner basket). `delisted_fraction` + `absent_fraction` ride report-only in `warnings[]`.

**14.4 Stale / panel = PER-CELL, not run-level** — `asof_stale` / `run_date_stale` / `panel_survivorship_unverified` are per-period exclusions (the period is marked `eligible=False`; `_score_cell` consumes only eligible rows; the data floor + `power_floor_n` then gate honestly). They are NOT promoted to run-level terminal blocks — that matches §6's per-cell classification (RUN-LEVEL terminal guards stay limited to `mirror_arm_absent` / `no_eligible_periods` / `INCOMPLETE_RUN_PLUMBING_ONLY`) and avoids one stale period perpetually blocking an otherwise-scorable forward-accruing run.

**14.5 `warnings[]` added to the report** — reconciles the §2.4-vs-§7.5b inconsistency: the run-level report carries a `warnings[]` array (thin-print `gap_winner_count` + `thin_gap_excluded_count` + per-period `delisted_fraction`/`absent_fraction`). The §7.5b per-cell schema is unchanged; the thin-print count rides at run level.

**14.6 Hardening shipped** — (a) survivorship dates parsed strictly (`_yyyymmdd` / `_parse_iso_day`: 8-digit or canonical ISO only; the `[:8]` strip-coerce that silently accepted `2020-0101-99` is removed; non-str → `ShadowEvalInputError`). (b) all four `--shadow-*` JSON file reads (incl. kill-rule) route through a fail-closed loader catching `json.JSONDecodeError`/`OSError`/`UnicodeDecodeError` → `ShadowEvalInputError` → `[ABORT]`. (c) verdict JSON recursively maps NaN/Inf → null with `allow_nan=False` (RFC-8259-valid for jq/JS/Postgres consumers) + markdown verdict sibling. (d) `_hard_gate_counterfactual` skips immature `h*` periods (§2.5). (e) weekly period with `selection_as_of=None` → graceful per-period `forward_insufficient` (no `YYYY-Www-01` construction). (f) per-cell `recall_shadow`/`recall_mirror`/`regime_stratified_lift` emitted; the +5pp NaN-fallback is short-circuited to `INCOMPLETE_RUN` and PASS is structurally unreachable at Stage-1 (Gate B `NOT_APPLICABLE` → `DIRECTIONAL_GATE_B_NA`).

**14.7 Deferred (spec-noted, not shipped)** — SECONDARY vs persisted production-150 is not implemented or emitted by the PR-B5 Stage-1 evaluator; the design remains conceptually `INCOMPLETE_RUN` until ★#1 (§0.2 — honest, not a bug). The `secondary_*` / `NOT_APPLICABLE_WEEKLY_GRAIN` codes are spec placeholders for the ★#1-unlocked path, not current shipped output. The formal alpha-spending function stays deferred (§4.5). `beta_orthogonalized_lift` lands `null` (orthogonalization deferred to the verdict run, §4.0).

**14.8 Convergence provenance** — Claude dynamic-workflow draft → omxy direct-fix R1 (FWER tracks_cfg, `_track_config` fail-closed, survivorship two-market, thin-volume, worst-track rollup) → Claude ce-* R1 (correctness/adversarial/testing/spec-flow) + adjudication → omxy R2 (§8 counterfactual, §5.2.3 panel, §7.5b fields, stale, markdown, alpha_corr) → Claude ce-* delta R2 + adjudication → omxy R3 (14.3/14.4/14.6 fixes) **SIGNAL CONVERGED** → Claude independent final verification (all gates + each fix functionally re-checked). Every omxy CONVERGED claim was independently re-verified against code (`feedback_workflow_verify_false_convergence`).
