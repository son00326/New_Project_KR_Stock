...
> **상태**: 설계 스펙. Round 1 omxy 적대리뷰(직접 수정) → Round 2 Claude 적대리뷰(ce 문서-리뷰 5종) findings 반영. 코드 미작성.
...
2. `production-snapshot` == `productionResult.selected` byte-for-byte (JSON serialize 후).
...
begin
  ...
  if v_track not in ('short','midlong') then raise exception 'bad_track'; end if;
  ...
  if v_arm not in ('production-snapshot','sector-soft-reserve','regime-sector-soft-reserve','candidate-pool-hard-gate') then raise exception 'bad_arm'; end if;
...
|---|---|---|---|
| PR-A1 | Pure `computeArmSelections` + tests (track-aware fixture, invalid source throw, no-mutation 테스트) | `shadow-harness-arms.ts` | none |
| PR-A2 | Worker optional DI seam, **finalizeSelection 내부 finalized 경로** logger 호출, default OFF | `tier1-selection-batch-worker.ts`, route wiring | none when flag OFF |
| PR-A3 | Migration 0038(+RPC, begin/commit, RPC-only writes, immutable snapshot) + smoke tests | `0038_shadow_arm_log.sql`, rollback | new shadow table only |
| PR-A4 | Reconcile/backfill(missed shadow rows) | script/test | shadow table only · **Stage 0 live logging에는 non-blocking, Stage 1 harvest/kill 전에는 blocking** |
| PR-A5 | Stage 1 harvest/kill evaluator | 별도 spec 선행 | USER 결정 전 production effect 0 |
