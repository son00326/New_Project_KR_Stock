...
- **closed reason-code set**(모든 §5 branch가 정확히 하나로 매핑): `INCOMPLETE_RUN`(`n_below_data_floor`/`winners_below_floor`/`arm_incomplete_run`/`forward_insufficient`/`asof_stale`/`run_date_stale`/`survivorship_failed`/`panel_survivorship_unverified`/`no_eligible_periods`/`reconcile_partial`/`reconcile_missing`/`reconcile_anomaly`) · `INCOMPLETE_RUN_PLUMBING_ONLY` · `DIRECTIONAL_POWER_FLOOR` · `DIRECTIONAL_GATE_B_NA` · `DIRECTIONAL_BETA_UNVERIFIED` · `DIRECTIONAL_BETA_BLOCKED` · `NOT_APPLICABLE_PLUMBING_ONLY` · `INVALID_INPUT`(`malformed_row`/`cross_period`/`regime_unregistered`/`triple_gate_inconsistent`/`gate_c_claimed_no_metadata`). **⚠️ AS-BUILT(§11-D2): `power_floor_n_below`는 §4/§5상 n<power_floor_n ⇒ DIRECTIONAL이므로 emit하지 않고 `DIRECTIONAL_POWER_FLOOR`로 대체(set에서 제거). `arm_incomplete_run`/`winners_below_floor`는 reachable(§1/§5 — `_insufficiency_reason`). `period_not_complete`는 제거(coverage에 없는 extract period는 `malformed_row` INVALID_INPUT으로 fail-closed). `gate_b`는 키 `reason`(§6.5 schema 그대로), `gate_c`는 `{verdict:NOT_APPLICABLE}`(reason 키 없음).**
...
- **Round 1 — Claude 적대 리뷰 패널**(리뷰-목적 에이전트, 드래프팅과 분리): 5 ce-* 페르소나(correctness / adversarial / spec-conformance / data-migration-SQL / testing) fan-out → raw findings를 main(Opus)에서 adjudicate([[feedback_workflow_verify_false_convergence]]: aggregate converged 신뢰 금지, raw에서 재유도). 13 fix 적용 + 1 reject-with-rationale(delisted 비대칭 inflation — lift는 두 arm 대칭 + spec-locked → 메트릭 불변, 대신 delisted-pick transparency 추가) + survivorship 게이트 vacuous 버그(entry-window universe→absent≡0) 발견·수정(SELECTION-bar universe로 교체).
- **Round 2 — omxy(fresh Codex pane, gpt-5.5 xhigh) cross-runtime 리뷰 + 직접 수정**: omxy가 native code-review + architecture/devil's-advocate 2 lane 운용. 6 추가 fail-closed defect 직접 수정(cross-arm sector/regime metadata 일관성·all-row run_date/asOf staleness[min() future-stale 은폐 차단]·counterfactual_cut 6-digit+dedup·`_validate_arms` active-arm 검증·coverage-missing→`malformed_row` + complete-coverage 4-arm 일관성·partial-benchmark beta→UNVERIFIED) + CLI mutual-exclusion(PR-B5 ↔ PR-A5 shadow 모드 + print-branch preemption). omxy **SIGNAL: CONVERGED**.
- **Round 3 — Claude 독립 verification**(Trust 3-step: omxy patch = suggested, Claude가 git-state·full-gate·edit-by-edit 재검증): 2 리뷰-목적 에이전트(ce-correctness + spec-conformance) FINAL 상태 재검증 → **CLEAN(코드 defect 0)**. 잔여 = 본 §11 doc 박제(D1/D2)뿐. ⇒ 상호 CONVERGED.

**Gate evidence** (Claude 독립 실행): `python -m unittest test_shadow_arm_eval test_shadow_eval test_validate_tier0_ic` = **341 OK**(arm 105 + eval 112 + validate 124); `bash scripts/pg_smoke_0038_pra5.sh` = **PASS**(0038 apply → 4-arm RPC seed → shipped extract SQL → parser → synthetic-panel harvest+aggregate, + negative grant matrix: anon denied · no table DML · DEFINER RPC sole write); frozen `process_month`/`aggregate_harvest`/`harvest_pit_months` **byte-clean of `shadow_arm`**. production effect 0(DB write 0) · no real LLM/Supabase · 미머지.

**D1 — beta-orthogonalized lift (§4 literal 보정).** §4 literal `ortho_lift_p = eps_arm_p − eps_snap_p`(arm/snapshot 각각 OLS) 후 `mean(ortho_lift_p)`은 OLS 정규방정식상 각 잔차합 0 + 동일 paired period set이라 **항등 0** → "non-positive ⇒ PASS block" hard-blocker 역할 불가(self-contradictory). 구현은 의도(섹터-beta 제거 후 잔여 alpha) 그대로 두되 non-degenerate 실현: paired per-period lift `L_p = mean_arm_p − mean_snap_p`를 사전등록 benchmark `x_p`에 회귀(`L = a + βx + e`)한 **intercept `a`**(= benchmark 0 노출에서의 lift = 섹터-beta-orthogonalized lift); `a > 0` 필요. + **bracket-0 extrapolation guard**: observed `x` 범위가 0을 straddle 못하면(`min(xs) ≤ 0 ≤ max(xs)` 실패) α/β 분리 불가 → `DIRECTIONAL_BETA_UNVERIFIED`(spec보다 보수적 tightening). 두 verifier(Claude correctness + spec) 모두 math sound 확인. 코드 docstring `_beta_orthogonalized_lift` AS-BUILT 블록 = 단일 SoT.

**D2 — closed reason-code set(§6.5 보정).** (a) `power_floor_n_below`(§6.5가 INCOMPLETE_RUN family로 오기재)는 §4/§5상 n<power_floor_n ⇒ DIRECTIONAL → emit 안 함, set에서 제거(`DIRECTIONAL_POWER_FLOOR`로 대체). (b) `arm_incomplete_run`(§1/§5 hard-gate incomplete_run arm-cell) + `winners_below_floor`(§5 |winners|<10 cell 제외)는 `_insufficiency_reason`로 reachable화. (c) `period_not_complete` 제거 — coverage(PR-A4 reconcile)에 없는 extract period는 두 operator-fed 입력 불일치 → `malformed_row` INVALID_INPUT으로 fail-closed(silent INCOMPLETE 금지). (d) `gate_b` 키 = `reason`(§6.5 schema 정합), `gate_c` = `{verdict:NOT_APPLICABLE}`(reason 키 없음). (e) `gate_c_claimed_no_metadata`는 future sleeve-claim 경로용 reserved INVALID_INPUT(Stage-1 unreachable, 의도적). 최종 set = 23 codes.

**L (reserved, no action)**: `gate_c_claimed_no_metadata`는 Stage-1에 emitter 없음(sleeve metadata 입력 경로 부재) — 향후 sleeve-claim 경로 추가 시 배선. 

**메트릭/정책 불변**: USER-locked metric(per-tf paired mean return lift + market-winner hit-rate), forward-only, PASS Stage-1 구조적 unreachable(Gate B NA), claim-discipline cap("in-pool 30-reranking forward observation (verification pending)"), production effect 0 — 전부 무변경. 본 §11은 §6 정책을 재결정하지 않고 구현 현실만 박제.

**USER 게이트(미실행, §9)**: (1) 0038 apply + FORWARD_SHADOW_ENABLED 지속 + per-period manual sector/regime 사전등록 + owner/service-role-psql extraction (2) timeline/power_floor_n sign-off + frozen pre-registration(parameter_lock_commit_hash+freeze_tag). evaluator는 dormant(default-OFF) landing; 첫 verdict RUN은 USER-gated.
