...
| **D-9** | **3 trigger path(cron / reject 후 trigger 버튼 / 종목별 Regen) 모두 orchestrator(D-1)로 수렴.** manual trigger 2종은 orchestrator 호출 wrapper로 재구현(track-record action 대체). | 기획 D23 lock-in 1.3. [omxy R1 MED 반영] |
...
| **D** | Tier0 150후보 producer/consumer (B1/D-3) | 마이그 `tier0_candidates_150` + Python `--emit-candidates` 150 + cron/admin `tier0Source` 실 SELECT(mock 제거) | A | 마이그 apply + Python 시드 | high |
| **E** | short_list_30 AI 스키마 + persist + 배치 un-stub (B4/D-7) | 마이그 `consensus_badge`+**`ai_score`**+`weighted_score_*`+`winning_timeframe`+`conviction`+**`ai_comment_kr`** + `upsertShortList30` 매핑(drop 수정) + orchestrator non-⚪ `commit_persona_eval` 경로 + 4 stub→real 최종 교체 | A,C,D | 마이그 apply | high |
...
| **G** | env/seed + **실 AI 첫 30선정 검증** | 코드 0. env 4종 + cron-system seed + 첫 트리거 + verify(cost_log>0/배지 non-null/votes>0) | E (+F) | env·seed·비용·trigger | high |
| **H** | 실 리포트 enrich + worker 가동 + **manual trigger 2종(D-9)** | placeholder→실 DART/배지 source(report 입력) + report-worker 가동 + reject-trigger 버튼·Regen 버튼 orchestrator wire | B,E,G | `PR5_CRON_AUTO_ENABLED` + 비용 | high |
...
| writer Section 0~7 리포트 | PR-H (기구현 PR3b/PR5 + 입력 enrich) |
