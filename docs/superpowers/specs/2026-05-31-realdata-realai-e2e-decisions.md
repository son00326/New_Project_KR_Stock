...
| **E** | short_list_30 AI 스키마 + persist + 배치 un-stub (B4/D-7) | 마이그 `consensus_badge`+**`ai_score`**+`weighted_score_*`+`winning_timeframe`+`conviction`+**`ai_comment_kr`** + `upsertShortList30` 매핑(drop 수정) + orchestrator non-⚪ `commit_persona_eval` 경로 + 4 stub→real 최종 교체 | **A,B2,C,D** (B2 선행 필수 — real-call path가 fail-closed guard 없이 열리면 안 됨, R2 MED-1) | 마이그 apply | high |
