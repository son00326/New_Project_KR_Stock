...
| **D-6** ~~(defer)~~ → **D32 supersede** | **Reflection(D19(f) 실현수익률→다음 선정 prompt 주입) = ~~go-live OOS, defer → PR-K~~ → [SUPERSEDED by D32, 2026-06-24] 출시 전 빌드 + S9 운용 검증 중 가동·검증.** | 실 결과 누적이 선행되어야 의미 — D32: S9 운용 검증 창(출시 전 1개월+ 실 선정 운용)이 그 누적을 충족하므로 출시 전으로 승격. [omxy R1 MED 반영 · **D32 supersede**: `docs/superpowers/specs/2026-06-24-reflection-prk-pre-launch-promotion.md`] |
...
| **K** (~~defer~~ → **D32 출시 전 승격**) | Reflection 자가학습 (D-6, **D32 supersede**) | reflection_log 마이그 + track별(주1/월1) 실현수익률→Tier1 prompt 주입 | **출시 전 빌드 + S9 운용 검증 중 가동**(구 go-live 후, D32) | 비용 | med |
...
| Reflection (실현수익률→prompt) | PR-K (~~defer~~ → **D32 출시 전 승격**, D-6) |
