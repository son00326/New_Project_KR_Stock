...
| **D-3** | **Tier0 150후보 = Python `screen_shortlist_tier0.py`가 DB write(producer) + TS cron이 read(consumer).** | Vercel = Node 전용(Python cron 불가). 기획 D19(b) Tier0 = KRX·DART → 단/중/장 50씩 = 150. **[63차 정밀화 → D-10]**: S1·S2·universe = KRX 공식 API / S3 = pykrx / S4·S5 = DART. |
...
| **G** | env/seed + **실 AI 첫 30선정 검증** (63차: ⓐ MERGED #68 / ⓑ 잔여) | ⓐ cron 실 AI prep ✅. ⓑ = **(D-10) `screen_shortlist_tier0.py` KRX 공식 API 전환**(cost 0) + 150 시드 + **(D-11) 선정 청크 워커 PR**(로컬 러너 대체) + 수동 트리거 verify(cost_log>0/배지 non-null) + B2 guard 확인 | E (+F) | env·seed·비용·trigger | high |
