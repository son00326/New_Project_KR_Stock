...
**Goal:** 트랙별 재선정 시 기존 리스트 종목(incumbent)을 fresh Tier0 후보와 union해 **반드시 평가에 포함**(무심사 탈락 금지)하고, 각 incumbent에 직전 논거·배지·점수·리포트 thesis·실현 성과를 "incumbent thesis context"로 주입해 재점검시키며, delta_status(hold/new)를 직전 row diff로 실계산한다. incumbent-only가 재선정되어도 기존 `name`/`composite_score`/`signal_label` 메타데이터는 보존한다.

**Architecture:** W2a 트랙 seam(period_key/track + rolling composite RPC) 위 **additive — 마이그 0 · RPC 무변경 · reader(getActiveShortList) 무변경**. 후보풀만 fresh(50/100) ∪ incumbent-only(≤10/≤20)로 가변화하고, per-ticker 컨텍스트는 기존 `reflectionContext` seam을 per-call 인자로 확장해 주입한다. incumbent display 메타데이터는 기존 `short_list_30` row fallback으로 persist에 전달하되 sector fallback은 제외한다. 유지는 자동이 아님 — incumbent도 신규와 동일 랭킹 경쟁(top10 진입 시만 유지).
...
3. per-ticker incumbent thesis context builder(직전 ai_comment_kr/배지/ai_score/conviction + stock_reports `section_0`(headline/thesis, incumbent month 이하 최신) + 실현 성과[portfolio_snapshot `entry_price>0`시만 — W3 graceful]) → `reflectionContext` seam per-call 확장 주입. incumbent-only display meta는 기존 row에서 carry하되, sector는 canonical 검증된 aggregate에서만 carry하고 persist fallback은 `name`/`composite_score`/`signal_label`로 제한.
...
- **D1 incumbent 식별 = "최신 month 그룹" 조회.** `short_list_30`에서 `bucket = any(트랙 buckets) AND month <= 선정월`을 month desc로 조회해 **최신 month의 rows만** 채택(short: 주중 재선정이라 보통 같은 달 = 직전 주 선정분, 월초 carry 후엔 carry분 / midlong: 직전 월분). cold start(0 rows) = incumbents `[]` → W2a fresh-only와 동일 동작(D27: cold start=신규 취급). latest 그룹이 존재해도 `0 < count <= TRACK_SELECT_COUNT`는 허용한다. 특히 short 월초 carry는 `carry_short_into_month`가 midlong으로 졸업한 ticker를 `not exists`로 제외하므로 9/10 같은 부분 그룹이 정당 상태다. `> TRACK_SELECT_COUNT`만 corruption/수동 DB 조작 방어로 fail-closed `incumbents_count_exceeded` throw. `trackOfPeriod`(W2a 잔여 ③)는 worker가 track 인자를 직접 들고 있어 소비 불필요 — 보존만.
- **D2 union은 순수 함수, fresh 우선.** `mergeFreshWithIncumbents(fresh, incumbents)` — fresh ticker와 겹치는 incumbent는 fresh row 유지(tier0 score 보유), incumbent-only는 `Tier1Candidate` 합성: `tier0_buckets[직전 bucket]=true` + `tier0_scores` **전부 null**(Tier0 후보 아님 → `computeTier0Ranks`에서 자연 제외 → tier0IsTop=false → 배지는 🟣/⚪ 경로 = 의미 정합) + sector는 `isCanonicalSector` 통과분만. `name`/`compositeScore`/`signalLabel`은 ranking 타입에 넣지 않고 persist metadata fallback으로 별도 전달한다.
- **D3 pool 동적화는 3단 게이트.** (a) worker: fresh 정확 count(`TRACK_FRESH_POOL`) 불변 + incumbents는 `0...TRACK_SELECT_COUNT` 허용(`> TRACK_SELECT_COUNT`만 fail-closed) → union 후 enqueue. fresh와 incumbent가 겹치면 incumbent-only 추가분만 줄어 pool은 `[FRESH, FRESH+SELECT_COUNT]`. 부분 incumbent 그룹은 carry 졸업 제외의 정상 결과이며, 졸업 ticker는 midlong row로 존재해 midlong incumbent로 재점검된다(드랍 아님). (b) `runTier1Screening`: 고정 `!== TRACK_FRESH_POOL` → **range** `[FRESH, FRESH+SELECT_COUNT]`. (c) 최종 count 정합 = `makeTier1ScreeningResultSchema(track, candidates.length)` (W2a factory가 이미 poolSize 인자 — notSelected=poolSize−selectCount 자동).
- **D4 컨텍스트 주입 = per-call reflectionContext 인자.** `makeCallPersonaPanel` deps의 `reflectionContext: string`(invocation 고정)을 유지하되 **per-call override** 추가: `callPersonaPanel({ticker, financials, reflectionContext?})` → `reflectionContext ?? deps.reflectionContext`. worker chunk loop는 incumbent에만 `incumbentContextByTicker[ticker]`를 전달하고 비-incumbent는 `undefined`로 둔다(deps default `''` 사용). enqueue/claim/run-mutex/preflight 등 W2a 큐 메커니즘 **무변경**. finalize replay 콜백은 reflectionContext 미사용(LLM 0콜)이라 무영향.
- **D5 thesis/실현 성과는 graceful-by-construction.** `stock_reports` thesis는 incumbent month 이하 최신 `section_0`만 사용(미래/현재 신규 리포트가 직전 thesis를 덮어쓰면 안 됨). `portfolio_snapshot`에서는 incumbent ticker의 최신 행 중 `entry_price > 0`인 것만 `total_return` 사용. 현 production은 entry_price=0 placeholder(W3 TODO)라 전부 생략 — 별도 flag/stub 없이 데이터가 생기면 자동 포함.
...
- **D8 incumbent set의 invocation 간 미세 변동은 수렴.** enqueue는 매 invocation idempotent upsert — 중간에 incumbent set이 변해도(월초 carry 등) 신규 ticker는 새 job으로 추가되어 평가받고(무심사 탈락 금지 유지), 사라진 ticker의 기존 job은 잔존·평가됨(후보 재조회 시 pool 밖이면 replay에서 미사용 — 무해). finalize는 nonTerminal===0까지 대기하므로 정합 수렴.
- **D9 incumbent-only display metadata 보존.** 기존 `admin-shortlist-persist.ts`의 display-meta lookup은 `tier0_candidates_150` 전용이라 incumbent-only selected는 `name`/`composite_score`/`signal_label`이 null로 회귀할 수 있다. `IncumbentInfo`에 기존 row 메타를 싣되, persist fallback은 Tier0 meta lookup이 못 채운 `name`/`compositeScore`/`signalLabel`만 보존한다. `sector`는 `mergeFreshWithIncumbents`에서 canonical 검증된 aggregate 값으로만 carry하며, 비-canonical sector 재유입을 막기 위해 metadata fallback 대상에서 제외한다.
...
- `tudal/src/lib/data/admin-shortlist-incumbents.ts` — `getIncumbents`(D1, over-cap fail-closed + display meta carry) + `buildIncumbentThesisContexts`(D5) — client DI(SelectChain mock 타이핑, [[feedback_test_mock_typing]]).
...
- `tudal/src/lib/data/admin-shortlist-persist.ts:80-141`(buildShortListRows delta + incumbent metadata fallback 인자) + `:245-307`(upsertShortListTrack options.incumbentTickers/options.incumbentMetadataByTicker).
...
function incumbent(ticker: string, bucket: 'short' | 'mid' | 'long', sector: string | null = '반도체'): IncumbentInfo {
  return {
    ...
    compositeScore: 72.5, signalLabel: '기존 신호',
  };
}
...
describe('getIncumbents', () => {
  ...
  it('short: 최신 그룹 10 반환 + IncumbentInfo 필드 매핑(aiCommentKr/배지/aiScore/conviction/name/compositeScore/signalLabel)', async () => { /* 매핑 검증 */ });
  it('short: 월초 carry 졸업 제외로 최신 그룹 9행이어도 정당 → 9행 반환', async () => { /* partial carry valid */ });
  it('cold start: rows 0 → [] (throw 아님)', async () => {
    expect(await getIncumbents({ track: 'short', month: '2026-06', client })).toEqual([]);
  });
  it('최신 그룹이 TRACK_SELECT_COUNT를 초과하면(11) → throw incumbents_count_exceeded', async () => { /* short 11행 mock */ });
 ...
 * TRACK_SELECT_COUNT 초과만 fail-closed.
 */
...
git commit -m "feat(w2b): getIncumbents — 직전 트랙 rows 최신 month 그룹 capped 조회 + incumbent meta carry (D1, TDD)"
```
...
describe('buildIncumbentThesisContexts', () => {
  it('직전 row 필드 + 리포트 thesis + 실현 성과를 한국어 컨텍스트로 합성', async () => {
    // mock: stock_reports is_latest + month<=incumbent.month section_0 {headline, thesis: ['t1','t2','t3','t4']},
  ...
  it('incumbent month보다 미래인 stock_reports row는 직전 thesis로 사용하지 않음', async () => { /* lte(month, inc.month) 검증 */ });
...
## Task 6: upsertShortListTrack delta_status 실계산 + incumbent display meta fallback (TDD)
...
it('incumbent-only selected는 tier0 meta lookup 부재 시 incumbentMetadataByTicker로 name/composite/signal_label을 보존', async () => {
  await upsertShortListTrack('2026-06', 'short', tenShort, {
    client,
    incumbentTickers: new Set(['000001']),
    incumbentMetadataByTicker: {
      '000001': { name: '기존명', compositeScore: 72.5, signalLabel: '기존 신호' },
    },
  });
  const rows = rpcSpy.calls[0].args.p_rows as Array<{ ticker: string; name: string | null; composite_score: number | null; signal_label: string | null }>;
  expect(rows.find((r) => r.ticker === '000001')).toMatchObject({ name: '기존명', composite_score: 72.5, signal_label: '기존 신호' });
});
```

- [ ] **Step 2: 실패 확인** — Expected: FAIL (options.incumbentTickers/options.incumbentMetadataByTicker 미수용 / delta_status 전부 'new' / incumbent-only meta null).

- [ ] **Step 3: 구현**

`buildShortListRows`에 인자 추가(시그니처: `buildShortListRows(supabase, monthDate, buckets, selected, commentsByTicker?, incumbentTickers?, incumbentMetadataByTicker?)`):
```typescript
...
Tier0 meta lookup 이후 incumbent-only fallback 추가(lookup이 못 채운 필드만 보존). sector는 `agg.sector`의 canonical 검증분만 사용하고 fallback 대상에서 제외한다(B93 비-canonical 재유입 방지):
```typescript
...
```typescript
  options: {
    ...
    /** incumbent-only selected가 tier0_candidates_150에 없을 때 기존 카드 메타 보존 fallback. */
    incumbentMetadataByTicker?: Record<string, {
      name: string | null;
      compositeScore: number | null;
      signalLabel: string | null;
    }>;
...
```
→ `buildShortListRows(..., options.commentsByTicker, options.incumbentTickers, options.incumbentMetadataByTicker)` 전달. `upsertShortList30`(단발 보존 경로)은 incumbentTickers/metadata 미전달 = 전부 'new' + 기존 fresh-only meta lookup 유지.

- [ ] **Step 4: 통과 확인** — 기존 track-persist 테스트 전부 + 신규 3건 PASS.

- [ ] **Step 5: commit**

```bash
git add tudal/src/lib/data/admin-shortlist-persist.ts tudal/src/lib/data/__tests__/admin-shortlist-track-persist.test.ts
git commit -m "feat(w2b): upsertShortListTrack delta_status + incumbent-only display meta fallback (RPC pass-through, TDD)"
```
...
it('union enqueue: incumbent source 10 중 fresh와 5개 중첩 → fresh 50 + incumbent-only 5 = 55 jobs (무심사 탈락 금지)', async () => {
  // incumbentsSource → 10 incumbent, 그중 5개는 fresh와 중첩 / 5개는 fresh 밖
  // upsert 호출 rows 길이 55 + incumbent-only ticker 포함 검증
});
...
it('finalize: persist options.incumbentTickers + incumbentMetadataByTicker = incumbent set/meta + removed 구조화 로그', async () => {
  // selected에 incumbent 일부만 잔존 → console.info 'incumbent_removed' (탈락 ticker 목록) spy 검증
  // persistSpy options.incumbentTickers + incumbentMetadataByTicker 검증
});
it('carry 졸업 제외로 incumbent 9행이어도 abort 없이 union enqueue (partial 정당)', async () => { /* short 9개 mock */ });
it('incumbents가 TRACK_SELECT_COUNT를 초과하면(11) → abortBeforeSpend(incumbents_count_exceeded) (fail-closed, spend 0)', async () => { /* 11개 mock */ });
...
```
`RunTier1SelectionChunkInput`에 `incumbentsSource: IncumbentsSource;` + `buildIncumbentContexts: BuildIncumbentContexts;` 추가. `CallPersonaPanel` 타입에 `reflectionContext?: string` 추가(Task 3 계약 일치). `Persist` options에 `incumbentTickers?: ReadonlySet<string>` + `incumbentMetadataByTicker?: ...` 추가(Task 6 계약 일치).
...
```typescript
  ...
  // 부분 그룹은 carry_short_into_month가 midlong 졸업 ticker를 제외하는 정당 상태이므로 허용.
  ...
  const incumbentMetadataByTicker = Object.fromEntries(
    incumbents.map((i) => [i.ticker, {
      name: i.name,
...
```
(이하 기존 `candidates` 사용처 — enqueue rows·finalize 인자 — 는 union 결과를 그대로 사용. finalize 호출은 `finalizeSelection(input, candidates, incumbentTickers, incumbentMetadataByTicker)`로 변경한다. `bucketOf(c)`는 합성 candidate의 직전 bucket을 반환 → enqueue bucket 정합.)
...
(d) `finalizeSelection` — incumbents 전달 + delta/removed/meta carry (시그니처: `finalizeSelection(input, candidates, incumbentTickers, incumbentMetadataByTicker)`):
```typescript
  await input.persist(month, track, result.selected, {
    ...
    incumbentMetadataByTicker, // W2b D9 — incumbent-only display meta 보존
  });
...
- [ ] **Step 4: 통과 확인** — 기존 worker 테스트(트랙/finalize/preflight/deferred 전부) + 신규 8건 PASS. 기존 테스트는 DI 2종 mock 추가(`incumbentsSource: async () => []` 기본) 외 기대값 무변경 = 무회귀 증명. union +5 테스트는 full incumbent, partial 9 테스트는 carry 졸업 제외 정당 상태를 각각 고정한다.
...
git commit -m "feat(w2b): selection worker incumbent union + thesis context + delta/removed/meta carry (D27 Q5, TDD)"
```
...
3. **Type consistency:** `IncumbentInfo`(Task 1 SoT: thesis + display meta) ↔ getIncumbents 반환(Task 4) ↔ worker DI(Task 7). `callPersonaPanel({ticker, financials, reflectionContext?})`(Task 2 adapter = Task 3 persona-eval 타입 = Task 7 worker DI). `incumbentTickers?: ReadonlySet<string>` + `incumbentMetadataByTicker`(Task 6 persist = Task 7 Persist DI). `TRACK_SELECT_COUNT` import 일관.
...
- incumbent 무심사 탈락 금지: union enqueue + incumbentsSource 실패 throw + partial carry group 허용 + over-cap fail-closed 테스트 (Task 4,7).
