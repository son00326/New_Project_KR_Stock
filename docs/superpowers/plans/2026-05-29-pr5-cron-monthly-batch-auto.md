...
- **batch preflight**: `claim_next_report_jobs`가 stale sweep/attempt cap을 먼저 정리한 뒤 `preflightHardcap({month, callCount: pendingCount, maxCostPerCallKrw: ORCHESTRATE_TOTAL_COST_BUDGET_KRW}, {client})`. `pendingCount = pending/running count after claim`(resume 중복 reserve 방지). 초과 시 `cost_hardcap` alert + pipeline_health failed + abort.
...
| C8 | `src/lib/report/full-report-batch-worker.ts` (신규) | sequential loop driver(NOT allSettled): **(0) fail-closed 선행 검증(R2 HIGH-2 + R3 HIGH-1 + R3 MEDIUM-2, 첫 LLM 호출 전)**: (a) `CRON_SYSTEM_USER_ID` 존재 + UUID 형식 + auth.users 존재(`supabase.auth.admin.getUserById` admin API — `.from('auth.users')` 불가, R4 MEDIUM-1 lock) / (b) **`AI_COST_LOG_REAL_INSERT_ENABLED === 'true'` 강제(R3 HIGH-1)** — false면 insertCostLog noop → cost_log 0 → preflightHardcap fail-open으로 400k hardcap 미보장이므로 **LLM 호출 전 abort + alert**(spend 0); → `getActiveShortList({month: YYYY-MM-01, client: serviceRoleClient})` + **exactly 30 guard**(0/partial success 방지) → **enqueue idempotent**(`report_batch_job` ON CONFLICT DO NOTHING) → `claim_next_report_jobs(month, CHUNK_N)`(stale sweep/attempt cap 먼저 수행) → batch `preflightHardcap({month, callCount: pendingCount, ...}, {client})` → 각 job: `reportExistsAndCompleteForMonth`(section_0 AND section_7) SKIP+`mark_report_job(done)` / 아니면 `retryWithBackoff(()=>orchestrateFullReport(input, {client: serviceRoleClient, callerKind:'cron'}), {...})` → `mark_report_job(done/failed)` → 결과 누적 → `insertPipelineHealth` + 조건부 summary `scheduler_fail`. cost_hardcap throw 시 remaining 'deferred' + `cost_hardcap` alert + pipeline_health 실패 row + STOP. **run-mutex는 C9가 보유**(단일 worker 보장 → preflight cost 직렬화, R2 HIGH-1). input shaping = `triggerFullReport`(actions.ts:581)(**`adminUserId = process.env.CRON_SYSTEM_USER_ID` reserved UUID — §3.6/HIGH-3**, `tier1Verdict`/`consensusBadge`는 short_list_30 row source, 없으면 `'HOLD'`/`'🟡'` stub). |
...
**구현 완료 (C1~C10 + 마이그 0027 + 테스트)**: 마이그 0027 (report_batch_job + report_worker_run + claim/mark/acquire/release/upsert_cron 5 RPC, run_id fencing + attempts<3 sweep + cron UPSERT) + rollback / C1 getActiveShortList DI / C2 reportExistsAndCompleteForMonth(section_0 AND section_7) / C3 insertPipelineHealth / C4 emitCostAlert / C5 retryWithBackoff(critic/revise transient) / C6 COST_WARNING_THRESHOLD_KRW / C7 orchestrator cron-upsert 3-way branch / C8 full-report-batch-worker(step-0 fail-closed: cost flag + CRON_SYSTEM_USER_ID UUID + auth.users admin API + short_list_30 exactly-30 guard / 순차 / retry / systemic abort / cost_hardcap deferred / runGuardedReportChunk run-mutex wrapper) / C9 report-worker route(auth + flag + mutex + optional self-continue env-gated) / C10 vercel.json(daily cron + maxDuration 300) / C12 service-role allow-list / `.env.example` PR5 flag+system-user docs. **테스트 +24** (retry-with-backoff 11 + worker 9 + route 4 — wait 실측: 3 files 24 tests). 검증 게이트: build 26 routes / lint 0 err / **test:ci 121 files 1349 PASS** / tsc clean. grep gate clean.

**omxy direct-fix 반영 (실코드 R5)**: (1) `short_list_30` 30개 미만/초과 시 fail-closed + alert(0-report success 방지), (2) claim RPC를 batch preflight 앞으로 이동해 stale sweep 후 cost projection, (3) hardcap return도 pipeline_health/scheduler_fail 기록, (4) `CRON_SYSTEM_USER_ID` TS narrowing + test lint 정리, (5) `.env.example` PR5 env 문서화.
...
### 4.4 impl 리뷰 (omxy direct-fix + Claude verify — 워크플로우 반전, 사용자 명시 61차)

Claude 1차 impl(`89bd28d`) → **omxy direct-fix(catch-only 아닌 직접 수정, 23분)** → **Claude 최종 리뷰**. omxy 8 fix 모두 Claude 코드 검증 통과(맹목 수용 X):

- **(가장 중요) month/date 버그 fix**: `getActiveShortList`에 `YYYY-MM` 전달 → `short_list_30.month`는 **date 컬럼**(YYYY-MM-01)이라 0 rows 매칭. omxy가 `monthDate=${month}-01`로 정정. (Claude 독립 확인: getActiveShortList가 `eq("month", options.month)` + row.month는 date string → YYYY-MM-01 필요. 실 버그 확정.)
- **exactly-30 fail-closed**(short_list !=30 → abort+alert, B96 정합) + **claim 재배치**(stale sweep/attempt cap을 preflight 앞) + hardcap abort 시 pipeline_health failed 기록 + defer.
- **RPC error 체크 추가**(claim/count/mark/release — Claude 1차가 무시했던 error 처리: `markJob`/`releaseWorkerLock` helper) + runGuardedReportChunk double-release 방지(`chunkSucceeded` flag).
- **systemic abort job reset**(ai_key_unavailable 시 attempts reset, stranding 방지).
- **0027 mark_report_job**: `if not found → report_job_not_found`(silent no-op 방지).
- **.env.example PR5 env 문서화**(PR5_CRON_AUTO_ENABLED/PR5_CRON_SELF_CONTINUE/CRON_SYSTEM_USER_ID + placeholder UUID).
- grep false-positive cleanup(0027 committee-vote 표현).

**Claude 리뷰 verdict = APPROVED**. 독립 게이트 재확인: build 26 routes / lint 0 err 5 warn / test:ci 121 files **1349 PASS** / tsc clean. omxy 잔여 HIGH/MEDIUM 0.

**Claude 리뷰 finding (defer, LOW) → `W-pr5-shortlist-not-ready-quiet-skip`**: exactly-30 guard가 **0-rows(월 short_list 미seed)** 도 partial과 동일하게 abort+scheduler_fail alert → daily 반복 noise 가능. 권장: 0 → quiet skip(no alert, "not_ready" 200) / 1~29 → abort+alert / 30 → proceed. 비차단(step-0 통과 후 + USER-enable 후의 운영 edge case이며 alert도 informative). impl follow-up.
...
- T4 batch preflight: claim stale sweep 후 `callCount = pending/running count`; `currentTotal + pendingCount×535 > 400_000` 시 `cost_hardcap_40man` throw + `cost_hardcap` alert 1회 + remaining 'deferred' + pipeline_health failed + STOP(NOT failed).
