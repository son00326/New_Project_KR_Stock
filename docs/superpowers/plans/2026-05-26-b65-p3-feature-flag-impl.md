...
- `format-error.ts` 한국어 매핑 추가 (신규 2 keys + 1 prefix handler = 3 entries — omxy R1 Schop B4 + R3 Sartre W3 fix; line 691 PR body summary와 sync).
...
// strict 'true' enables admin path; unset/empty/'false' 모두 false fallback.
// .env.example default=false (safe local — omxy R1 Schop B5 + R2 Plato B3 fix); Production Vercel env만 USER가 true 설정 (§3.3.5 step, B98 lock-in).
const upsertEnabled =
...
**신규 추가 entries (2 keys + 1 prefix handler = 3 entries, omxy R3 Sartre W3 fix)**:
...
| (ii) upsert create + same-month UPDATE generated_at bump | Test 2 (orchestrator seam) + Test 4b **Phase A INSERT branch** + Test 4b **Phase B UPDATE branch + generated_at > old** | Test 2 = 단위 mock RPC name + payload invariant. Test 4b Phase A = missing-row INSERT branch SQL smoke. Test 4b Phase B = same-month UPDATE branch + generated_at bump verify (omxy R3 Sartre B2 fix). |
| (iii) section_8/consensus_badge preserve | Test 4a (payload key) + Test 4b Phase B (DB SELECT after UPDATE) | omxy R1 Schop B7 + R3 Sartre B2 fix — payload key + DB-side preserve 양쪽 검증. |
| (iv) same-month 재호출 UPDATE + generated_at bump | Test 2 + Test 4b Phase B | (ii)와 결합. generated_at bump invariant = `new_generated_at > old_generated_at` 검증 (Phase B4 SELECT). |
...
- forbidden grep **implementation-only scope** (omxy R1 Kepler B3 + R2 Plato B2 + R3 Sartre W1 fix): `tudal/src/lib/report/full-report-orchestrator.ts` 단 1개에서 `p_version|p_schema_version|p_regen_auto|p_regen_manual|p_is_latest` 0 매치. test 파일은 runtime `Object.keys(payload).sort()` exact equality assertion으로 검증 — literal 박제 합법.
...
**Test 4b (DB integration smoke, 2-phase, omxy R2 Plato W7 + R3 Sartre B1+B2 fix)**:

⚠️ **schema verified (omxy R3 Sartre B1 fix)**: `stock_reports` 컬럼 (0003 line 15-35) = `id / ticker / month / version / schema_version / is_latest / section_0..8 / appendix / regen_auto_count / regen_manual_count / generated_at`. **`name`, `sector` 컬럼 없음** (short_list_30 테이블에만 존재). 따라서 본 smoke는 stock_reports 직접 INSERT 시 name/sector column 미사용.

⚠️ **pseudo-smoke 어휘 명시 (omxy R3 Sartre B1 fix)**: 본 SQL은 **USER 치환 pseudo-smoke** (실제 production verify 시 `<ADMIN_USER_UUID>` + `<admin@example.com>` 부분만 실제 값 치환 후 mcp__supabase__execute_sql 또는 psql 실행). placeholder 잔존 = USER 의도적 치환 지점.

⚠️ **precondition**: 마이그 0025 + 0017 + 0003 production 적용 + admin user UUID 확보 (실제 ADMIN_EMAILS 등록된 user). production 직접 X — staging branch 또는 production verify 단일 test row 박제 후 cleanup.

**Phase A — missing-row INSERT branch (spec invariant ii: upsert create)**:

```sql
-- A1. Pre-condition: 해당 (ticker, month) 부재 확인
delete from public.stock_reports where ticker = '999999' and month = '2026-12-01';
select count(*) from public.stock_reports where ticker = '999999' and month = '2026-12-01';
-- 기대: 0
...
select
  ticker, month, section_0, section_8, consensus_badge,
...
-- 기대 결과 (Phase A INSERT branch):
--   ticker = '999999' / month = '2026-12-01' (정상 INSERT)
--   section_0..7, appendix = '{"v":"insert"}' (RPC 입력값)
--   section_8 = null (admin path 미터치)
--   consensus_badge = null (admin path 미터치)
--   version = 1, schema_version = 1, is_latest = true (default)
--   regen_auto_count = 0, regen_manual_count = 0 (default)
--   generated_at = now() (RPC 내부 박제)
```

**Phase B — same-month UPDATE branch (spec invariant ii/iii/iv/vi: same-month UPDATE + generated_at bump + section_8/consensus_badge/version/regen_* preserve)** — Phase A 직후 같은 row에 두번째 호출:

```sql
-- B1. Pre-condition: Phase A row 존재 + section_8/consensus_badge를 cron path 시뮬레이션 INSERT (commit_persona_eval 우회)
update public.stock_reports
  set section_8 = '{"partA":[{"persona_id":"core-1","vote":"BUY"}],"partC":{},"partD":[]}'::jsonb,
      consensus_badge = '🟢'
  where ticker = '999999' and month = '2026-12-01';

-- B2. 기존 generated_at 캡처
select generated_at as old_generated_at from public.stock_reports
  where ticker = '999999' and month = '2026-12-01';
...
-- 기대 결과 (Phase B UPDATE branch):
--   section_0..7, appendix = '{"v":"update"}'                                      (UPDATE branch 적용 ✓)
--   section_8 = '{"partA":[{"persona_id":"core-1","vote":"BUY"}],"partC":{},...}' (preserve ✓ — invariant iii)
...
--   generated_at > old_generated_at (Phase B2에서 캡처한 값)                       (bump ✓ — invariant ii/iv)

-- B5. Cleanup
delete from public.stock_reports where ticker = '999999' and month = '2026-12-01';
```

**Phase A + B 통과 = spec invariant ii (upsert create + same-month UPDATE) + iii (preserve) + iv (UPDATE + generated_at bump) + vi (version/regen 불변) 모두 검증**.

- **CI 환경 fallback (omxy R1 Schop W2 + R3 Sartre B1 fix)**: Vitest in supabase local test environment에서 동등 verification. CI에서 production smoke 직접 X — staging supabase project 분리 권장.
- impl PR body acceptance criteria에 명시 (production verify USER + cleanup row).
- `<ADMIN_USER_UUID>` + `<admin@example.com>` = production admin profile에서 USER 실제 값 치환 (pseudo-smoke 의도적 placeholder, R3 Sartre B1 fix).
...
| R-2 | feature flag toggle false 시 기존 path regression | TDD Test 1 (flag=false 시 B65-P1 guard 동일 동작) + 기존 test:ci baseline PASS 강제 (절대 count 박제 금지 — omxy R3 Sartre W4 fix). |
