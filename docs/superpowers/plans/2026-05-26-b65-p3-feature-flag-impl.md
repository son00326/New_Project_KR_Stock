...
|---|---|---|
| (i) admin auth + RLS bypass (auth.uid null → auth_unavailable / is_admin=false → admin_required) | Test 7 (admin auth + service_role deny) | DB invariant pin — Layer 1 (has_function_privilege) + Layer 2 (PostgREST permission_denied). 마이그 dry-run smoke. |
...
**Test 4b (DB integration smoke, mcp__supabase__execute_sql dry-run, §6.2 박제)**:
```sql
-- precondition: 마이그 0025 + 0017 적용 후 staging supabase (production 직접 X)
-- 1. INSERT row with section_8 = {persona_id: 'core-1', vote: 'BUY', ...} (commit_persona_eval 또는 직접)
insert into stock_reports (ticker, month, section_8, consensus_badge, ...)
  values ('123456', '2026-06-01', '{"partA":[],"partC":{}}'::jsonb, '🟢', ...);

-- 2. authenticated 컨텍스트로 upsert_report_sections_0_7_admin 호출 (section_0~7 + appendix만 변경)
set local role authenticated; set local request.jwt.claims = '{"sub":"<admin-uuid>","email":"<admin@>"}'::jsonb;
select public.upsert_report_sections_0_7_admin('123456', '2026-06',
  '{"a":1}'::jsonb, '{"a":2}'::jsonb, '{"a":3}'::jsonb, '{"a":4}'::jsonb,
  '{"a":5}'::jsonb, '{"a":6}'::jsonb, '{"a":7}'::jsonb, '{"a":8}'::jsonb,
  '{"a":9}'::jsonb);

-- 3. SELECT 후 section_8 + consensus_badge + version/schema_version/regen_* 모두 변경 0 검증
select section_8, consensus_badge, version, schema_version, regen_auto_count, regen_manual_count
  from stock_reports where ticker='123456' and month='2026-06-01';
-- 기대: section_8 = {"partA":[],"partC":{}} (변경 0) / consensus_badge = '🟢' / version=1 / regen_*=0
```
- **본 smoke는 production 직접 X — staging 또는 마이그 dry-run apply 후 즉시 검증** (CI 환경 fallback: skip + USER manual 박제).
- impl PR body acceptance criteria에 명시.
...
### Test 7 — admin auth + service_role deny (spec invariant i, omxy R1 Kepler B1 fix)
...
-- 기대: false
select has_function_privilege('authenticated', '...'::regprocedure, 'EXECUTE');
-- 기대: true
```
...
const { error } = await sr.rpc('upsert_report_sections_0_7_admin', { p_ticker: '123456', p_month: '2026-06', p_section_0: {}, ... });
...
| payload field drift (section_8 누설) — **scope 제한 critical (omxy R1 Kepler B3 fix)** | scope `tudal/src/lib/report/full-report-orchestrator.ts` + `tudal/src/lib/report/__tests__/orchestrate-upsert-flag.test.ts` + `tudal/src/app/(admin)/admin/portfolio/__tests__/triggerFullReport-flag.test.ts` 만: `p_section_8\|p_consensus_badge\|p_version\|p_schema_version\|p_regen_auto\|p_regen_manual\|p_is_latest` 0 매치 |
