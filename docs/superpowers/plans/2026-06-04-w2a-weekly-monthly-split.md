...
- **D4 rolling writer = 원자적 RPC.** **(omxy R1 HIGH-1 + MED-4)** 클라이언트측 DELETE-then-UPSERT(비원자, reader가 10/20/0 부분상태 관측 + report worker `length!==30` abort)를 폐기. 신규 SECURITY DEFINER RPC `replace_shortlist_track(p_month date, p_track text, p_rows jsonb)`가 **단일 트랜잭션**에서 (a) cross-bucket overlap fail-closed 체크(신규 ticker가 동일 month의 **다른** bucket에 이미 존재 → `shortlist_track_cross_bucket_overlap` raise) (b) bucket-scoped DELETE(`bucket IN (트랙 buckets) AND ticker NOT IN (new)`) (c) UPSERT onConflict(month,ticker). count gate 트랙별(short=10 / midlong=20)은 TS writer(`upsertShortListTrack`)가 RPC 호출 전 선검증. `upsertShortList30`(단발 경로용)은 보존하되 W2a 청크 finalize는 신규 RPC writer 사용.
...
- **D6 finalize 게이트 = job count 기반(저장 expected_total 폐기).** **(omxy R1 HIGH-3 순서 모순 해소)** acquire(run-mutex) → enqueue → process 순서상 acquire 시점엔 enqueue 수를 모름. ∴ `run.expected_total` 컬럼/acquire 인자 **신설 안 함**. finalize 트리거 = `openJobs(period_key) === 0 AND totalJobs(period_key) > 0`(전 enqueue job이 terminal). 최종 count 정합은 트랙별 schema(D7)가 검증. (enqueue는 onConflict ignoreDuplicates 멱등 → totalJobs = 풀 크기.)
...
- **D8 월초 carry-forward (omxy R1 HIGH-2).** 월초(1일≠월요일)에 midlong이 새 월 `m:M`을 쓰면 그 주 short period는 직전 월(`s:전월 마지막 월요일`→month=전월)이라 새 월 M에 short 0 → 30 미달로 report worker abort + 홈/포트 20행. **fix:** midlong finalize가 month M을 쓸 때, `replace_shortlist_track('midlong')`와 동일 트랜잭션(또는 직후 RPC `carry_short_into_month(p_month)`)에서 **month M에 short 행이 없으면 직전(최신 short 보유 month)의 short 10을 month M으로 carry(`delta_status='hold'`, rank 보존)**. 다음 월요일 short run이 in-place로 refresh. ⇒ 모든 월이 1일부터 30행. short_run의 month=run 월요일의 캘린더월(monthYMOfPeriod) 유지(1일≤그 달 첫 월요일이므로 그 달 midlong은 이미 존재).
...
- **전 RPC**: SECURITY DEFINER + `set search_path = public, pg_temp` + **revoke execute from public, anon + grant execute to authenticated, service_role 명시 재선언**(0027 Kepler B2: CREATE OR REPLACE는 과거 grant 미보존).

- [ ] **Step 4: 신규 원자적 shortlist writer RPC (HIGH-1 + MED-4)**

`replace_shortlist_track(p_month date, p_track text, p_rows jsonb)` — 단일 트랜잭션:
```sql
create or replace function replace_shortlist_track(p_month date, p_track text, p_rows jsonb)
returns void language plpgsql security definer set search_path = public, pg_temp as $$
declare
  v_buckets text[] := case when p_track = 'short' then array['short']
                           when p_track = 'midlong' then array['mid','long']
                           else null end;
  v_new_tickers text[];
begin
  if v_buckets is null then raise exception 'replace_shortlist_track_bad_track:%', p_track; end if;
  select array_agg(value->>'ticker') into v_new_tickers from jsonb_array_elements(p_rows);
  -- (MED-4) cross-bucket overlap fail-closed: 신규 ticker가 동일 month 다른 bucket에 존재하면 abort
  if exists (
    select 1 from short_list_30 s
    where s.month = p_month and s.ticker = any(v_new_tickers) and not (s.bucket = any(v_buckets))
  ) then
    raise exception 'shortlist_track_cross_bucket_overlap';
  end if;
  -- (HIGH-1) atomic: bucket-scoped DELETE + UPSERT 동일 트랜잭션
  delete from short_list_30 s
   where s.month = p_month and s.bucket = any(v_buckets) and not (s.ticker = any(v_new_tickers));
  insert into short_list_30 (month, ticker, bucket, rank, ...)  -- 모든 컬럼 jsonb→컬럼 매핑
  select p_month, value->>'ticker', value->>'bucket', (value->>'rank')::int, ...
...
`carry_short_into_month(p_month date)` — (D8/HIGH-2) month에 short 행 부재 시 최신 short carry:
```sql
...
begin
  ...
  if v_src is null then return; end if;  -- cold start: carry 없음(첫 월)
  insert into short_list_30 (month, ticker, bucket, rank, delta_status, ... )
  select p_month, ticker, 'short', rank, 'hold', ...  -- 직전 short 10 복사, month만 교체
    from short_list_30 where month = v_src and bucket = 'short'
  on conflict (month, ticker) do nothing;
end; $$;
-- revoke public/anon + grant authenticated/service_role
```
> jsonb→컬럼 매핑은 short_list_30 전 컬럼(0002+0012+0029) 포함 — Task 6 writer가 보내는 row shape와 1:1. NULL 허용 컬럼은 `value->>'x'` null-safe.
...
1. `upsertShortListTrack('2026-06','midlong', 20개 mid/long, {client})` → `client.rpc('replace_shortlist_track', { p_month:'2026-06-01', p_track:'midlong', p_rows: [20 rows] })` 호출 **+ 이어서** `client.rpc('carry_short_into_month', { p_month:'2026-06-01' })` 호출.
2. `upsertShortListTrack('2026-06','short', 10개 short, {client})` → `replace_shortlist_track('short', [10])` 호출, **carry 미호출**(short 경로).
3. short selected=9 → throw `shortlist_track_count_mismatch:short:9!=10` (RPC 미호출). midlong=19 → throw.
4. selected가 트랙 외 bucket 포함(예 short 트랙에 mid row) → throw `shortlist_track_bucket_impurity` (RPC 미호출).
5. ticker 형식 위반(TICKER_RE) → throw(RPC 미호출). degraded ticker comment 없음 → row의 ai_comment_kr null.

```typescript
it('midlong writer = replace RPC + carry RPC (DoD: short 보존은 DB RPC가 atomic 처리)', async () => {
  await upsertShortListTrack('2026-06', 'midlong', twentyMidLong, { client });
  expect(rpcSpy.calls.map(c => c.fn)).toEqual(['replace_shortlist_track', 'carry_short_into_month']);
  expect(rpcSpy.calls[0].args.p_track).toBe('midlong');
});
it('short writer = replace RPC only (carry 미호출)', async () => {
  await upsertShortListTrack('2026-06', 'short', tenShort, { client });
  expect(rpcSpy.calls.map(c => c.fn)).toEqual(['replace_shortlist_track']);
});
```
> DELETE bucket-scope·cross-bucket overlap·원자성은 **RPC(Task 5 Step 4) 내부**에서 보장 — TS 단위테스트는 RPC 호출 인자/순서/선검증을 검증하고, RPC 자체 로직(atomic DELETE+UPSERT, overlap raise, carry)은 Task 5의 SQL 및 (선택) 통합 검증으로 커버.
...
export async function upsertShortListTrack(
  ...
  const { error } = await supabase.rpc('replace_shortlist_track',
    { p_month: monthDate, p_track: track, p_rows: rows });
  if (error) throw new Error(`shortlist_persist_failed:${error.code ?? 'unknown'}`);
  if (track === 'midlong') {                                  // (D8) 월초 carry-forward
    const { error: cErr } = await supabase.rpc('carry_short_into_month', { p_month: monthDate });
    if (cErr) throw new Error(`shortlist_carry_failed:${cErr.code ?? 'unknown'}`);
  }
}
```
...
- [ ] **Step 1: 실패 테스트** — 워커 input에 `{track, periodKey, month}` 추가. enqueue가 period_key/track 기록. claim이 p_period_key 사용. countOpenJobs/totalJobs가 period_key 필터. **(D6/HIGH-3)** finalize 트리거 = `openJobs(periodKey)===0 AND totalJobs(periodKey)>0`(저장 expected_total 없음). finalize가 `runScreening({track})` 후 `upsertShortListTrack(month, track, selected)`. short 트랙: 50 enqueue → 10 selected → short bucket persist. preflight callCount = openJobs(periodKey) × 11. 빈 풀(totalJobs=0)에서 finalize 미발동 검증.
- [ ] **Step 2: 실패 확인.**
- [ ] **Step 3: 구현**
...
- countOpenJobs(:258-271): `.eq('period_key', periodKey)`. + 신규 `countTotalJobs(periodKey)`(status 무관 전체).
- preflight(:356-375): callCount = openJobs(periodKey) × 11 × `getRoleMaxCostPerCallKrw('tier1_panel')`(W0 무회귀).
- finalize(:515-566): **게이트 = `openJobs(periodKey)===0 && totalJobs(periodKey)>0`** (`terminal.length === EXPECTED_TOTAL` 폐기). select where period_key; `runScreening({candidates, track, ...replay})`; persist = `upsertShortListTrack(month, track, selected, {client, commentsByTicker})`; `mark_selection_finalized(periodKey, runId)`.
