...
**SoT:** `Document/Process/HANDOFF.md` ⭐ 65차 W1(:53-57) + D26 Q4 + D28 B-final + `model-registry.ts` `D28_DEBATE_CONFIG`(slots 6/5 · r2BoundaryWindow 5 · r2VarianceTopFraction 0.2 · maxRounds 2) + `projectD28MonthlyReservationKrw`(350,167≤50만 — R1 mix+R2 worst 이미 포함).
...
- **D3 R2 대상 = 결정론 순수 함수 `computeR2Targets`.** 입력 = R1 done rows의 `(ticker, panel)` + 트랙. ① 트랙 활성 tf마다 `computeWeightedScores(panel)[tf]` desc 순위(동점 ticker asc) → **rank ∈ [10−w+1, 10+w]** (w=`r2BoundaryWindow`=5 → 6..15위) ② persona 11명 점수 분산(해당 ticker의 active tf 중 최대 가중점수 tf 기준 모표준편차) 상위 `r2VarianceTopFraction`(20%, ceil) ③ 합집합 + ticker asc 정렬. **degraded(panel null/failed) ticker 제외**(R1 없음 → 반박 불가, ⚪ 유지). pool < 10+w여도 안전(rank 범위 교집합). `computeWeightedScores`는 persona-eval 모듈 private → **export로 승격**(이동 없음, 재구현 금지).
- **D4 R2 = round=2 jobs, 큐 메커니즘 재사용.** R1 nonTerminal==0 && terminal>0 시점(현 finalize 게이트 지점)에서: targets = computeR2Targets(...) → **targets 비어있으면 즉시 finalize**(현행 동일) / 있으면 round=2 rows idempotent enqueue(onConflict `period_key,ticker,round`) 후 **이번 invocation은 finalize 안 함**(다음 invocation/self-continue가 R2 claim). claim/mark/mutex/preflight RPC 무변경 — claim은 period_key 단위라 round 무관 pending을 집는데, R2 rows는 R1 전부 terminal 후에만 존재 → 라운드 순서 자연 보장. **finalize 게이트 확장**: 전 라운드 nonTerminal==0 && terminal>0 **&& (R2 불필요 ∨ R2 targets 전부 round=2 row 존재)** — targets는 R1 저장 panel의 순수 함수라 매 invocation 재계산 가능(저장 플래그 불필요, 결정론 멱등). R2 job 실패(attempts 소진 failed)는 terminal → 해당 ticker R1 점수 유지(graceful).
...
- **D7 마이그 0032 (0031 applied → ALTER 신규).** `alter table tier1_selection_job add column round smallint not null default 1 check (round in (1,2))` + `drop constraint <unique period_key,ticker>` → `add unique (period_key, ticker, round)` + 기존 (period_key, status) 인덱스 유지. **RPC 7종 본체 무변경** — claim/mark/acquire/release/finalized/replace/carry 전부 round 무인지(설계상 불필요). rollback 짝 필수(컬럼 drop + unique 원복). dormant 작성 → USER apply 게이트.
...
it('provider 429/“rate limit”/ECONNRESET 류 → ai_call_failed:transient:* throw', async () => {});
...
```
- retry-with-backoff.ts TRANSIENT_CODES에 `'ai_call_failed:transient'` 추가.
...
do $$ begin
  if not exists (
    select 1 from pg_constraint where conname = 'tier1_selection_job_period_ticker_round_key'
  ) then
    alter table public.tier1_selection_job
...
```
(rollback = unique 원복 + 컬럼 drop. **주의**: round=2 rows 존재 시 rollback의 unique(period_key,ticker) 원복이 충돌 → rollback 선두에서 `delete from tier1_selection_job where round > 1` 후 원복 — rollback 파일에 명시.)
...
- claim 후 chunk loop: job에 `round` 포함 SELECT(claim RPC returning에 round 포함 — RPC는 `returning *` 형태면 무변경; 아니면 TS에서 jobs id로 round 조회 1콜). round===2면: 같은 (period_key, ticker) round=1 row의 panel_result 로드 → `callDebatePanel`; round===1이면 기존 `callPersonaPanel`.
...
- 카운트 헬퍼: 기존 period_key 단위(라운드 무관) 유지 — finalize 게이트는 라운드별 보조 카운트 2개 추가(`countByStatusRound`).
...
1. **Spec coverage:** HANDOFF W1(:53-57) → D28 mix(Task 1,2,3,7) / 멀티라운드 반박 ≤2(Task 4,5,6 — maxRounds=2는 R1+R2 구조 자체로 박제) / R2 선택적 config 소비(Task 4) / incumbent context 토론 주입(Task 3,6 — reflectionContext seam 재사용) / consensus 결정론 유지(Task 6 replay 무변경) / judge·dual-judge=W1b 명시 분리 / follow-up ⑤ retry classifier(Task 2).
