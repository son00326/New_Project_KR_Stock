...
# 28 samples 한번에 dump (operator가 복붙하면 정확히 28 prompts 출력):
cd tudal && npx tsx -e "
import { resolveSectorPersona } from './src/lib/ai/prompts/personas/sector-persona-builder';
const ids = [
  ...
  'sector-금융-slot-8',           'sector-금융-slot-11',
  'sector-2차전지-slot-4',        'sector-2차전지-slot-12',
  'sector-자동차-slot-8',         'sector-자동차-slot-11',
  'sector-IT/SW-slot-8',          'sector-IT/SW-slot-12',
  'sector-유통/소비재-slot-8',    'sector-유통/소비재-slot-11',
  'sector-에너지-slot-4',         'sector-에너지-slot-11',
  'sector-엔터/미디어-slot-8',    'sector-엔터/미디어-slot-12',
  'sector-통신-slot-10',          'sector-통신-slot-11',
  'sector-철강/소재-slot-4',      'sector-철강/소재-slot-12',
  'sector-운송/물류-slot-8',      'sector-운송/물류-slot-13-subtag-조선',
  'sector-보험/증권-slot-8',      'sector-보험/증권-slot-12',
];
// 28 samples 정합 자체 검증 (omxy R2 catch 박제 — operator가 false-coverage 회피)
if (ids.length !== 28) { console.error('expected 28, got ' + ids.length); process.exit(1); }
for (const id of ids) {
  console.log('=== ' + id + ' ===');
