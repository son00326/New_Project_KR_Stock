...
| 2026-06-04 | **v2.9** | **65차 후속 — D27 Q5 incumbent thesis 재점검 노트.** §8 Step 0에 "직전 리포트가 다음 선정의 입력" supersede 노트 추가 — 재선정 시 incumbent는 직전 리포트·논거+실현 성과("incumbent thesis context")로 재평가(유지=top10 랭킹 경쟁), 후보풀=fresh Tier 0 ∪ incumbents(W2 union, 무심사 탈락 금지). PR-K Reflection(Step 4 후속)과 별개 — Q5=선정 시점 review(MVP). 상세 SoT = `HANDOFF.md ⭐ 65차 MVP 엔진 섹션` + `ServicePlan-Admin §1A.5 D27`. |
| 2026-06-24 | **v3.0** | **D32 — Reflection/PR-K(AI 자가 학습) 출시 전 승격 (USER 결정).** §8 Step 4 후속(line ~793) 노트 갱신: Reflection/PR-K = "출시 후 defer" → **"출시 전 빌드 + S9 운용 검증 기간 중 실가동·검증"**(구 62차 doc-class supersede). 트랙별 회고 주기(주1/월1, 65차 Q1) + reflection_log 영속 + 다음 선정 prompt 주입(`reflectionContext` seam, Q5 incumbent thesis와 별개). 빌드 완료 = S9 진입 선행조건 · 동작 검증 = soft launch criterion(§2.2 7-criteria 미변경). 상세 SoT = `docs/superpowers/specs/2026-06-24-reflection-prk-pre-launch-promotion.md` + `ServicePlan-Admin §1A.5 D32`. |
| 2026-06-25 | **v3.1** | **B-PARTB — Section 8 Part B "쟁점" 실제 issue-extraction 구현 (출시 전 품질, 코드 완료·CONVERGED).** §8 ③("쟁점별 찬반 토론 인용 3~5건")이 그간 writer.ts `buildSection8AndVotes`에서 stub/raw-JSON(`content.slice`)으로 placeholder였던 것을 pure deterministic `extractIssueDebates`로 교체 — Core 11 응답(vote+one_line+argument_excerpt)에서 의견 대립 큰 쟁점 3~5개를 테마 카탈로그+voteRank polarity로 추출, 찬(pro)/반(con)/중(arbiter) 인용을 `${label}: ${one_line}` 형태로 합성. 같은-vote 강세 인용을 가짜 반대로 안 세움(con↔라벨 정합), raw JSON/stub/빈문자열 누출 0, 항상 3~5. **문서 §8 ③ 박제 의미 변화 없음**(처음부터 "찬반 인용 3~5"였고 이제 writer가 그 contract를 실제로 충족). 기존 30 리포트 DB 재생성(실 AI ~₩30k)=USER 비용 게이트(미실행). branch `tier0-bpp-multiregime`(main 미머지). SoT = `docs/superpowers/specs/2026-06-25-section8-partB-issue-extraction.md`. |
