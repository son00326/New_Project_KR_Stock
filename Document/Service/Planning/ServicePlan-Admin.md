...
- R3.7-7: 섹터별 합산 표(섹터명 · 찬성 · 반대 · 기권)와 핵심 토론 인용 3~5건(찬성 측 1~2건 + 반대 측 1~2건 + 중립 1건)을 표시한다.
- R3.7-8: 투표 패널의 인터랙티브 페르소나 탐색(개별 위원 프로필·논거 상세)은 Should(S2) 범위. MVP에서는 정적 집계 표만 제공.
...
- **Story**: 어드민으로서, "왜 이 종목이 선정됐는가"를 논거 수준까지 추적하기 위해, Core·Sector 위원의 찬반 집계와 핵심 토론 인용을 Section 8에서 보는 기능을 원한다.
- **AC**:
  - [ ] Section 8에 Core 11 위원 찬/반/기권 카운트와 관련 섹터 Board 위원 찬/반/기권 카운트가 각각 집계되어 표시된다.
  - [ ] 섹터별 합산 표와 핵심 토론 인용 3~5건(찬성 1~2·반대 1~2·중립 1)이 정적 텍스트로 렌더된다.
#### DoD (Definition of Done) — M3 투심위 투표 요약 패널
- [ ] Section 8에 Core 11 위원 찬/반/기권 카운트와 관련 섹터 Board 위원 찬/반/기권 카운트가 각각 집계되어 표시된다.
- [ ] 섹터별 합산 표(섹터명·찬성·반대·기권)와 핵심 토론 인용 3~5건이 정적 텍스트로 렌더된다.
- [ ] 인터랙티브 페르소나 탐색(개별 위원 프로필·논거 상세)은 표시되지 않는다(Should S2 범위).
- [ ] `npm run build` + `npm run lint` 통과.
