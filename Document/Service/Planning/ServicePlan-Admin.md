...
Status: **v1.6 (2026-05-19, S7a 49차) — 5종 합의 배지 박제 (🟡 관망 신규). Q5b omxy CONVERGED: 비-top tier + 비-top tier + AI 가용 케이스를 ⚪(AI 분석 대기)와 구별해 🟡 관망으로 분류. §1A.5 D19 + §3.1 R3.1-6 + Short List 30 컬럼 명세 + §3.7 R3.7-? + §4.X stock_reports.section_8 canonical contract + consensus_badge 컬럼 갱신. (v1.5 이력: 2026-05-08 D19 — JooPick AI 강화 Tier 0/1/2 병렬 + 합의 배지(4종) + Reflection. v1.4 이력: 2026-05-08 D18 — S8 자동매매 진입 시점 재조정 + KIS 발급 비블로커화. v1.3 이력: 2026-04-22 D17 — DQ-7 per-admin UI + AES-256-GCM + Vercel 첫 배포 트랙. v1.2 이력: D16 어드민 내부 도구 재정의 + 자동매매 S8 승격.)**
...
| consensus_badge | text NULL | 합의 배지 5종 enum (`strong / numeric / ai / wait / pending`). **v1.6 (49차)** 신규. Legacy nullable (backward compat); S7a 신규 row는 RPC가 NOT NULL 강제 (commit_persona_eval / commit_badge_only RPC body — 마이그 0017). UI 표시: NULL → ⚪ fallback 렌더 (Plan R3 BLOCKER 7). 코드 SoT: `tudal/src/lib/screening/consensus.ts` 5종 type union. |
...
|---|---|---|---|---|
| `partA` | array(sectorVoteRow) | optional | **0 또는 14** | Sector Board 위원별 한 줄 의견 표. **본 PR 범위 B** = `0` (Tier 2 deferred). Tier 2 활성 시 = `14` (해당 종목 섹터 14명 전원). **1~13은 invalid** (R3.7-7 ①, omxy R1 BLOCKER 2). |
