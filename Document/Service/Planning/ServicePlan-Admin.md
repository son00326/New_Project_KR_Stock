...
| **D22** | Tier 2 production sector persona prompts quality target = Kevin v3.1 reference 박제 (2026-05-20, 52차) | (a) **product/spec decision**: D21에서 박제한 canonical 14 sectors × 14 personas/sector overlay (196 persona)의 **실제 production system prompts 작성 시 quality target은 `origin/IMVCOM @ 1faee1b` Kevin v3.1 초보 친화 알테오젠 리포트 reference**. 단순 lorem-ipsum / "Mock Persona" stub 어휘 금지. (b) **Kevin v3.1 reference 4 commits** (origin/IMVCOM, main 미반영): `44f6151` 알테오젠 HTML + `3b22d01` v2.1 + 알테오젠 v2 + `c00153c` 삼천당 풀 리포트 + `1faee1b` v3.1 초보 친화 + NarrativeDesign + Section4-Trial. (c) **Step 3a (Kevin IMVCOM 정합 머지 PR)** 선행 필수 — Kevin v3.1 진화분 (ReportFramework v3.1 + NarrativeDesign + Section4-Trial)이 main에 정합 머지된 후, Step 3b에서 그 reference를 분석하여 196 persona system prompt 박제. (d) **Step 3b 진입 시점**: PR #4 (Tier 2 SoT) + PR #5 (Tier 2 impl) 머지 후 → Step 3a Kevin 정합 머지 → Step 3b production prompts 196. (e) **quality follow 항목**: Section별 narrative 톤 (초보 친화 vs 전문가) / 비유·해설 패턴 (예: 알테오젠 = "기술 라이선싱 = 좋은 레시피 빌려주기") / 섹터별 깊이 가이드 (바이오 = 임상 단계·FDA, 반도체 = 공정 노드·CAPEX 등) / 비교 분석 테이블 형식. (f) **상세 follow target 분석은 Step 3a 분석 단계에서 catalog**. | 본 §1A.5 D22 (본 행) · §8 v1.8 changelog · `Document/Process/HANDOFF.md §5 SoT 표 + §9 운영 원칙 박제 [[handoff_kevin_v31_quality_target]]` 동시 박제 |
