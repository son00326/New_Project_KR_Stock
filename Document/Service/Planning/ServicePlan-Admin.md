...
Status: **v1.3 (2026-04-22) — DQ-7 Admin Credential System 재설계 박제. per-admin UI 입력 + AES-256-GCM 암호화 + Vercel 첫 배포 · S7a 선행 트랙. D17 추가. (v1.2 이력: D16 어드민 내부 도구 재정의 + 자동매매 S8 승격.)**
...
| **D6** | Short List 30 vs 백테스트 갭 | **Must 고정 (MVP 축소 불가)**: Short List **30 (단기 10 · 중기 10 · 장기 10)** 표시. 종목 수·관점 분배 변경 금지. 분석 로직은 인디케이터뿐 아니라 **AI 투심위(Core 11 + Sector 14×10, `ReportFramework.md`) 판단 기준 + 리포트 워딩(Section 0~8)** 포함. **백테스트는 6종목에서 시작 → 같은 알고리즘으로 점진 확장**. 백테스트 6종 단기/중기/장기 배분은 P3.1/B3에서 결정 (Short List 30 노출 구조와는 무관). | §3, §4 |
...
- **Short List 30 홈 구조**: **3 고정 섹션 세로 스택** (단기10 → 중기10 → 장기10). 탭·단일 스크롤 기각 — 30종목 동시 비교·버킷 간 delta 파악이 J1 핵심이므로 섹션 스택으로 확정. 각 섹션 컬럼: 종목명·섹터·5-Signal Composite 점수·투심위 미니바·목표가 괴리율·7일 스파크라인·NEW/HOLD/REMOVED 배지.
