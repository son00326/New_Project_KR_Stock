...
운영 제약: Accept go-live(MVP②)가 여전히 #1 critical path이며 리디자인이 막지 않는다. 출시 runbook 순서(Accept→B-SEL-CRON→S7b→D11→S7c→S7d→pre-launch 섹터 비교/Tier2/D4→**PR-K Reflection 빌드(D32)**→S9→출시)에 디자인은 그 위에 결합되는 병행 트랙이다(D32로 PR-K가 S9 직전에 추가된 것 외 순서 불변). 면책 Footer 등 기존 제약은 무변경.
