...
- R3.12-3: 비용 기여 상위(종목·페르소나·섹션·모델) 브레이크다운 테이블을 위젯 하단에 제공한다. Top 5 비용 기여 항목을 일별/월별로 스위치하여 조회 가능. 프롬프트 최적화 판단 지원. **65차 Q3**: '모델' 차원 = 멀티프로바이더(Claude+GPT)·역할별 모델 차등(토론=저가 Haiku/GPT-mini · judge·리포트=고가 Opus 4.8+). 비용 브레이크다운에 provider 컬럼 추가 검토. 모델 하드코딩 제거+레지스트리화는 W0 구현. (HANDOFF.md ⭐ 65차 MVP 엔진 섹션 참조)
