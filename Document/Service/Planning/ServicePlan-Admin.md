...
- R3.8-3: Short List 테이블에서 NEW(편입) / HOLD(유지) / REMOVED(제외) 배지를 각 종목 행에 표시한다. REMOVED 종목은 Short List에서 제외되지만 Delta 상세 패널에는 목록으로 유지한다. **(REMOVED 원인 2종 — ① 정기 재선정 시 알고리즘 탈락[delta_status=removed, 본 R3.8] ② M12a 뉴스 기반 자동 제외[mid-cycle, durable removal ledger, §3.10 R3.10-7b~c]. UI는 hover/아이콘으로 사유 구분 권장. delta_status 의미는 ①에 한정.)**
...
- R3.10-7: **자동 제외 조건** — `recommended_action=auto_remove`는 **direct + material + high-confidence + affected_ticker 명시**이며 (scope=company **또는** sector-wide가 해당 종목 thesis를 직접 깸)일 때만. scope=market/sector-general(직접 thesis break 없음) · low-confidence · unknown → **alert_only(자동 제외 금지)** + removal ledger의 negative-news context로만 적재.
- R3.10-7a: **안전장치(smart brake)** — 평상시(회사 고유 1~2종목)는 완전 자동. circuit-breaker(후보 산출 후): 1 run 자동제외 후보 > `max_auto_removals_per_run`(기본 3·config; 즉 4건↑) **또는** track floor 위반 → 그 run의 자동제외를 **전체 hold_for_review**(자동 제외 보류 + "대량 제외 감지·검토 요망" 알림, 부분 제외 안 함). floor = 홈 리스트 track 70%(short<7 / midlong<14 / full<21); 가상포트는 N≥10일 때만 70% floor 적용, 집중포트 N<10은 1건 direct/high-conf 자동 허용·≥2건 hold_for_review. = 폭락일·시장 전반 악재 안전판(순수 자동의 유일 예외).
...
- R3.10-7c: **durable removal ledger + 재진입** — 별도 lockout 없음. 자동 제외 사유는 **durable removal ledger**(`news_event` linked, 홈 리스트 row 삭제와 무관하게 영속)에 기록한다. 다음 정기 선정 시 최근 negative-news context를 **모든 Tier1 후보(fresh ∪ incumbent)** 에 주입(W2b incumbent-only seam과 **별개·additive**)하여 페르소나가 악재를 보고 재판단한다 → thesis 깨짐=재탈락 / 회복=재진입 허용. **E1 short_list_30 row semantics 불변**(delta_status new/hold/removed=정기재선정 diff와 별개). 이번 docs pass는 `alert_event.alert_type` closed enum에 removal 타입 **추가 안 함**; durable removal ledger는 별도 planned entity(news_event_id + optional alert_event_id link), AlertEvent enum 확장은 build-time migration 결정으로 defer. 정확 DB schema = build.
- R3.10-7d: **알림 채널** — 자동 제외 / 대량보류 알림은 **텔레그램 + 앱 내(`/admin/alerts`)** 발송. **이메일 미발송**(본 뉴스 기능 한정 — M15 Exit 이메일 백업 D10은 불변). 알림 형식 = 티커 + 뉴스 제목 30자 + 사유 1줄 + `/admin/alerts` 상세 링크.
...
- **AC**:
  ...
  - [ ] direct·material·high-confidence·회사고유(또는 thesis 직접 깨는 sector 악재)로 판정된 종목만 홈 리스트 + 가상포트에서 **자동 제외**되고(빼기만, freed→현금), 텔레그램 + 앱 알림이 발송된다. 거시/저확신/불명 = 알림만.
  - [ ] 안전장치: 1 run 4건↑(>max 3) 또는 track 70% floor 위반 시 그 run 전체가 hold_for_review로 보류되고 검토 알림만 발송된다(집중포트 N<10은 1건 자동·2건↑ 보류).
  - [ ] 자동 제외 사유가 durable removal ledger에 기록되고, 다음 정기 선정 시 모든 Tier1 후보에 negative-news context로 주입되어 재판단된다(lockout 없음).
#### DoD (Definition of Done) — M12a 뉴스 기반 자동 제외 *(planned — 코드 빌드는 별도 future task)*
- [ ] AI 페르소나 평가 → recommended_action 판정 → auto_remove 조건(direct/material/high-conf/affected_ticker + company|thesis-breaking-sector) 충족 시에만 자동 제외(빼기만), freed weight 현금화, 다음 cron refill.
- [ ] smart brake(max_auto_removals_per_run + track 70% floor) 동작, 초과 시 전체 hold_for_review + 알림.
- [ ] 텔레그램 + 앱 알림(이메일 미발송), durable removal ledger 기록 + 다음 선정 candidate-level context 주입(fresh∪incumbent).
- [ ] `npm run build` + `npm run lint` 통과.
...
- R3.12-2: **35만원 도달 시**〔현재 코드값/기존 경보값 — 65차 W0에서 재산정〕 경보 알림(텔레그램·이메일)을 어드민 3인에게 발송한다. **50만원 도달 시 자동 재생성 차단 하드 캡**(65차 hardcap 40만→50만 정정)이 발동하여 수동 재생성 버튼(§3.4 R3.4-4)과 자동 재분석(R3.3-3)을 모두 차단한다. 사용자 override 액션으로만 해제 가능하며, override는 `admin/settings` 별도 토글로 로그 기록.
...
- **DoD**:
  ...
  - [ ] 35만원 도달 시〔현재 코드값/기존 경보값 — 65차 W0에서 재산정〕 텔레그램·이메일 경보가 발송되고, 50만원 도달 시(65차 hardcap 40만→50만 정정) 자동 재생성 하드 캡이 실제로 재생성 경로를 차단한다.
...
| **정정 후 (메인 path, 64차 현황)** | **Tier 1 AI 30 선정** (Tier 0 150 후보 → Core 11 AI 평가 + 시간대별 페르소나 가중치 → 단/중/장 top 10 = 30) | **0029 applied**(AI 컬럼 8종) + 0028(tier0_candidates_150) + 0031(선정 청크 워커, written-not-applied dormant). 컬럼 재사용 확정(신규 마이그 불요) | 150 후보 × Core 11 (PR-E 150/150 게이트) | D19 + D23 메인 path 박제 |
