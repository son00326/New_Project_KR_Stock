...
Status: **v1.1 (2026-04-15 13차 후속) — Q-OP1·Q-OP2 반영 완료(Must 승격 3건 + Holding Period 24h + 2인 게이팅). Q-OP3·Q-OP4는 개발 완료 전까지 유보 고정. Must 19 / Should 17 / Nice 6 / Deferred 7.**
Prior: v1.0 (13차) — P5 검증 완료 (critic REJECT → 해소 / ux-researcher / pre-mortem 3병렬). Critical 3건(I-01/I-02/I-03) 해소 · Major 10건 전원 해소(I-04~I-13) · 사용자 결정 D10~D13 박제. 다음: P8 진입 준비(P6 폐지, P7 완료).
...
- **담는 것**: 어드민(3명 가정, §1.1)이 메인 서비스에서 만나는 화면·기능·데이터·제약.
...
- **체결 가정 (D3)**: 승인 시점 **종가로 100% 일괄 매수** 가정. 슬리피지·수수료 0% (단순 가정, Stage 1 한정)
...
- 증권사 API 연동 자동 체결 (Stage 2 로드맵에서 추가 예정)
- AI 자율 운용 (Stage 3 로드맵, 별도 트랙 기획 후 어드민 메뉴 통합)
- 멤버에게 Short List·풀 리포트 노출
- 공개 마케팅·무료 체험·가입 퍼널
- 실제 체결가·증권사 잔고 동기화 (Stage 1 범위)
- "매수하세요/매도하세요" 형태의 멤버 대상 추천 (어드민 내부 도구에서만 허용)

> **트레이딩 실행 3-Stage 로드맵**: Stage 1(현재 MVP)=매뉴얼 → Stage 2=API 연동(승인 후 자동 체결) → Stage 3=자율 운용(AI 전자동). Stage 1~2는 어드민 서비스 내, Stage 3는 별도 기획 후 어드민 메뉴로 통합. 세 Stage 모두 Quant 분석 엔진 공유.
...
| **(3) 자동매매 서브시스템** | 주식·코인 자동매매. 미확정. | ✅ (서브시스템) | `AutoTrading.md` 별도 |
...
| # | 의제 | 결정 | 박제 위치 |
|---|------|------|----------|
| **D10** | Exit 시그널 백업 채널 | Critic I-01 해소 — 텔레그램·이메일 중 하나 실패 시 다른 채널 catch-up, **둘 다 실패 시 SMS 1회 재시도**를 Must 범위에 포함. Anti-Metric "Exit 시그널 미수신 1건+" 방어. | §3.10 R3.10-15 |
...
- **모바일 대응**: MVP Stage 1은 **데스크톱 전용**. 모바일 반응형은 Stage 2 로드맵(사이드바 → 바텀 탭바, 리포트 Sticky Tab 전환). 단, 텔레그램·이메일 딥링크는 Stage 1부터 동작 — **P7.3 권장(⑧)**: `/admin/alerts/[id]`와 `/admin/report/[ticker]` Section 0만 CSS 단일 컬럼 스택(`<768px`)으로 최소 readable 제공. 풀 모바일 UI는 Stage 2.
...
Stage 1 MVP에서 어드민 UI에 표시되는 분석엔진 산출물. P3.0 결정 D5 박제.
...
- R3.1-5: 모든 분석엔진 출력은 EOD 배치 기준으로 갱신된다(상시 모니터링 모드에서는 장중 스트림 추가, §3.5 참조). Stage 1 한정 — Stage 2·3에서 추가 지표 확장.
...
- **R3.3-8 (D15, v1.1): 2인 풀 리포트 열람 게이팅**. 어드민 3인 중 **최소 2인이 풀 리포트를 Section 0 이상 열람**(`report.view` 이벤트 발생, E2 기반) 한 경우에만 Accept 버튼 활성화. 열람 카운터는 `/admin/portfolio` 화면에 "현재 2/2명 열람 완료" 형태로 노출한다. 게이트가 false이면 버튼 disabled + 미열람 어드민 안내 표시.
...
**핵심 요구사항**:
- R3.4-1: 승인 확정 시점의 당일 종가를 모든 종목의 매수가(승인가)로 기록한다. 슬리피지·수수료 0%, 100% 일괄 매수 가정. Stage 1 한정.
...
- R3.10-15: **Exit 시그널 백업 채널 (D10, P5 I-01 해소)**: 텔레그램·이메일 중 **하나 실패 시 다른 채널이 catch-up**(재시도 포함)한다. **둘 다 실패 시 SMS 1회 재시도**를 수행한다(SMS 벤더 연동은 BuildPhase B3에서 확정). 3채널 모두 실패 시에도 `/admin` 대시보드 상단 배지는 항상 작동하여 최소 1채널을 보장한다. Anti-Metric "Exit 시그널 미수신 1건+" 방어를 Must 범위에 포함.
...
- **AC**:
  - [ ] Exit 트리거 감지 시 텔레그램·이메일·`/admin/alerts` 세 채널에 동시 발송되고 `exit.signal.sent` 이벤트가 기록된다.
...
- [ ] Exit 트리거 감지 시 텔레그램·이메일·`/admin/alerts` 세 채널에 동시 발송되고 `exit.signal.sent` 이벤트(ticker·severity·timestamp·trigger_reason)가 기록된다.
- [ ] **백업 채널 (D10)**: 텔레그램·이메일 중 하나 실패 시 다른 채널 catch-up, 둘 다 실패 시 SMS 1회 재시도가 작동한다. 대시보드 상단 배지는 항상 작동.
...
| report_view_count | int | 2인 열람 게이팅(R3.3-8) 캐시 — `report.view` 집계. Accept 버튼 활성 조건 (≥ 2) |

**갱신 주기**: 어드민 승인 액션 시. **관계**: PortfolioSnapshot(1:N, 해당 month).
...
|---|---|---|
| id | uuid | PK |
| alert_type | enum | exit_signal / news_critical / price_anomaly / briefing / scheduler_fail |
...
| account_no | varchar | 계좌번호 (마스킹 저장, 예: `12-***-5678`) |
| api_key_ref | text | Vault/Secrets 참조 키 (**평문 금지**) |
...
- **E9 BrokerageConnection 시크릿 정책 (D12 P5)**: `api_key_ref`는 평문 금지 — Supabase Vault 또는 외부 Secrets Manager 참조 키만 저장. 본인 키는 **본인 admin_id만 접근** 가능하도록 RLS로 격리(다른 어드민도 조회 불가). 키 회수(로테이션) 로그는 `last_used_at` + 별도 감사 테이블(BuildPhase B2.3 확정)에 기록.
