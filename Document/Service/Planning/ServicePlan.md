...
- **AI 모델/프로바이더 추상화** (65차 Q3 + D28, 2026-06-04): AI 모델 ID 하드코딩 금지 → 설정값(모델 레지스트리)화. **역할별 모델 차등 기준은 비용이 아니라 예측 적중률+리포트 정확성**(hardcap 50만=제약)이며, 초기 기본값은 토론 참가 Sonnet 4.6×6 + GPT mid×5 / judge·writer·revise·W3 = Opus 4.8 중심 / critic = GPT mid 교차(가설, track-record로 조정). **Claude + GPT 멀티프로바이더** — Claude 필수 primary, GPT 선택 secondary, GPT-only 미지원, provider availability auto-detect(GPT 키 없으면 Claude-only fallback). 구 'AI 키 = Anthropic 유일 공통 키' 전제 폐기. 레지스트리·provider 어댑터 구현은 W0~W3 위임. SoT → HANDOFF.md ⭐ 65차 MVP 엔진 섹션 / ServicePlan-Admin.
...
| **2026-06-04** | **65차 7결정(D26) + D27 Q5 + D28 supersede 포인터 반영** — §2 매뉴얼 트레이딩 행에 선정주기 단기 주1회/중장기 월1회 분리(Q1), AI 투심위 행에 실시간 멀티라운드 토론 loop supersede 포인터(Q4), Quant 분석 엔진 행에 AI 자율 포트구성 supersede 포인터(Q2), §3 공통원칙에 'AI 모델/프로바이더 추상화' 항목 신설(Q3 — 역할별 모델 차등·멀티프로바이더 auto-detect). D27 Q5 incumbent thesis 재점검(기존 종목 논거+성과 컨텍스트, fresh Tier0 ∪ incumbents) + D28 역할→모델 기본 배분(품질 목적함수, Claude primary/GPT secondary, Sonnet×6+GPT mid×5 가설 기본값)은 ServicePlan-Admin / HANDOFF SoT 참조. hardcap 40만→50만·MVP 재정의·빌드순서 W0→W2→W1→W3·역할별 모델 차등 비용은 본 인덱스 문서에 직접 텍스트 없음 → ServicePlan-Admin / HANDOFF SoT 참조. 깊은 spec/schema 재작성은 W0~W3 구현 위임. | 65차 사용자 확정 7결정(D26)+D27 Q5+D28 (omxy CONVERGED) |
