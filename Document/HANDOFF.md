...
- **보고서 프레임워크 v2.0 고도화**:
  - 8-Section → Section 0~8 + Appendix 구조 변경 (투자 서사/회사 개요 신설, 촉매+리스크 통합)
  - 투심위 토론 구조 변경: "각자 독백" → "쟁점별 찬반 대결" (핵심 쟁점 도출 + 찬성 대표 vs 반대 대표 vs 중재자)
  - 위원 선정 원칙 확립: 독립성(동일 논거 2명 금지), 철학 반영 강제, 고유 통찰 의무
  - Core Committee 투자 철학 매핑 (10명 × 투자 철학 슬롯 재설계)
  - Sector Board 14개 섹터별 슬롯 구성 가이드 (경쟁사 CEO 배제 원칙)
  - 용어 가이드라인 (영어 약자 한글 풀이 필수 + 주요 용어 해설 목록)
  - 독자 수준 가이드라인 ("섹터 비전문가" 대상, 비유 필수, 섹션별 가이드 문구)
  - 비교 분석 가이드라인 (크로스섹터 비교 금지, 섹터별 비교 축 가이드)
  - `Document/ReportFramework.md` 신규 작성 (정의서)
  - `Document/BusinessPlan.md` §8 업데이트 (§8.2~§8.5 재구성, §14 의사결정 기록 추가)
