...
- **검증/리뷰**: build/lint 0·0 / test:ci **2345 PASS+5 skip** / tsc 0 / PG smoke 0043. Claude dynamic-workflow **5-lens 적대 리뷰(4 HIGH catch+fix: getCyclePanels round double-count·LLM cost fail-open·CRON_SYSTEM_USER_ID guard·live-wiring 미테스트)** → omxy §2.0a R1 적대 검토(**7 confirmed defect 직접 수정**: LLM fail-closed/atomic claim/예측 출력 필터/now seam/period_key 정규식) → Claude 2차 독립 검증(게이트 green·테스트 substantive·H1 보존) **CONVERGED**. **production 미접촉(flag off·0043 미적용 안전)**. USER 활성 게이트 = `REFLECTION_ENABLED`+0043 apply+`SELECTION_CRON_AUTO_ENABLED`+`KRX_OPENAPI_KEY`+reflection-job schedule(+선택 LLM 요약 키/비용). SoT = `docs/superpowers/specs/2026-06-27-reflection-prk-build.md`.

**2026-06-27** (S7b M12a 뉴스 기반 자동 제외(AI 페르소나) ✅ shadow-first 빌드·dormant · branch `tier0-bpp-multiregime`):
- **신규 `tudal/src/lib/news/m12a/`**: `types.ts`(공유 타입 — PerTickerAssessment/BrakeConfig/M12aTickerLedgerRow/CashoutRecord 등) · `config.ts`(DEFAULT_BRAKE_CONFIG: maxAutoRemovalsPerRun=3·listFloors short7/midlong14/full21·concentratedPortfolioMax10) · `flags.ts`(`isM12aNewsEvalEnabled`/`isM12aAutoRemoveEnabled`, 둘 다 env true 필요·유일 env 경계) · `verdict.ts`(`decideRecommendedAction` 결정론 auto_remove 게이트, scope 비-게이트) · `brake.ts`(`applySmartBrake` per-company unique mass>3·track floor·portfolio floor → whole-run hold) · `cashout.ts`(`buildCashoutRecord` GAP2 fail-closed, price>0 + priceBasisDate YYYYMMDD) · `ledger.ts`(`buildM12aLedgerRow` GAP1 action_taken 귀속, price 3필드 removed-only) · `negative-news-context.ts`(`buildNegativeNewsContext` 재진입 컨텍스트, forward-validate·예측어휘 0·빈입력 "") · `telegram-text.ts`(`buildM12aTelegramText` telegram-only·markdown/이메일 0) · `evaluator.ts`(`makeM12aNewsEvaluator` Core 11 패널 + `parseNewsPersonaJudgment`(zod) + `aggregateNewsAssessment` 전체 panelSize 과반 consensus) · `orchestrator.ts`(`runM12aNewsEvaluation` DI 코디네이터: gate→preflight→eval→verdict→brake→mutate(auto만)→ledger→alert) · `auto-remove.ts`(`resolveCachedCashout`/`applyAutoRemoveMutation` — mutation-before-ledger·ticker dedup·missing-applyAutoRemove throw) · `run-result.ts`(`emptyM12aRunResult`) · `briefing-integration.ts`(`runM12aForBriefing` — flag gate + go-live fail-closed step-0[AI_COST_LOG+CRON_SYSTEM_USER_ID UUID/getUserById] + 유니버스/뉴스 fetch + orchestrator 배선 + resolveCashout/applyAutoRemove throw-stub). + `tudal/src/lib/data/admin-m12a.ts`(`insertM12aAssessments` service-role DI + 입력검증). ~10 test files / ~100 unit tests.
- **마이그 0042 `m12a_ticker_assessment` (DORMANT, USER apply-only)** + `.rollback.sql` + `scripts/pg_smoke_0042.sh`(docker-free PG16 10 assertions: CHECK[ticker/surface/scope/level/action/month/price] + GAP2 price-consistency + FK news_event(cascade)/alert_event(set null) + RLS + rollback). `news_event` 1 + `m12a_ticker_assessment` N (per-ticker thesis-break ledger). `alert_event.alert_type` 12-enum **불변**(news_critical/news_warning 재사용).
- **배선 (전부 additive·dormant·off→byte-identical)**: ① morning-briefing cron(`runM12aForBriefing` flag off→{ran:false,[]}·attentionTickers seam, try/catch 격리로 M12a 실패가 M11 미차단) ② 재진입 negative-news 컨텍스트(`render-user-prompt.ts`/`anthropic-client.ts`/`persona-panel-adapter.ts`/`selection-worker` — macro 패턴, "" → byte-identical·범주 분리). 기존 `news/classifier.ts` 규칙기반은 collection/dedup만 재사용(supersede 아님).
- **shadow-first**: `M12A_AUTO_REMOVE_ENABLED` off→short_list_30/portfolio_snapshot mutation 0; `M12A_NEWS_EVAL_ENABLED` off→평가/비용/알림 0. 자동 mutation=출시 후 fast-follow(throw-stub).
- **검증/리뷰**: build/lint 0·0 / test:ci **2246 PASS+5 skip** / tsc 0 / PG smoke 10 assertions. Claude dynamic-workflow 빌드 → **omxy R1~R3 CONVERGED(5 fix: consensus panelSize·per-company mass·date-format guards·mutation-before-ledger·모듈 추출)** + **Claude 5-dim deep-review(20 agents): HIGH 2(go-live fail-closed)+MEDIUM 1(M11 격리) fix, LOW 11 verified-false**. **production 미접촉(flag off)**. USER 활성 게이트 = `M12A_NEWS_EVAL_ENABLED`+`AI_COST_LOG_REAL_INSERT_ENABLED`+`CRON_SYSTEM_USER_ID`+0042 apply+Naver/Telegram/AI 키. SoT = `docs/superpowers/specs/2026-06-26-m12a-news-auto-remove-shadow-first.md`.

**2026-06-26** (G4 거시/뉴스 → AI 컨텍스트 레이어 ✅ 완료·dormant · D33 §4 · branch `tier0-bpp-multiregime`):
- **신규 `tudal/src/lib/macro/`**: `context.ts`(pure distill — `buildMacroContext`(verdict+indicators→regime/score/headline/drivers/asOf) + `renderMacroContextString`(forward-validate 면책 '예측 아님·Tier0 스크리닝 팩터 아님·asOf' 포함) + `EMPTY_MACRO_CONTEXT`; env/now/IO 0) + `source.ts`(env+now 경계 — `getMacroContextString({now?,maxStaleDays?,source?})` = flag `MACRO_CONTEXT_ENABLED` off→"" / asOf>7일 stale→"" / else render; `getMacroContextSource` = mock-macro seam[실 FRED drop-in 지점]). 18+ unit tests.
- **3-consumer 주입 (전부 additive·dormant·off→byte-identical)**: ① 리포트 writer `report-input-enricher.ts`(`buildMacroSummary` DI seam → `macroSummary` 대체, off→근거 부족) ② 모닝 브리핑 `briefing/compose.ts`(`macroContext` 1줄 + `formatMacroLine` 정규화) + `cron/morning-briefing/route.ts`(`getMacroContextString()` 주입 + **Resend 제거** = Telegram best-effort+dashboard) ③ **LIVE Tier1 선정** `cron/monthly-batch/selection-worker/route.ts`(getMacroContextString 1회) → `screening/persona-panel-adapter.ts`(`CallPersonaPanelDeps.macroContextString` R1+R2 패널) → `ai/anthropic-client.ts callPersona` → `ai/prompts/render-user-prompt.ts`(비어있지 않으면 끝에 append, 빈값=byte-identical). `screening/persona-eval.ts`(runMonthlyPersonaEval — dangling, 무해 dormant)도 동반.
- **가드레일 코드 박제**: `lib/macro/__tests__/guardrail.test.ts`(@/lib/macro importer allowlist scan — Tier0/funnel deny; static+side-effect+dynamic import 캐치) + Tier0 정량 funnel 무누수(Python screen_shortlist_tier0 구조적 분리). M12a 범주 분리·forward-validate 테스트.
- **검증/리뷰**: build/lint 0·0 / test:ci **2141 PASS+5 skip** / tsc 0. Claude TDD draft → omxy R1 CONVERGED(latestAsOf NaN-guard+브리핑 1줄 정규화) → **Claude deep-review(general-purpose, architecture lane)가 HIGH catch**(macro가 dangling runMonthlyPersonaEval에만 배선·LIVE selection 경로 미도달) → live-path fix(persona-panel-adapter+selection-worker) + route 양성 테스트 + guardrail 테스트 → omxy R2 CONVERGED. USER 활성 게이트 = `MACRO_CONTEXT_ENABLED`+실 거시 source. **production 미접촉(flag off)**. SoT = `docs/superpowers/specs/2026-06-26-g4-macro-news-ai-context-layer-design.md`.
...
- **섹터 lens provenance (FE-only, ₩0)**: Section 8 Part A "섹터 14인 패널" 헤더에 `SECTOR_LENS_SUMMARY`(canonical-sectors.ts, 14 sector→핵심판단 ≤30자 압축, 표준약어 허용) 기반 `AI 섹터 관점 · {sector}: {lens}` + 면책 1줄(`isCanonicalSector` 가드, off-canonical→title만). Kevin 섹터 페르소나가 이미 분석에 적용 중이나 라벨이 일반 역할명이라 provenance 숨던 것 보완(저장 데이터·report 재생성 무변경). 3파일(page.tsx + canonical-sectors.ts + test) + 5 unit test. Claude↔omxy 토론 CONVERGED(설계) → dynamic workflow 드래프트 → omxy 직접수정(8 lens 충실도 보정·면책 정합) → Claude review-purpose subagent 적대검토(13/14 grounded·invented 0·mutation-verified) → omxy 재검토 CONVERGED (feat a2a1269). test:ci 2102+5skip.
...
**2026-06-09** (72차 — ⭐ 뉴스 기반 자동 제외 (M12a) **spec 신설 — 문서 only · 코드 미구현(planned)**, omxy scope debate R1~R4 CONVERGED) *(historical — ✅ 2026-06-27 shadow-first 빌드로 구현됨, 상단 2026-06-27 entry가 forward-supersede; per_ticker_assessments→구현명 `m12a_ticker_assessment`)*:
