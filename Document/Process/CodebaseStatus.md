...
- **2026-06-29 (news-sweep 유니버스 정합 · main cfb1bde)**: `src/app/api/cron/news-sweep/route.ts` 워치리스트 = 실 active short_list_30(getActiveShortList) — 구 3종 하드코딩(005930/000660/035420) 제거. M12a/모닝 브리핑 수집 범위 정합(키 없는 production은 기존처럼 500 early·동작 변화 0). Claude TDD+5렌즈 → omxy 리뷰+수정 CONVERGED → Claude Phase3 적대 → omxy reconverge. test:ci 2525 pass/5 skip. **잔여 follow-up(비차단·dormant)**: M12a `getRecentNewsEvents({limit:50})` 전역 윈도 → 30종 per-ticker 커버리지 universe-aware 개선.
