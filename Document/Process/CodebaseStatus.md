...
- **⭐ 스코어링 방법론 토론 CONVERGED (다음 세션 실행 대기, 코드 미구현)**: 사용자 핵심 질문 = "후보 150 간추릴 때 정말 향후 상승할 기업을 올바르게 예측·선별하는 스코어링이냐". quant-research 스킬 + 독립 Claude 퀀트 에이전트 + omxy 2 sub-agent(MSCI/Alphalens 웹리서치) **독립 수렴** = 현 Tier0 = 합리적 팩터 컨셉 + 실행 결함 다수(결측 0주입 분포오염[HIGH·신규]·z fat-tail·외국인 WON 절대합 size bias·가중치 IC 미검증·시그널 상관 무시) + **예측력 검증 0건**. 결론 = **"B+ 형태수정 + 경량 IC 검증"이 최소 허용 경로**(B-only=무근거 교체 BLOCKER / 풀 재설계=과함). **SoT spec = `docs/superpowers/specs/2026-06-12-tier0-scoring-bplus-validation.md`** (B1 winsorize→percentile rank+결측 tiering / B2 외국인 flow/ADV 정규화 / B3 quality sector-relative rank / B4 ensemble 비교 + 경량 rank-IC/분위 스프레드/regime/거래비용 게이트 + 문구 게이트 "robust factor-informed candidate shortlist"까지만 허용). **현 산출물 설명에 "향후 상승 예측" claim 금지 — IC 검증 통과 후에만.**
