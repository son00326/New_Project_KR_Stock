...
> ⚠️ **다음 세션 1순위 = USER go-live 게이트** (AI 키 `ANTHROPIC_API_KEY`[+`OPENAI_API_KEY`] / flags / 마이그 0031·0032·0033·0034·0035 apply → 실 AI 첫 30선정·포트 제안·cash 행). **키-free CLAUDE 빌드 큐는 소진** — R31 합의 키-free 실테스트 3종(A✅ PR #101 · S7d[코드 완결, 실검증만 Docker/USER] · W3b-2c✅ PR #102) 처리 완료. 다음 CLAUDE 작업은 USER 외부상태(키/flag/마이그 apply/Docker 실env) 선행 필요. 키-free 추가 후보 = S7b(뉴스/브리핑, Naver/Resend 키 필요 → 부분만 키-free). 완료 경로 = W0 ✅ → … → W3b-3 ✅ → A ✅ → **W3b-2c ✅ MERGED(PR #102, main `9698d74`)**. **W3·W3b·MVP 엔진 코드 + 키-free 하드닝 완결**(UI→생성→표시→영속→Accept weight→명시 cash 행). 현 게이트 수치 = test:ci 1908+2skip(W3b-2c 후).
...
**main HEAD**: `git rev-parse --short origin/main` (2026-06-05 68차 W1b PR #94+#95 후 = `30107cc` 또는 자손).
**OPEN PR**: **#2**(format-error, CONFLICTING 보류) only.
**검증 게이트**: build 26 routes / lint 0 err 0 warn / **test:ci 1792 PASS + 2 skipped(smoke 의도) / 150+1 files** / tsc clean / Python 95.
**Production**: 마이그 0001~**0031** applied (0028 tier0_candidates_150 + 0029 short_list_30 AI 컬럼 + **0030 get_cost_log_monthly_total_admin cost RPC, 2026-06-02 applied+verify** + **0031 W2a replace_shortlist_track/carry_short_into_month selection RPC, 67차 applied+verify**) · short_list_30 30 rows canonical 14(B93 PASS, AI 컬럼 전부 null = Tier 0 fallback, **AI 선정 아님**) · tier0_candidates_150 **150 rows**(2026-06 · 50/50/50 · canonical 14 · unresolved/placeholder 0 · 63차 `sector_override` +21 §2.0a 적용) · **실 AI 호출 0건**(cost_log=0).
...
#   main 기준 기대 (2026-06-05 68차 PR #95 후, main `30107cc`): build 26 routes / lint 0 err 0 warn / test:ci 1792 PASS + 2 skipped(w0 smoke 의도) / 150+1 files / tsc clean / Python 95 tests PASS
...
| 실 AI 호출 | **현재 0건** (cost_log=0). **[→ 65차]** 선정 path 첫 실 AI = ~~PR-G~~ **W0 cheap-model provider+cost_log smoke → W2a/W2b/W1 엔진**(상단 ⭐). report-only 첫 smoke = Task 7(PR5 트랙, 별도). 둘 다 USER 비용 승인 후. |
| Supabase | project `rbrpcynhphrpljbjirfo` · **0001~0034 현황**: 0001~0031 production applied (0028/0029/0030 + **0031 = W2a period_key/track job·run + 7 RPC — 2026-06-04 67차 applied**: omxy apply-readiness CONVERGED → list_migrations 미적용 확인 → MCP apply → 사후검증 테이블2/RPC7 SECURITY DEFINER/anon grant 0/short_list_30 30·cost_log 0 무손상) · **0032·0033·0034 dormant(USER apply)** (0034 = W3b-2a portfolio_proposal 영속, production 미적용) · SECURITY DEFINER 패턴 · dart_corp_codes 2,766/2,766 induty 백필. |
...
| 우선 | 작업 | 필요 액션 |
|---|---|---|
| 🟡 W0 live smoke 실행 (66차 신규 · **선택** — W2 착수 블로커 아님) | provider 계층 실 키 검증 1회 (haiku+gpt-5.4-mini 각 1콜 ≈ 20원 미만). `OPENAI_API_KEY`를 `.env.local`/Vercel에 저장(미설정 시 openai 케이스 자동 skip) 후: `cd tudal && set -a && source .env.local && set +a && W0_LIVE_SMOKE=true npx vitest run src/lib/ai/__tests__/w0-provider-smoke.live.test.ts` | ⭐ 66차 W0 박제 entry 참조 |
| ⭐ 마이그 0032 apply (68차 W1a — R2 반박 라운드 가동 전 필수) | `0032_tier1_selection_round`(round 컬럼+unique 교체) production apply. **미적용 상태는 fail-closed 안전**(`selection_round_schema_missing` — R1-only 동작 아님, 명시 중단). apply 후 R2 라운드 활성 | ⭐ 68차 W1a 박제 entry 참조 |
| ⭐ 마이그 0033 apply (68차 W1b — judge 라운드 가동 전 필수, 0032→0033 순서) | `0033_tier1_selection_judge_round`(round 1,2,3). **미적용=fail-closed**(`selection_round_schema_missing` — judge enqueue 차단). 0032 선행 apply 필수 | ⭐ 68차 W1b 박제 entry 참조 |
...
| ⭐ portfolio_proposal AI 제안 활성 (69차 W3b-1, 실 AI·비용) | `PORTFOLIO_AI_PROPOSAL_ENABLED=true` + `ANTHROPIC_API_KEY` + `AI_COST_LOG_REAL_INSERT_ENABLED=true`(isCostLoggingEnabled 게이트) 모두 Vercel env 충족 시에만 proposePortfolio가 실 Opus 호출(월 hardcap 50만 preflight 적용). 미설정=proposal_disabled(cost 0, behavior-neutral). **W3b-1은 제안 반환만**(DB 영속·Accept 무변경) — UI 연결은 W3b-3 | ⭐ 69차 W3b-1 박제 entry 참조 |
| ⭐ portfolio_proposal 영속 활성 (69차 W3b-2a) | **마이그 0034 production apply**(USER) + `PORTFOLIO_PROPOSAL_PERSIST_ENABLED=true`(Vercel env) 둘 다 충족 시에만 proposePortfolio가 제안을 portfolio_proposal에 영속. 미설정=영속 0(W3b-1 동작 1:1, behavior-neutral). 0034 미적용+flag on=`proposal_schema_missing` fail-closed(AI 호출 0). Accept 통합은 W3b-2b | ⭐ 69차 W3b-2a 박제 entry 참조 |
| ⭐ Accept proposal weight 활성 (69차 W3b-2b) | `PORTFOLIO_USE_PROPOSAL_ENABLED=true`(Vercel env) 충족 시 Accept가 영속 proposal weight로 snapshot 구성. 미설정=현 균등 suggestedWeight 1:1(behavior-neutral). 실 동작엔 W3a(`PORTFOLIO_REAL_ENTRY_PRICE_ENABLED`+`KRX_OPENAPI_KEY`) + 0034 apply + persist flag + 실 proposal 영속 필요. 0034 미적용/proposal 미존재=suggestedWeight fallback(무브릭) | ⭐ 69차 W3b-2b 박제 entry 참조 |
| ⭐ PR-G ⓑ (실 AI 첫 30선정, ~~다음 1순위~~ → **65차 W0로 superseded**) | ⓐ ✅ MERGED. **63차 결정: ❶ B 청크 워커**(로컬 러너 폐기) **/ ❷ 하이브리드 데이터**(KRX 공식 API[시세·거래량·universe] + pykrx[S3 외국인] + DART[S4·S5]). **KRX 전환(PR #80)·150 시드·선정 청크 워커(PR #82, 구 0031 dormant; 67차 W2a 0031 재작성으로 superseded) 빌드 ✅ 완료.** 잔여 = **`SELECTION_CRON_AUTO_ENABLED=true` + AI 키/비용 승인 + 주간 tier0 producer 자동화 + 마이그 0032→0033 apply** → 워커 수동 트리거. **(PR #84 launch-readiness 감사: AI 선정 persist가 이제 name/sector/composite/signal carry — 첫 실 선정 시 빈 카드 잠복결함 제거)** | 한눈에 §다음 액션 큐 1번 참조 |
