...
| **풀 리포트 (production)** | `stock_reports` 2026-06 **30행 = B++ 새 30 (Tier2 포함, 2026-06-24 재생성·검증)** — section_0~8+appendix + committee_votes 330 core + 420 sector = 750 + partA 14 + partC.sector_aggregate + NULL-sector 0. **PR-T2a(Tier2 배선) ✅ CONVERGED + 마이그 0040/0041 applied**. **PR-T2c FE 렌더 ✅ 검증**(Part A 14 + sector_aggregate 렌더 OK, 30/30 modern-parse, fallback 0). **B-PARTB ✅ 코드 완료**: Part B 쟁점 실제 추출(writer.ts `extractIssueDebates`, CONVERGED 2026-06-25, branch `tier0-bpp-multiregime` 미머지). **기존 30 리포트 DB는 재생성 전까지 옛 stub 잔존 — 재생성(~₩30k 실 AI)=USER 비용 게이트(미실행)**. 완결성 체크 갭(pre-launch). report_batch_job 30 done. 2026-05-01 004150 1행(P1 잔존). |
