...
| 검증 게이트 | build OK / lint 0 err 0 warn / **test:ci 2141 PASS + 5 skipped**(G4 macro context/source/guardrail + adapter live-path + route wiring + render append +) / tsc clean / DART pytest 18 + screen/shadow python unittest 189. |
...
| **Step 7 S7b** 뉴스 자동 제외(M12a) + 모닝 브리핑(M11) + **G4 거시/뉴스 AI 컨텍스트** | USER(Naver B-8 + Telegram B-9 + AI 키 + `MACRO_CONTEXT_ENABLED`+실 거시 source) + CLAUDE | B++ G1/G2 + go-live USER gates 완료 후 | **G4 ✅ 완료(2026-06-26, dormant·D33 §4)**: `lib/macro` distill → 리포트 writer/모닝 브리핑/**LIVE Tier1 선정 패널(R1/R2)** 컨텍스트 입력(`MACRO_CONTEXT_ENABLED` off→byte-identical). Tier0 factor 금지(allowlist 테스트 박제)·M12a 범주 분리·forward-validate·stale fail-safe. **M11 모닝 브리핑 Resend 제거**(Telegram+dashboard). omxy R1+R2 CONVERGED + Claude deep-review(live-path HIGH catch)·게이트 green. SoT `docs/superpowers/specs/2026-06-26-g4-macro-news-ai-context-layer-design.md`. **잔여 = M12a AI-persona 뉴스 자동제외**(Core 11 뉴스 평가 → per-company thesis-break → direct/material/high-conf 자동 제외·smart brake·durable ledger·텔레그램/`/admin/alerts`). **개발 순서 = shadow-first**(`M12A_AUTO_REMOVE_ENABLED` default false → D11 관찰 → 자동 제거 ON은 출시 후 fast-follow). **M12a 자동 mutation = 출시 게이트 아님.** spec SoT = `ServicePlan-Admin §3.10 M12a`. 이메일/Resend 전역 미사용. **디자인 결합**: D3 신규 화면 final-style 동시구현, D1은 S7b UI 착수 전 필수. |
