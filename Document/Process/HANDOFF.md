...
- **다음 1순위**:
  ...
  3. **(CLAUDE 병렬 가능)** **Mock cleanup Step 2.2** (settings: MOCK_ADMIN_SETTINGS + MOCK_ADMIN_TICKER_PREFS + MOCK_ADMIN_ID) — admin_settings / admin_ticker_prefs 실 SELECT + WRITE boundary 결정 + 마이그 신설 필요시.
...
> ⚠️ **5줄 요약 본문은 57차 §3 종료 시점 historical 박제**. 58차 Mock cleanup Step 2.1 OPEN PR #33 진입 박제는 §1 첫 단락 + §2.1 본문 + §6 신규 entry 참조. 본 5줄 요약 본문은 다음 차수 sweep에서 재작성.
...
| **다음 세션 1순위** | ⭐ (USER) PR #33 머지 (Mock cleanup Step 2.1 alerts, Vercel preview canary verify 후) + Vercel env `PR4_TRIGGER_UPSERT_ENABLED=true` 추가 → Task 7 Smoke Stage 2 진입 가능. **(CLAUDE 병렬 가능) Mock cleanup Step 2.2** (settings: `MOCK_ADMIN_SETTINGS` + `MOCK_ADMIN_TICKER_PREFS` + `MOCK_ADMIN_ID` → `admin_settings` / `admin_ticker_prefs` 실 I/O, WRITE 경로 결정, 마이그 신설 필요 시) — Step 2.3~2.7 (regenerate/cost/health/news mock 5 종) 순차. **(CLAUDE 병렬 가능) Task 5 B66 backfill** (Supabase MCP access 확보됨). → Task 7 Stage 2 (USER 승인 + `AI_COST_LOG_REAL_INSERT_ENABLED='true'` env 선행) → Task 8 audit + PR5. |
