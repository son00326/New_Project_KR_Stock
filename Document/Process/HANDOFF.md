...
Last updated: 2026-06-24 (**⭐ B++ funnel 적용 완료(G1/G2) + Tier2 리포트 배선(PR-T2a) CONVERGED. production `tier0_candidates_150`/`short_list_30` 2026-06 = B++ cfg1 기준(150 리더 4/11·30 리더 2/11, 73차 1/11 대비 개선). B++ = diagnostic funnel 업그레이드지 예측 게이트 통과 아님(NO-CONFIG-PASSES 유지). 다음 1순위 = ⓐ 새 30 리포트 Tier2 포함 1회 생성(USER 마이그 0040 apply + flags + ~₩15-20k 게이트) ⓑ go-live USER 게이트 + S7b. PR-T2c 렌더·섹터 비교 메뉴·PR-K Reflection은 S9 전 pre-launch lane.**)
...
4. ★ **다음 1순위(ⓐ) — 새 30 리포트 1회 생성 (Tier2 섹터 페르소나 포함).** PR-T2a(Tier2 섹터 보드 → live 리포트 배선) 코드 ✅ CONVERGED(b3d9a88/e4066a7, omxy R1+R2 + Claude 적대 검토, dormant). 73차 옛 30 리포트는 새 30 대비 stale → USER가 **마이그 0040 apply(`commit_sector_personas_cron`) + `SECTOR_BOARD_ENABLED`/`PR5B_SECTION8_ENABLED`/`AI_COST_LOG_REAL_INSERT_ENABLED` + 리포트 비용 승인(~₩15-20k)** 후 CLAUDE가 P4-style 생성(Tier2 포함) 실행. USER 결정 = "Tier2로 한 번에"(Core-11만 먼저 굽고 Tier2 나중 = 이중작업 회피). [SoT: `docs/superpowers/specs/2026-06-23-tier2-sector-persona-report-wiring.md`]
5. **go-live USER 게이트 + S7b 뉴스·브리핑.**
...
9. **Pre-launch 섹터 비교 메뉴 + Tier2 리포트 배선 잔여(PR-T2c 렌더) + 디자인 freeze(S9/출시 전).**
   ...
   - **Tier2 섹터 페르소나 → 30 리포트 배선**: **PR-T2a(생성 seam) ✅ DONE(위 #4) — live 경로 배선 완료(dormant).** 잔여 = **PR-T2c(Section 8 섹터 보드 FE 렌더)** — 리포트 페이지에서 sector board Part A + 실 sector_aggregate 표시(현재 0/빈 fallback). PR-T2b(비용 reservation)는 PR-T2a에 흡수. 상세 = `🔧 Pre-launch` 섹션.
...
- **Tier2 섹터 페르소나 → 30 리포트 배선(PR-T2a/b/c)**: Core-11 리포트만으로 MVP ③은 완료됐지만, 출시 전 리포트 품질 상향 deliverable이다. **PR-T2a ✅ 완료/CONVERGED(2026-06-24)**: live `full-report-batch-worker → orchestrateFullReport` 경로에 `SECTOR_BOARD_ENABLED` seam + cron service-role commit path가 배선됐고, 마이그 0040 `commit_sector_personas_cron`은 **written-not-applied(USER apply 게이트)**, 기본은 dormant(플래그 OFF)라 production 무변경. PR-T2b 비용 reservation/모델 preflight는 PR-T2a에 흡수. 잔여 = **PR-T2c Section 8/render/`committee_votes` 섹터 보드 렌더 확인** + 선택 후속 **D-1 dangling action cleanup**. 새 30 리포트에 Tier2를 포함하려면 USER 마이그 0040 apply + flags + 비용 승인(~₩15-20k)이 필요. SoT = `docs/superpowers/specs/2026-06-23-tier2-sector-persona-report-wiring.md`.
...
# 2026-06-24: B++ funnel 적용 완료(G1/G2) + PR-T2a Tier2 리포트 배선 CONVERGED. production 150/30 = B++ cfg1.
#    다음 launch next = ⓐ 새 30 리포트 Tier2 생성(USER 마이그 0040 apply + SECTOR_BOARD_ENABLED/PR5B_SECTION8_ENABLED/AI_COST_LOG_REAL_INSERT_ENABLED + ~₩15-20k) → ⓑ go-live USER gates + S7b.
#    B++ = retrieval 개선 diagnostic funnel(NO-CONFIG-PASSES 유지·예측 claim 금지). 마이그 0040 = written-not-applied(USER gate).
...
select count(*), round(coalesce(sum(cost_krw),0)::numeric,2) from cost_log where month='2026-06';
  -- 기대 ≈₩68,034 (직전 ₩41,369 + G2 B++ 재선정 ~₩27k[2026-06-24]). 추가 증가 = 새 30 리포트 생성(USER 게이트 후).
...
```
> **2026-06-24: `short_list_30`/`tier0_candidates_150` 2026-06 = B++ cfg1 기준으로 교체됨(G1/G2 적용).** B++/B+C 예측 검증은 NO-CONFIG-PASSES로 닫혔으나(상승 예측 claim 금지), USER 결정으로 B++ diagnostic funnel을 production에 적용. ⚠️ **`stock_reports` 2026-06 30행은 73차 옛 30 티커를 서술하는 stale 상태** — 새 B++ 30 리포트(Tier2 포함) 재생성은 USER 마이그 0040 apply + flags + 비용 게이트 후(§9 #4·§3). 따라서 위 stock_reports/committee_votes 30/330 audit는 옛 리포트(미삭제)이며 새 30과 불일치 상태.
...
| **실 AI 검증** | P1(2026-05 ₩334.71) + 73차 풀 P3(₩24,655.64) + 74차 P2b(₩1,695.83) + 75차 P4 30 리포트(₩14,962.66) + 76/77차 proposal(₩54.71) + **2026-06-24 G2 B++ 재선정(~₩27k)**. 2026-06 월 누계 **≈₩68,034**(hardcap 50만 내). 다음 실 AI 비용 이벤트 = 새 30 리포트 Tier2 생성(~₩15-20k, USER 마이그 0040 + flags 게이트). |
| **선정 흐름 (production)** | `short_list_30` 2026-06-01 = **B++ 새 30 AI 배지/ai_score**(🟣16/🟢4/🟡9/🔵1, 10/10/10; 리더 2/11 SK하이닉스·HD현대일렉트릭; **2026-06-24 B++ 재선정으로 73차 30 교체**) · 2026-05-01 = 30 Tier0 incumbents. 메인 path = short 주간 + mid·long 월간 rolling. **B++ funnel 적용 완료(G1/G2). 다음 = 새 30 리포트 Tier2 생성 → go-live USER 게이트 + S7b**. |
...
| **풀 리포트 (production)** | `stock_reports` 2026-06 **30행 = 73차 옛 30 티커 서술(stale)** — G2 재선정으로 `short_list_30`은 B++ 새 30이나 리포트는 미재생성. ⚠️ 새 30과 불일치. **PR-T2a(Tier2 배선) ✅ CONVERGED(dormant)** — 새 30 리포트(Tier2 포함) 재생성은 USER 마이그 0040 apply + flags + 비용 게이트 후. report_batch_job 30 done(옛). 2026-05-01 004150 1행(P1 잔존). |
| Supabase | project `rbrpcynhphrpljbjirfo` · **마이그 0001~0037 production applied** + **0040 `commit_sector_personas_cron` written-not-applied(USER apply 게이트 — PR-T2a 리포트 생성 선행조건)**. cron RPC grants = service_role(섹터 0040 동일 패턴: public/anon/authenticated revoke). |
...
| **Pre-launch 섹터 비교 메뉴 + Tier2 잔여 렌더 + D4 디자인 freeze** | CLAUDE + USER 비용승인 | S7d 완료 후, S9 진입 전 | 섹터 추천 비교 메뉴 build(**production B++ 30 vs Track2 `sector-soft-tilt`(B++ + 섹터) 30 + returns**, hard-gate live 금지·검증 전 자동교체 금지) + **PR-T2a ✅ 완료/CONVERGED(dormant)** 후 잔여 **PR-T2c Section 8 섹터 보드 렌더**(+ 선택 D-1 dangling cleanup) + D4 `/gstack-design-review` QA/polish/freeze. 새 30 리포트 Tier2 생성은 USER 마이그 0040 apply + flags + 비용 게이트. PR-A5/PR-B5 통계 verdict는 deferred/연구. |
...
| ★ **새 30 리포트 Tier2 포함 1회 생성 — USER 게이트(다음 1순위 ⓐ)** | PR-T2a 코드 ✅ CONVERGED(dormant). 새 B++ 30 리포트(Tier2 섹터 보드 포함)를 생성하려면 USER: ① **마이그 0040 apply** `tudal/supabase/migrations/0040_commit_sector_personas_cron.sql`(Supabase MCP apply_migration[OAuth] 또는 dashboard SQL editor) ② Vercel/env flag `SECTOR_BOARD_ENABLED=true` + `PR5B_SECTION8_ENABLED=true` + `AI_COST_LOG_REAL_INSERT_ENABLED=true` ③ 리포트 재생성 비용 승인(~₩15-20k). 이후 CLAUDE가 P4-style 생성(Tier2 포함) 실행. | USER 마이그 apply + flags + 비용 → CLAUDE 실행 |
...
| Tier0 B++/B+C 예측 검증 캠페인 | **Research/CLOSED for prediction claim only**. step-2/full-factor/tradable-denominator/combination 모두 NO-CONFIG-PASSES/FAIL이라 상승 예측 claim 금지. USER 결정으로 production 150/30은 **B++ 기준 교체됨(2026-06-24)**; 단 `stock_reports` 2026-06 30행은 **73차 옛 30 stale**이라 새 B++ 30 리포트(Tier2 포함) 재생성 전까지 불일치. money-path / production effect 0 불변은 검증 캠페인에 한정. | `docs/superpowers/specs/2026-06-12-tier0-scoring-bplus-validation.md` 2026-06-19 UPDATE, `docs/superpowers/reviews/2026-06-17-tier0-4config-multiregime-verdict.md`, `docs/superpowers/2026-06-18-tier0-tradable-winner-denominator.md`, `docs/superpowers/2026-06-18-tier0-combination-campaign.md` |
...
| Tier2 섹터 페르소나 → 리포트 배선 | Pre-launch quality lane. **PR-T2a ✅ 완료/CONVERGED(2026-06-24)**: live report path에 `SECTOR_BOARD_ENABLED` seam + cron service-role commit path 배선, migration 0040 written-not-applied(USER gate), dormant default. 잔여 = **PR-T2c render** + 선택 D-1 dangling cleanup; 새 30 리포트 Tier2 포함 생성은 USER 마이그 0040 apply + flags + 비용 게이트. | `docs/superpowers/specs/2026-06-23-tier2-sector-persona-report-wiring.md` |
