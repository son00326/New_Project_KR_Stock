...
### 빌드 순서 = W0 ✅ → W2a ✅ → W2b ✅ → W1a ✅ → W1b ✅ → W3a ✅ → **W3b-1 ✅** → W3b-2 → W3b-3  (**69차: W3b-1(portfolio_proposal AI 클라이언트 + proposePortfolio 플러밍) MERGED PR #97, code-merge `6b118bc`, docs-sync main `89a4f18` — 다음 = W3b-2(portfolio_proposal DB 영속 + Accept 통합)**)
...
**W3 — Q2 AI 자율 포트 구성** (분할: **W3a ✅**[entry_price KRX EOD, AI-key-free] / **W3b-1 ✅**[AI 제안 클라이언트+proposePortfolio 플러밍] / **W3b-2 = 다음 1순위**[DB 영속+Accept 통합] / **W3b-3**[admin UI])
- **W3b-1 완료 (69차, PR #97)**: `callPortfolioProposal`(Opus `portfolio` role) + `PortfolioProposalSchema`/`parsePortfolioProposal` + `proposePortfolio` admin action(is_admin+flag+key+cost-logging+hardcap 게이트 → proposal **반환만**). 편입 여부/총 개수/단·중·장 분배/종목별 비중/현금 0~30% **AI 판단**. behavior-neutral·DB 영속 0·Accept 무변경.
- **W3b-2 (다음)**: `portfolio_proposal` DB 테이블 + 영속 + Accept가 proposal 기반 종목/비중 사용(money-path 통합). **W3b-3**: admin UI 버튼·표시 + selection finalize 자동 연결.
...
> ⚠️ **다음 세션 1순위 = W3b-2(portfolio_proposal DB 영속 + Accept 통합).** 완료 경로 = 상단 ⭐ 65차 MVP 엔진 W0 ✅ → W2a ✅ → W2b ✅ → W1a ✅ → W1b ✅ → W3a ✅ → W3b-1 ✅ MERGED(69차 PR #97). 아래 "지금 어디 / 다음 액션 큐"의 **PR-G ⓑ 1순위 표현은 W0~W3로 superseded**(코드 자산은 토대 재사용). 현 게이트 수치 = 69차 기준(test:ci 1847+2skip, W3b-1 후).
