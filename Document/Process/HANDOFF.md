...
git rev-parse --short HEAD          # 2026-06-25 tier0-bpp-multiregime FF-merged → main(c1b4981) + Vercel 배포 (runtime verify)
...
| 영역 | 상태 |
|---|---|
| main HEAD | **runtime verify** `git rev-parse --short origin/main` = **`c1b4981`**(B-PARTB merge ea27656 + 후속 docs). **2026-06-25: `tier0-bpp-multiregime`(72 commits = B++ funnel/shadow PR-A·B/PR-T2a/B-PARTB) FF-merged → main + Vercel prod 배포(canary 4/4 200, OPEN PR 0)** — 비-B-PARTB 코드는 flag-gated/dormant(분류 = main-merge-classification.md). (직전 285339a 77차 PR #122.) |
