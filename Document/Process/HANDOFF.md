...
### 빌드 순서 = W0 ✅ → W2a ✅ → W2b ✅ → W1 → W3  (**68차: W2b(D27 Q5 incumbent) MERGED PR #91 — 다음 = W1(반박 토론 loop)**)
...
- ~~즉 **다음 세션 1순위 = W0**~~ **[68차: W0 ✅ → W2a ✅ → W2b ✅ MERGED(PR #91) → 다음 세션 1순위 = W1(Q4 반박 토론 loop)]**. 아래 §한눈에·§다음 액션 큐의 "PR-G ⓑ 1순위" 표현은 본 섹션으로 대체된다(코드/DB/게이트 현 상태 서술은 유효).
...
> ⚠️ **다음 세션 1순위 = 상단 ⭐ 65차 MVP 엔진 섹션 — W0 ✅ → W2a ✅ → W2b ✅ MERGED(68차 PR #91) → W1 → W3.** 아래 "지금 어디 / 다음 액션 큐"의 **PR-G ⓑ 1순위 표현은 W0~W3로 superseded**(코드 자산은 토대 재사용). 현 게이트 수치 = 68차 기준(test:ci 1745+2skip, PR #91 후).
...
**main HEAD**: `git rev-parse --short origin/main` (2026-06-05 68차 W2b PR #91 후 = `ff32dbd` 또는 자손).
**OPEN PR**: **#2**(format-error, CONFLICTING 보류) only.
**검증 게이트**: build 26 routes / lint 0 err 0 warn / **test:ci 1745 PASS + 2 skipped(smoke 의도) / 148+1 files** / tsc clean / Python 95.
...
> ⚠️ **superseded by 상단 ⭐ 65차 MVP 엔진 섹션.** 68차 W2b ✅ MERGED(PR #91) 이후 다음 세션 1순위 = **W1** (빌드 순서 W0 ✅ → W2a ✅ → W2b ✅ → W1 → W3). 아래 PR-G ⓑ 큐는 코드 자산(청크워커/큐/run-mutex)을 W0~W3 토대로 재사용하는 형태로 흡수됨 — 역사 기록으로 보존.
...
git rev-parse --short HEAD          # 2026-06-05 68차 W2b PR #91 후 `ff32dbd` 또는 자손
...
#   main 기준 기대 (2026-06-05 68차 PR #91 후, main `ff32dbd`): build 26 routes / lint 0 err 0 warn / test:ci 1745 PASS + 2 skipped(w0 smoke 의도) / 148+1 files / tsc clean / Python 95 tests PASS
...
| 영역 | 상태 |
|---|---|
| main HEAD | **runtime verify** `git rev-parse --short origin/main` (2026-06-05 68차 W2b PR #91 후 `ff32dbd` 또는 자손). |
...
| 검증 게이트 (main, 2026-06-05 68차 PR #91 후 `ff32dbd`) | build 26 routes / lint 0 err 0 warn / **test:ci 1745 PASS + 2 skipped / 148+1 files** / tsc clean / **Python 95 tests PASS**. |
...
### §2.1 Step matrix — 8-row (legacy B65/B66/PR5 Task 구조 — 역사 기록. **현 1순위 = 상단 ⭐ MVP 엔진 W1** [W0 ✅ → W2a ✅ → W2b ✅ MERGED · 구: PR-G])

**현재 위치** = **실데이터+실AI e2e 트랙** (PR-A~F + PR-G ⓐ MERGED + 마이그 0028~0030 applied). **[→ 65차 supersede · 68차: W0 ✅ → W2a ✅ → W2b ✅ MERGED] 현 1순위 = 상단 ⭐ MVP 엔진 W1** (구: PR-G ⓑ 실 AI 첫 30선정 — 코드 자산은 W0~W3 토대 재사용). 아래 8-row Task matrix는 **legacy(B65/B66/PR5 Task 구조) 역사 기록** — 현 진행은 e2e 11-PR 로드맵(ADR §4)이 SoT. PR5 go-live는 별도 트랙(USER 게이트, dormant). **상세 박제는 git log + PR body + ADR/plan docs 위임.**
...
> ⚠️ **본 §6은 과거 세션 완료 기록(history).** 각 entry의 "다음: PR-G ⓑ …" 표현은 **해당 차수 당시 기준**이며, **현재 1순위는 상단 ⭐ MVP 엔진 W0✅→W2a✅→W2b✅→W1→W3 중 W1** (PR-G ⓑ는 W0~W3로 superseded·코드 자산 재사용). 과거 행은 역사로 보존하며 개별 정정하지 않는다.
...
- **다음**: **W1(Q4 실시간 반박 토론 loop)** — incumbent context는 동일 seam으로 토론 프롬프트 재사용. W1 follow-up 누적: 단발 orchestrator/`upsertShortList30` 정리 후보(67차 ②) + trackOfPeriod 소비(67차 ③) + 66차 W1 티켓 4종(D28 mix 배선/R2 trigger·dual-judge config 소비/SDK retry 단일화/calibration). USER 게이트 불변 = `SELECTION_CRON_AUTO_ENABLED`+키/비용 + 주간 tier0 producer.
