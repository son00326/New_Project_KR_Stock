...
### 빌드 순서 = W0 ✅ → W2a ✅ → W2b → W1 → W3  (**67차: W2a(주간/월간 split) MERGED PR #89 — 다음 = W2b(D27 Q5 incumbent)**)
...
**W2a — Q1 주간/월간 split + rolling composite** ✅ **MERGED (67차, PR #89, main `f5f11ca`** — plan SoT = `docs/superpowers/plans/2026-06-04-w2a-weekly-monthly-split.md` R1~R5 CONVERGED · 신규 SoT = `selection-period.ts` + `makeTier1ScreeningResultSchema(track)` + `upsertShortListTrack` + 마이그 0031 재작성 `replace_shortlist_track`/`carry_short_into_month` RPC + selection-worker due-gate + 소비자 <30 가드. **D27 Q5 incumbent는 W2b로 분리.** USER 게이트 = 0031 apply + `SELECTION_CRON_AUTO_ENABLED` + 주간 tier0 producer. 아래 원 scope는 역사 기록.)

**W2b — D27 Q5 incumbent thesis 재점검** (⭐ **다음 세션 1순위** — W2a 위 additive)
- 트랙별 후보풀 = fresh Tier0 ∪ incumbents(직전 short_list_30) union+dedupe + expected_total 동적화(단기 50+≤10 / 중장기 100+≤20). incumbent 무심사 탈락 금지(탈락=W1 랭킹 후).
- per-ticker incumbent thesis context builder(직전 ai_comment_kr/배지/점수 + stock_reports section_0.thesis + 실현성과[W3 entry_price 의존, graceful]) → reflectionContext seam(현 ''→per-ticker map 확장) 주입.
- delta_status hold/removed 실계산(직전 row diff). W2a의 트랙 seam 위 additive.
...
- ~~즉 **다음 세션 1순위 = W0**~~ **[67차: W0 ✅ → W2a ✅ MERGED(PR #89) → 다음 세션 1순위 = W2b]**. 아래 §한눈에·§다음 액션 큐의 "PR-G ⓑ 1순위" 표현은 본 섹션으로 대체된다(코드/DB/게이트 현 상태 서술은 유효).
...
- **잔여 follow-up (비차단, W2b/W1에서)**: ① vercel.json의 옛 `monthly-batch` 스케줄(`5 0 1 * *`)은 stub 200이라 무해하나 W2b에서 제거 검토 ② 단발 orchestrator/`upsertShortList30` 코드 본체는 deprecated-미호출 상태로 잔존(W2b 정리 후보) ③ `trackOfPeriod`는 W2b incumbent 식별에서 소비 예정.
...
> ⚠️ **다음 세션 1순위 = 상단 ⭐ 65차 MVP 엔진 섹션 — W0 ✅ → W2a ✅ MERGED(67차 PR #89) → W2b → W1 → W3.** 아래 "지금 어디 / 다음 액션 큐"의 **PR-G ⓑ 1순위 표현은 W0~W3로 superseded**(코드 자산은 토대 재사용). 현 게이트 수치 = 67차 기준(test:ci 1703+2skip, PR #90 후).
...
**main HEAD**: `git rev-parse --short origin/main` (2026-06-04 67차 W2a PR #89 + 배선감사 fix PR #90 후 = `6e1aa03` 또는 자손).
**OPEN PR**: **#2**(format-error, CONFLICTING 보류) only.
**검증 게이트**: build 26 routes / lint 0 err 0 warn / **test:ci 1703 PASS + 2 skipped(smoke 의도) / 146+1 files** / tsc clean / Python 95.
...
> ⚠️ **superseded by 상단 ⭐ 65차 MVP 엔진 섹션.** 67차 W2a ✅ MERGED(PR #89) 이후 다음 세션 1순위 = **W2b** (빌드 순서 W0 ✅ → W2a ✅ → W2b → W1 → W3). 아래 PR-G ⓑ 큐는 코드 자산(청크워커/큐/run-mutex)을 W0~W3 토대로 재사용하는 형태로 흡수됨 — 역사 기록으로 보존.
...
#   main 기준 기대 (2026-06-04 67차 PR #89+#90 후, main `6e1aa03`): build 26 routes / lint 0 err 0 warn / test:ci 1703 PASS + 2 skipped(w0 smoke 의도) / 146+1 files / tsc clean / Python 95 tests PASS
...
| 영역 | 상태 |
|---|---|
| main HEAD | **runtime verify** `git rev-parse --short origin/main` (2026-06-04 67차 PR #89 + 배선감사 fix PR #90 후 `6e1aa03` 또는 자손). |
...
| 검증 게이트 (main, 2026-06-04 67차 PR #89+#90 후 `6e1aa03`) | build 26 routes / lint 0 err 0 warn / **test:ci 1703 PASS + 2 skipped / 146+1 files** / tsc clean / **Python 95 tests PASS**. |
| **W2a (67차)** | ✅ **MERGED** (PR #89 rebase FF). 주간/월간 트랙 split + rolling composite. selection-period(period_key s:월요일/m:YYYY-MM) + makeTier1ScreeningResultSchema(track) + upsertShortListTrack + 마이그 0031 재작성(replace_shortlist_track/carry_short_into_month RPC, **dormant=USER apply**) + selection-worker due-gate + 소비자 <30 가드. omxy plan R1~R5(22 catch)+impl R1~R2 CONVERGED. **다음 = W2b(D27 Q5 incumbent).** |
...
### §2.1 Step matrix — 8-row (legacy B65/B66/PR5 Task 구조 — 역사 기록. **현 1순위 = 상단 ⭐ MVP 엔진 W2b** [W0 ✅ → W2a ✅ MERGED · 구: PR-G])

**현재 위치** = **실데이터+실AI e2e 트랙** (PR-A~F + PR-G ⓐ MERGED + 마이그 0028~0030 applied). **[→ 65차 supersede · 67차: W0 ✅ → W2a ✅ MERGED] 현 1순위 = 상단 ⭐ MVP 엔진 W2b** (구: PR-G ⓑ 실 AI 첫 30선정 — 코드 자산은 W0~W3 토대 재사용). 아래 8-row Task matrix는 **legacy(B65/B66/PR5 Task 구조) 역사 기록** — 현 진행은 e2e 11-PR 로드맵(ADR §4)이 SoT. PR5 go-live는 별도 트랙(USER 게이트, dormant). **상세 박제는 git log + PR body + ADR/plan docs 위임.**
...
> ⚠️ **본 §6은 과거 세션 완료 기록(history).** 각 entry의 "다음: PR-G ⓑ …" 표현은 **해당 차수 당시 기준**이며, **현재 1순위는 상단 ⭐ MVP 엔진 W0✅→W2a✅→W2b→W1→W3 중 W2b** (PR-G ⓑ는 W0~W3로 superseded·코드 자산 재사용). 과거 행은 역사로 보존하며 개별 정정하지 않는다.
