...
### Tier0 스코어링 B++ 1차 구현 + 삼중 게이트 harness + cross-model 적대 검토 루프 (77차, PR #121 MERGED, main `8110e8c`)
...
- **다음 = gated step-2**: Gate A/B 실 12-24M PIT harvest(장시간 run + DART rcept_dt PIT + baseline 3종 wiring) → 삼중 게이트 ALL PASS 시에만 `--scoring bpp --apply`(비용 0) → **Tier1 재선정(USER ₩25k)** → production short_list_30. **Gate C smoke**(실 KRX dry-run·비용 0)은 본 세션 진행(망 블립 1회 → throttle-내성 hardening 후 재가동).
