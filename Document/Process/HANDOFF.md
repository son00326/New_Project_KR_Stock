...
1. **현재 branch**: `feat/tier2-implementation` (PR #5, stacked from PR #4)
2. **OPEN PRs**: #2 (format-error 보류) / #4 (Tier 2 SoT) / #5 (Tier 2 impl) — 모두 MERGEABLE
3. **USER-gated 대기**: PR #4 → #5 → #2 순서 review/merge + 마이그 0018+0019 production apply
4. **다음 1순위 CLAUDE 작업** (§2.1 Step 3a): **Kevin IMVCOM branch (`origin/IMVCOM`) 분석 + golden reference 정합 머지 PR** — 그 후 Step 3b (production prompts 196, Kevin v3.1 quality target)
...
- (A) PR #4 + #5 둘 다 MERGED → main 기준 새 branch (Step 3a Kevin IMVCOM 분석 또는 Step 3b production prompts 196)
- (B) PR #4 MERGED, #5 OPEN → PR #5 rebase 후 진행
- (C) PR #4 OPEN → USER-gated (PR review/merge 트리거 보고) + 동시 가능한 CLAUDE 작업 (Step 3a Kevin 분석 등) 자동 시작
...
| **Kevin IMVCOM reference** | 📌 **Tier 2 production prompts quality target** = `origin/IMVCOM @ 1faee1b` (v3.1 초보 친화 알테오젠 리포트 + NarrativeDesign + Section4-Trial). main 미반영 4 commits. **Step 3a 정합 머지 PR 진입 필요**. |
...
### §2.1 Step matrix (52차 종료 시점 — 현재 위치 = **Step 1c USER 대기 (PR #4/#5 review/merge)** + 동시 **Step 3a CLAUDE 자동 시작 가능**)
...
| **1c** ⭐ | **USER** | PR #4 (Tier 2 SoT) + PR #5 (Tier 2 impl) OPEN MERGEABLE — destructive (production merge) | background blocker 보고 + Step 3a 자동 시작 | `gh pr merge 4 --rebase` → main fast-forward → 그 다음 `gh pr merge 5 --rebase` (PR #5는 PR #4 stacked) → Vercel production auto-deploy + 마이그 0018 + 0019 production apply (Supabase MCP `apply_migration`) | PR #4 + #5 둘 다 state=MERGED + 마이그 0018/0019 schema 검증 (`list_migrations` + smoke) | Step 3b·3c·5 trigger 가능 시점 |
...
| **3a** ⭐ NEW | **CLAUDE** | Step 1c USER 머지 후 (또는 stacked branch도 가능) | 즉시 자동 시작 — `origin/IMVCOM` (Kevin) 4 commits 분석 + 정합 머지 PR | new branch `feat/kevin-imvcom-merge` from main (또는 PR #4/#5 머지 후). push/PR create = SHARED. **PR merge = USER**. | v3.1 초보 친화 알테오젠 리포트 + NarrativeDesign + Section4-Trial + ReportFramework v3 진화분이 main에 정합 머지 (충돌 해결 — D21 v2.5 보존 + Kevin v3.1 추가) | Step 3b |
| **3b** ⭐ NEW | **CLAUDE** | Step 3a merge 후 (1세션+ 작업이라 진입 의사 1회 확인 후 자동) | 진입 의사 1회 확인 → 자동 진행 — production sector persona prompts 196 (canonical 14 × 14) + Core 11 prompt 정합 + Kevin v3.1 quality target 박제 | new branch `feat/tier2-production-prompts` from main. omxy 적대적 검토 + Kevin reference 정합. | 196 production sector persona prompts 작성 + tests + commit + push branch + PR create. **Kevin v3.1 톤·구조·깊이 follow** ([[handoff_kevin_v31_quality_target]] §9 박제). | Step 3c |
...
| **B-17c ⭐최우선** | **사용자 PR #4 (Tier 2 SoT) + PR #5 (Tier 2 impl) review/merge + 마이그 0018·0019 production apply** | (1) `gh pr view 4` 검토 + Vercel preview Ready URL 확인 → `gh pr merge 4 --rebase` (2) `gh pr view 5` 검토 + Vercel preview Ready 확인 → `gh pr merge 5 --rebase` (stacked, PR #4 merge 후) (3) Supabase MCP `apply_migration` 으로 0018 (`short_list_30_sub_tags`) + 0019 (`commit_sector_personas`) production apply (4) origin/main 갱신 후 Vercel production auto-deploy | **Tier 2 SoT + impl 완료 게이트 — Step 3a (Kevin IMVCOM 분석) 자동 진행 가능 시점** |
...
| **📌 Kevin IMVCOM reference (Tier 2 production prompts golden output target)** | `origin/IMVCOM` branch (Kevin `kevinoh@Kevinui-MacBookPro.local`), HEAD `1faee1b` "feat: 보고서 프레임워크 v3.1 + 알테오젠 초보 친화 리포트 (투심위 복원)". 4 commits ahead of main: `44f6151` (HTML 알테오젠) + `3b22d01` (v2.1 + 알테오젠 v2) + `c00153c` (삼천당 풀 리포트) + `1faee1b` (v3.1 초보 친화 + NarrativeDesign + Section4-Trial). **다음 세션 Step 3a Kevin 정합 머지 PR 진입 대상** |
...
상세는 git log + spec/plan/Slice 파일. 직전 항목 (52차 Tier 2 SoT + impl PR → 51차 PR #1 머지 + Tier 2 brainstorm → 50차 §1 B-17 EXECUTED 순):
...
- **52차 Tier 2 SoT (PR #4) + Tier 2 implementation (PR #5) + HANDOFF 박제 (2026-05-20)**:
  ...
  - **다음 1순위 (53차+)**: Step 1c USER (PR #4 + #5 머지 + 마이그 0018·0019 production apply) → Step 3a CLAUDE (Kevin IMVCOM 정합 머지 PR) → Step 3b CLAUDE (production prompts 196, Kevin v3.1 quality target) → Step 3c CLAUDE (caller wiring) → Step 4 CLAUDE (Reflection).
...
총 누적 (CONVERGED only):                 69 rounds  CONVERGED (52차 종료 시점, 안정)
```
...
- [ ] 새 branch 생성:
  - Step 3a (Kevin IMVCOM 정합) → `feat/kevin-imvcom-merge`
...
- [ ] **Step 3a 진입 시**: `origin/IMVCOM` 4 commits 분석 + main 정합 머지 전략 (cherry-pick / 충돌 해결 — D21 v2.5 보존 + Kevin v3.1 추가) + ReportFramework v2.5 ↔ v3.1 정합
- [ ] **Step 3b 진입 시**: Kevin v3.1 reference 톤·구조·깊이 분석 → 196 sector persona system prompt 박제 (`@/lib/ai/prompts/personas/` 신규 디렉토리)
...
- [ ] **ExecutionPlaybook.md 갱신 필요 여부 판단** (Step 3a/3b/3c가 새 execution routing 카테고리라면 §2 매핑 표 보강)
...
- **📌 Tier 2 production prompts quality target = Kevin v3.1 reference (52차 박제 [[handoff_kevin_v31_quality_target]])**: 다음 세션 Step 3b (production sector persona prompts 196 작성) 시 **`origin/IMVCOM @ 1faee1b` Kevin v3.1 초보 친화 리포트의 톤·구조·깊이를 따라야 함**. 단순 lorem-ipsum / "Mock Persona" stub 금지. 알테오젠 v3.1 reference 분석 → 14 sector × 14 persona system prompt 박제. Step 3a 정합 머지가 본 reference 안전 박제 전제. 자세한 SoT pointer = §5 표 + ServicePlan-Admin §1A.5 D22.
- **HANDOFF.md 다음 세션 자동 진행 가능 조건**: header (52차 5줄 요약) + §1 + §2 + §8 모두 stale 0. 본 52차 종료 시점 omxy 박제 R1 CONVERGED + 69 rounds 누적 + branch `feat/tier2-implementation` HEAD = 52차 §2 HANDOFF 박제 commit + Runbook 현재 위치 = Step 1c USER 대기 (PR #4/#5 머지) + Step 3a CLAUDE 자동 진행 가능.
