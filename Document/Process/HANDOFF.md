...
Last updated: 2026-06-27 (**⭐ PR-K Reflection(AI 자가 학습) 출시 전 shadow-first 빌드 ✅ 완료(dormant, D32)** — `tudal/src/lib/reflection/*`(types/config/flags/metrics/reflection-context/ledger/reflection-source/orchestrator/prices/summary-prompt/summarizer) + `data/admin-reflection.ts`(upsert/claim/getLatest/getPriorFinalizedCycle/getCyclePanels — 멀티라운드 R2>R1 dedup) + 마이그 **0043 `reflection_log` DORMANT(USER apply-only)** + `scripts/pg_smoke_0043.sh`. 직전 finalize 사이클(track별)의 평가 후보 풀 실현 수익률(KRX EOD, **무비용**) → 페르소나별 적중률·conviction-가중 수익률 → reflection_log 영속 → 다음 선정 W1 패널 prompt에 **신규 `reflectionLearningContext` 필드** 주입(macro/negative-news 3번째 supplementary block, **D27 Q5 `reflectionContext`[per-ticker]와 별개 필드**). **shadow-first**: `REFLECTION_ENABLED` off(default) → 선정 prompt byte-identical(선정 무회귀)·회고 미실행·mutation 0. fail-soft: prior cycle/KRX/snapshot 미가동 → no-op. **회고지 예측 아님**(reflection_kind='retrospective' CHECK + 면책 + summarizer 예측 출력 필터 + 예측어휘 0 테스트). LLM 케이스 요약은 별 flag `REFLECTION_LLM_SUMMARY_ENABLED`(default off) + fail-closed(AI_COST_LOG + CRON_SYSTEM_USER_ID UUID/getUserById + hardcap reservation + atomic claim re-burn 방지). cron route `/api/cron/reflection-job`(vercel.json 무변경 — write seam schedule = USER go-live). Claude 5-lens 적대 리뷰(4 HIGH) → omxy R1 적대 검토(7 fix: fail-closed/atomic claim/예측 필터/now seam/period_key) → Claude 2차 검증 **CONVERGED**. 게이트 green(test:ci 2345 PASS + 5 skipped)·tsc·PG smoke. 코드만(flag dormant·production 미접촉). USER 활성 게이트 = `REFLECTION_ENABLED`+(선택)`REFLECTION_LLM_SUMMARY_ENABLED`+0043 apply+`SELECTION_CRON_AUTO_ENABLED`+`KRX_OPENAPI_KEY`+reflection-job schedule(§3). SoT = `docs/superpowers/specs/2026-06-27-reflection-prk-build.md`. 이전: **⭐ S7b M12a 뉴스 기반 자동 제외(AI 페르소나) shadow-first 빌드 ✅ 완료(dormant)** — `tudal/src/lib/news/m12a/*`(verdict/brake/cashout/ledger/negative-news-context/telegram-text/evaluator/orchestrator/briefing-integration/auto-remove/run-result + config/flags/types) + `data/admin-m12a.ts` + 마이그 **0042 `m12a_ticker_assessment` DORMANT(USER apply-only)** + `scripts/pg_smoke_0042.sh`(10 assertions). Core 11 뉴스평가 → per-company thesis-break 구조화 판정 → 결정론 verdict → run-level smart brake(per-company mass>3·track 70% floor·집중포트) → durable ledger(`news_event` 1 + `m12a_ticker_assessment` N, action_taken 3-layer 귀속) → shadow/mutate 게이트 → 텔레그램+`/admin` durable alert(news_critical/news_warning 재사용·alert_type enum 불변·이메일 0). **shadow-first**: `M12A_AUTO_REMOVE_ENABLED` off→mutation 0·`M12A_NEWS_EVAL_ENABLED` off→morning-briefing byte-identical. 자동 mutation=출시 후 fast-follow(throw-stub). go-live fail-closed step-0(AI_COST_LOG+CRON_SYSTEM_USER_ID UUID/getUserById). Claude dynamic-workflow 빌드 → omxy R1~R3 CONVERGED(5 fix) + Claude 5-dim deep-review(HIGH 2/MEDIUM 1 fix) + omxy R4 docs. 게이트 green(test:ci 2246 +5 skip)·tsc·PG smoke. 코드만(flag dormant·production 미접촉). USER 활성 게이트 = `M12A_NEWS_EVAL_ENABLED`+`AI_COST_LOG_REAL_INSERT_ENABLED`+`CRON_SYSTEM_USER_ID`+0042 apply+Naver/Telegram/AI 키(§3). M12a 자동 mutation=출시 게이트 아님(§2.2 7-criteria 불변). SoT = `docs/superpowers/specs/2026-06-26-m12a-news-auto-remove-shadow-first.md`. 이전: **⭐ G4 거시/뉴스 AI 컨텍스트 레이어 ✅ 완료(dormant, D33 §4)** — `lib/macro/{context,source}` pure distill + 리포트 writer/모닝 브리핑(M11)/LIVE Tier1 선정 패널(R1/R2) 3-consumer 컨텍스트 주입(단일 flag `MACRO_CONTEXT_ENABLED` default off → byte-identical·선정 무회귀), Tier0 factor 금지(import allowlist 테스트 박제)·M12a 범주 분리·forward-validate(예측 아님)·stale-mock fail-safe + **M11 모닝 브리핑 Resend 제거**(72차/D10 = Telegram+dashboard). Claude TDD draft → omxy R1 CONVERGED → **Claude deep-review가 HIGH catch(macro가 dangling runMonthlyPersonaEval에만 배선·LIVE selection-worker 경로 미도달) → live-path fix** → omxy R2 CONVERGED. 게이트 green(test:ci 2141 passed). 코드만(flag dormant·production 미접촉). USER 활성 게이트 = `MACRO_CONTEXT_ENABLED`+실 거시 source. **완료 범위 = mock-macro source distill + 3-consumer wiring(dormant); 실 FRED/news source + Polymarket(선택) = USER 게이트/후속.** SoT = `docs/superpowers/specs/2026-06-26-g4-macro-news-ai-context-layer-design.md`. 이전: **⭐ B++ funnel 적용(G1/G2) + Tier2 리포트 배선(PR-T2a) + ⓐ 새 30 리포트 Tier2 생성·검증 전부 완료. production `tier0_candidates_150`/`short_list_30` 2026-06 = B++ cfg1 기준(150 리더 4/11·30 리더 2/11). `stock_reports` 2026-06 = B++ 새 30 (section_0~8+appendix + committee_votes 330 core + 420 sector = 750, partA 14·sector_aggregate, NULL-sector 0; 마이그 0040/0041 production applied — live-catch CHECK 23514 fix 포함). B++ = diagnostic funnel 업그레이드지 예측 게이트 통과 아님(NO-CONFIG-PASSES 유지). 다음 1순위 = ⓑ go-live USER 게이트 + S7b. PR-T2c 렌더·섹터 비교 메뉴·PR-K Reflection·D33 G1/G3는 S9 전 pre-launch lane. **B-PARTB(Part B 쟁점 실제 추출) ✅ 완료·CONVERGED + MERGED→main + Vercel prod DEPLOYED(2026-06-25, B-PARTB merge `ea27656`; 현 main HEAD = runtime verify `git rev-parse --short origin/main`) — writer.ts `extractIssueDebates` pure deterministic이 옛 stub/raw-json 대체, B-PARTB 29 tests, plan R1~R3 + impl R1~R2 omxy + Claude 3-lens review 수렴. **기존 30 리포트 Part B = ₩0 in-place 백필 APPLIED ✅(2026-06-25, AI 0원 — DB partD.one_line + committee_votes.argument_excerpt 재구성; 결함 인용 117[stub60+json30+empty27]→0, 30/30 schema-valid+clean; backup `scripts/out/b-partb-backfill-backup-2026-06-01.json`; gated driver omxy CONVERGED).** 찬/반/중 렌더 production live, canary 4/4 200.)
...
5. ★ **다음 1순위 — go-live USER 게이트 + S7b 뉴스·브리핑.**
   - **USER**: 매달 자동화 승인/flag(`SELECTION_CRON_AUTO_ENABLED=true`, Vercel env에서 `SELECTION_CRON_SELF_CONTINUE` 삭제), 주간 tier0 producer 외부 스케줄, 운영 비용 승인, S7b용 B-8 Naver + B-9 Telegram + AI 키 확인. PR5 report cron은 별도 flag(`PR5_CRON_AUTO_ENABLED`)로 §3에서 함께 관리. **(G4) `MACRO_CONTEXT_ENABLED=true` + 실 거시 source 연결**(현 source=mock-macro·stale fail-safe로 flag on이어도 ""→dormant) = 거시 컨텍스트 활성 게이트.
   - **CLAUDE**: **G4 거시/뉴스 AI 컨텍스트 레이어 ✅ 완료 (dormant, 2026-06-26 · D33 §4)** — `lib/macro/{context,source}` distill + 리포트 writer/모닝 브리핑(M11)/**LIVE Tier1 선정 패널(R1/R2)** 3-consumer 주입(`MACRO_CONTEXT_ENABLED` off → byte-identical·선정 무회귀). Tier0 factor 금지(import allowlist 테스트 박제)·M12a 범주 분리·forward-validate(예측 아님). **M11 모닝 브리핑 Resend 제거**(72차/D10 = Telegram+dashboard). Claude TDD draft → omxy R1 CONVERGED → Claude deep-review(HIGH: live-path 미배선 catch) fix → omxy R2 CONVERGED. 게이트 green(test:ci 2141). SoT spec `docs/superpowers/specs/2026-06-26-g4-macro-news-ai-context-layer-design.md`.
   - **CLAUDE**: **S7b M12a 뉴스 기반 자동 제외(AI 페르소나) ✅ shadow-first 빌드 완료 (dormant, 2026-06-27)** — `lib/news/m12a/*`(verdict/brake/cashout/ledger/negative-news-context/telegram-text/evaluator/orchestrator/briefing-integration + config/flags/types/auto-remove/run-result). Core 11 뉴스평가 → per-company thesis-break 구조화 판정 → 결정론 `decideRecommendedAction` → run-level **smart brake**(per-company mass>3 / track 70% floor / 집중포트) → **durable ledger**(기존 `news_event` 1 + **신규 `m12a_ticker_assessment` N**, action_taken=shadowed|held_by_brake|removed = GAP1 3-layer 귀속) → shadow/mutate 게이트 → **텔레그램 + `/admin` durable alert**(news_critical/news_warning 재사용, alert_type enum 불변·이메일 0). **shadow-first**: `M12A_AUTO_REMOVE_ENABLED=false`(default) → would-remove/held_by_brake 이벤트+알림+ledger만, short_list_30/portfolio_snapshot **mutation 0**. `M12A_NEWS_EVAL_ENABLED=false`(default) → morning-briefing byte-identical(DB/AI 0). 자동 mutation은 출시 후 fast-follow(briefing-integration resolveCashout/applyAutoRemove = throw-stub). 마이그 **0042 DORMANT(USER apply-only)** + docker-free PG smoke(10 assertions PASS). 재진입 negative-news 컨텍스트 seam(selection-worker, G4 macro 패턴·off→byte-identical). go-live fail-closed step-0(AI_COST_LOG + CRON_SYSTEM_USER_ID UUID/getUserById, sibling worker parity). Claude dynamic-workflow 빌드 → omxy R1~R2 CONVERGED(5 fix) + Claude 5-dim deep-review(HIGH 2/MEDIUM 1 fix) → omxy R3 검토. 게이트 green(test:ci 2246 +5 skip)·tsc·PG smoke. SoT spec `docs/superpowers/specs/2026-06-26-m12a-news-auto-remove-shadow-first.md`. **USER 활성 게이트 = `M12A_NEWS_EVAL_ENABLED`+`AI_COST_LOG_REAL_INSERT_ENABLED`+`CRON_SYSTEM_USER_ID`+마이그 0042 apply+B-8 Naver+B-9 Telegram+AI 키/비용**(§3). M12a 자동 mutation = 출시 게이트 아님(§2.2 7-criteria 불변). [SoT: `ServicePlan-Admin.md §3.10`, 본문 §2.2 Step 7]
6. **D11 운용 검증.** S7b 후 KIS 0개로 어드민 3인이 며칠~1주 운용 검증(의사결정 품질·승인·재생성 cap·알림 정확도). **(병렬 착수) G1 Reflection Lab / PR-K 계측 scaffold(로그·스키마·시작 신호) — 계측 먼저·완성 늦게**(D11 직후 병렬 착수해야 S9 첫날부터 데이터가 쌓임; 계측 scaffold ≠ 자동 회고 cron job·완성/튜닝은 S9 직전/중·자동 적용 금지). [SoT: §2.2 Step 8]
7. **S7c 장중·Exit 알림.** KIS read-only 1개 + Telegram/`/admin` 2-layer alert + 대안 3 + T+7 outcome. ⚠️ **KIS 발급 지연 시 G1/PR-K scaffold는 병렬 lane으로 계속(직렬 대기 금지)** — G1/PR-K는 KIS 불요. [SoT: §2.2 Step 9]
8. **S7d Silent Health.** success_rate/red_alert/heartbeat/override UI 실 연결. [SoT: §2.2 Step 10]
9. **Pre-launch 섹터 비교 메뉴 + Tier2 리포트 배선 잔여(PR-T2c 렌더) + TradingAgents graft(G1~G4) + 디자인 freeze(S9/출시 전).**
   ...
   - **Tier2 섹터 페르소나 → 30 리포트 배선**: **PR-T2a(생성 seam) ✅ DONE(위 #4) + 마이그 0040/0041 applied + 새 30 리포트에 섹터보드 데이터 생성 완료**(committee_votes 420 sector + partA 14 + sector_aggregate). **PR-T2c(Section 8 섹터 보드 FE 렌더) ✅ 검증 완료(2026-06-25)** — report page `Section8ModernView`가 "섹터 14인 패널(Part A)" + "Sector Board(Part C sector_aggregate)" 카드를 렌더(PR4 Task 4 기존 구현); 30/30 live 리포트가 modern-schema parse 통과 → 빈 fallback 0(live data shape + bounds + render-invariant test 검증). PR-T2b(비용 reservation)는 PR-T2a에 흡수. **B-PARTB(Part B 쟁점 실제 추출) ✅ 완료·CONVERGED + MERGED→main + DEPLOYED(2026-06-25, B-PARTB merge `ea27656`; 현 main HEAD = runtime verify `git rev-parse --short origin/main`)**: writer.ts `extractIssueDebates`(pure deterministic)가 옛 stub/raw-json/빈문자열을 대체 — Core 11 응답에서 의견 대립 쟁점 3~5개 추출 + label-attributed 인용(찬/반/중) 합성. Claude↔omxy 적대검토(plan R1~R3 + impl R1~R2) + Claude 3-lens review-purpose 검토 수렴, +9 mutation-pin 테스트(B-PARTB 29), 게이트 green. **기존 30 리포트 Part B = ₩0 in-place 백필 APPLIED ✅(2026-06-25, AI 0원)**: gated driver(`b-partb-backfill.live.test.ts`, omxy CONVERGED)가 DB partD.one_line + committee_votes.argument_excerpt를 재구성 → `extractIssueDebates` → partB만 UPDATE(나머지 무변경). backup→apply 30→post-verify 30/30 clean(결함 인용 117→0). backup `scripts/out/b-partb-backfill-backup-2026-06-01.json`(rollback). 상세 = `🔧 Pre-launch` 섹션 + spec `docs/superpowers/specs/2026-06-25-section8-partB-issue-extraction.md`.
   - **Toss-D0~D4 디자인 lane**: D0 now/spec-only → D1 S7b UI 전 → D2 D11 전 → D3 S7b/S7c 동시 → D4 S7d 후·S9 직전 freeze(`/gstack-design-review`).
   - **PR-K Reflection 자가학습 빌드(D32, 출시 전 승격) ✅ 완료(dormant, 2026-06-27)**: reflection_log(마이그 0043 DORMANT) + track별(주1/월1) 회고 job(cron `/api/cron/reflection-job`) + 다음 선정 prompt 주입(**신규 `reflectionLearningContext` seam — Q5 `reflectionContext`와 별개 필드**). 평가 후보 풀 실현 수익률(KRX·무비용) → 페르소나 적중률·가중수익 → reflection_log → 선정 패널 주입(off→byte-identical). 회고지 예측 아님(retrospective CHECK + 예측 필터). **빌드 완료 = S9 진입 선행조건(sequencing) 충족**, **동작·품질 검증 = S9 중 soft criterion**(§2.2 7-criteria 잠금 미변경). USER 활성 게이트 = §3. [SoT: `docs/superpowers/specs/2026-06-27-reflection-prk-build.md`(빌드) + `2026-06-24-reflection-prk-pre-launch-promotion.md`(D32 결정) · 본문 §2.2 runbook 표]
   - **TradingAgents graft 출시 전 lane (D33, 2026-06-25 USER 확정 · Claude↔omxy CONVERGED · 코드 빌드 0·로드맵만)** — per-ticker 토론 funnel 직접 이식 금지(oversight/Reflection 패턴만 차용) + NO-CONFIG-PASSES 유지(예측 claim 영구 금지·자동 교체 없음):
     - **G1 Tier0 Reflection Lab** (신규 pre-launch 빌드 + S9 관찰): 과거 150/30 + 실현 수익률 postmortem → 다음 달 B++ funnel 가중치를 champion/challenger **제안만 → USER 승인**. **최소 빌드(로그·제안만)·diagnostic only·자동 적용/예측 claim 금지·PR-K(D32)와 다른 층**(funnel 가중치 vs prompt 주입). 비용 낮음(데이터 DB 상주).
     - **G2 Leader-miss why-excluded audit** (기존 Path-A Track-2 흡수·**신규 빌드 금지**): 대형/리더 누락 사유 진단 1줄을 **섹터 비교 메뉴 + PR-B4 `shadow_reconcile.py` 위**에 표면화. **hard-gate live 영구 금지(soft only)·forward-validate**. ₩0.
     - **G3 Risk 3자 토론(공격/보존/중립)** (신규 pre-launch 빌드 · **USER가 출시 후→출시 전 이동**): portfolio Accept 전 1회, 30선정+포트(Q2 자율)를 risk 3-debator로 위험도 재판정(통과/조건부/거절) **컨텍스트 layer(advisory/shadow·비강제)**. **거래별 아님 = 포트 구성당 1회 shadow/advisory로 cost cap·Accept 게이트 substitute 아님**. 비용 중.
     - **G4 거시/뉴스/예측시장 → AI 컨텍스트** (S7b 결합) — **§4 1차 구현 ✅ 2026-06-26·dormant**(완료 범위 = `lib/macro` mock-macro distill seam + 3-consumer wiring[리포트/M11/LIVE Tier1 패널]; **실 FRED source + (선택)Polymarket = USER 게이트/후속**): 거시 regime+뉴스을 Tier1 평가/리포트/브리핑 **컨텍스트 입력**으로. **Tier0 정량 factor로 직접 사용 금지·M12a(per-ticker 자동제외)와 범주 분리**. ₩0.
     - **G5 Bull/Bear 심층 토론 = REJECTED** — JooPick Q4(실시간 반박 토론 loop, W1a/W1b 구현)와 중복 → 검토 후 채택 안 함(기록만).
     [SoT: `docs/superpowers/specs/2026-06-25-tradingagents-graft-prelaunch-roadmap.md` · 본문 §2.2 runbook 표]
10. **S9 1개월+ 운용 검증 → 🎉 출시.** 어드민 3인 실 사용 1개월+ + §2.2 7 criteria 통과 (★ S9 중 PR-K Reflection + D33 G1/G3 동작·품질 관찰, soft). 출시 = 자동매매 제외("AI 추천 + 가상 포트 + 알림" 내부 도구). S8 자동매매는 출시 후.
...
- **Tier2 섹터 페르소나 → 30 리포트 배선(PR-T2a/b/c)**: Core-11 리포트만으로 MVP ③은 완료됐지만, 출시 전 리포트 품질 상향 deliverable이다. **PR-T2a ✅ 완료/CONVERGED + 적용(2026-06-24)**: live `full-report-batch-worker → orchestrateFullReport` 경로에 `SECTOR_BOARD_ENABLED` seam + cron service-role commit path 배선 + **마이그 0040/0041 production applied** + 새 30 B++ 리포트에 섹터보드 생성 완료(420 sector votes + partA 14 + sector_aggregate, NULL-sector 0). PR-T2b 비용 reservation/모델 preflight는 PR-T2a에 흡수. 잔여 = **PR-T2c FE 렌더 ✅ 검증 완료(2026-06-25, omxy 교차검증 CONVERGED)**: `Section8ModernView`가 Part A 14인 패널(page.tsx:810-835) + Part C sector_aggregate 카드(page.tsx:779-807) 렌더, 30/30 modern-parse 통과·fallback 0 + **B-PARTB ✅ 완료·CONVERGED + MERGED + DEPLOYED(2026-06-25, main ea27656)**: `writer.ts` `extractIssueDebates`(pure deterministic)가 옛 stub/raw-json 대체 — Core 11에서 쟁점 3~5 + label 인용(찬/반/중). plan R1~R3 + impl R1~R2 omxy + Claude 3-lens review 수렴, B-PARTB 29 tests. **기존 30 리포트 = ₩0 in-place 백필 APPLIED ✅(2026-06-25, AI 0원, DB partD.one_line+committee_votes.argument_excerpt 재구성→partB UPDATE, 결함 인용 117→0, 30/30 clean, backup scripts/out/...)**. **FF-merge → main ea27656 + Vercel prod deploy 완료(omxy merge 교차검증 CONVERGED, canary 4/4 200) → production FE에서 찬/반/중(arbiter) 전부 live.** **섹터 lens provenance(FE-only, ₩0) ✅ 완료·배포(2026-06-25, feat a2a1269; main HEAD = runtime verify)**: Part A 헤더에 `SECTOR_LENS_SUMMARY`(14 sector 핵심판단 ≤30자) 기반 "AI 섹터 관점 · {sector}: {lens}" + 면책 1줄(isCanonicalSector 가드) — Kevin 섹터 페르소나 provenance 가시화(저장 데이터·재생성 무변경). Claude↔omxy 토론→workflow 드래프트→omxy 직접수정→Claude review-purpose subagent 적대검토(13/14 grounded·invented 0)→omxy CONVERGED. spec `docs/superpowers/specs/2026-06-25-section8-partB-issue-extraction.md` + 선택 후속 **D-1 dangling action cleanup** + **완결성 체크 갭 fix**(orchestrator 완결성 체크가 Core-only라 섹터보드 실패 시 reset+rerun이 섹터보드 미복구 — worker 완결성에 섹터보드 포함 권장; 이번엔 336570 targeted re-commit으로 해소). 마이그/생성/live-catch(0041 CHECK fix)·완결성 갭 상세 = memory [[feedback_mocked_rpc_hides_check_violation]]. SoT = `docs/superpowers/specs/2026-06-23-tier2-sector-persona-report-wiring.md`.
...
#    다음 launch next = ⓑ go-live USER gates + S7b. (PR-T2c FE 렌더 ✅ / B-PARTB 코드 ✅ + ₩0 백필 APPLIED ✅ + MERGED→main ea27656 + Vercel prod DEPLOYED ✅ 2026-06-25[중: arbiter render live] / 완결성 체크 갭 = pre-launch lane.)
# ⚠️ 2026-06-25: `tier0-bpp-multiregime`(72 commits)를 main에 FF-merge(ea27656) + Vercel prod 배포. 이로써 캠페인 전체(B++ funnel/shadow PR-A·B/PR-T2a/B-PARTB)가 main에 통합됨 — 기존 본 문서/CLAUDE.md의 'main 미머지' 표기는 다수 stale. deploy된 비-B-PARTB 코드는 전부 flag-gated/dormant(Vercel env flag OFF)·migration 0038/0039 미적용 — LIVE/DORMANT/RESEARCH 분류 = `docs/superpowers/2026-06-25-main-merge-classification.md`(omxy R1→R2 CONVERGED). live SoT('main 미머지'→merged) 정합 = HANDOFF/ProgressDashboard/CodebaseStatus 본 세션 갱신 완료; 잔여 'main 미머지'는 과거 차수 historical 로그(현재 claim 아님).
#    B++ = retrieval 개선 diagnostic funnel(NO-CONFIG-PASSES 유지·예측 claim 금지).
git checkout main 2>/dev/null && git pull --ff-only origin main  # 운영 기준 = origin/main (feature/docs branch 작업 시 별도 분기)
git status --short                  # clean 기대 (scripts/.venv·scripts/out·/out/ gitignored)
git rev-parse --short origin/main   # 현 main HEAD runtime verify; B-PARTB stable merge point = ea27656 + Vercel 배포
...
| 영역 | 상태 |
|---|---|
| main HEAD | **runtime verify** `git rev-parse --short origin/main` (moving HEAD — SHA pin 금지). **B-PARTB stable merge point = `ea27656`**. **2026-06-25: `tier0-bpp-multiregime`(72 commits = B++ funnel/shadow PR-A·B/PR-T2a/B-PARTB) FF-merged → main + Vercel prod 배포(canary 4/4 200, OPEN PR 0)** — 비-B-PARTB 코드는 flag-gated/dormant(분류 = main-merge-classification.md). (직전 285339a 77차 PR #122.) |
| OPEN PRs | **없음(0)** 기대. PR #19~#122 전부 머지(상세 git log). |
| 검증 게이트 | build OK / lint 0 err 0 warn / **test:ci 2345 PASS + 5 skipped**(+PR-K reflection metrics/context/ledger/flags/source/orchestrator/prices/summary-prompt/summarizer/admin-reflection/reflection-job route + render append + selection-worker PR-K seam) / tsc clean / **PG smoke 0043(reflection_log) + 0042** / DART pytest 18 + screen/shadow python unittest 189. |
| **PR-K Reflection 자가학습 (2026-06-27, D32)** | **✅ shadow-first 빌드·dormant**. `lib/reflection/*` pure core + DI orchestrator. 직전 finalize 사이클 평가군 실현 수익률(KRX·무비용) → 페르소나 적중률·가중수익 → reflection_log → 선정 W1 패널 주입(**신규 `reflectionLearningContext` 필드** — D27 Q5 `reflectionContext` per-ticker와 별개·별개 supplementary block). **shadow-first**: `REFLECTION_ENABLED` off → 선정 byte-identical·회고 미실행·mutation 0. fail-soft(prior cycle/KRX 미가동 no-op). 회고지 예측 아님(reflection_kind CHECK + summarizer 예측 출력 필터). LLM 요약 별 flag + fail-closed(cost gate + atomic claim). 마이그 **0043 DORMANT(USER apply)** + cron `/api/cron/reflection-job`(vercel.json 무변경). Claude 5-lens(4 HIGH) + omxy R1(7 fix) CONVERGED. SoT spec 2026-06-27-reflection-prk-build. USER 활성 게이트=§3. |
| **G4 거시/뉴스 AI 컨텍스트 (2026-06-26, D33 §4)** | **✅ 완료·dormant**. `lib/macro/{context.ts(pure distill)+source.ts(flag+stale fail-safe)}` → 리포트 writer(`report-input-enricher` macroSummary)·모닝 브리핑(M11 compose)·**LIVE Tier1 선정 패널(selection-worker→makeCallPersonaPanel/makeCallDebatePanel→callPersona)** 3-consumer. 단일 flag `MACRO_CONTEXT_ENABLED`(default off → byte-identical·선정 무회귀). 가드레일 코드 박제: Tier0/funnel import allowlist 테스트·M12a 범주 분리·forward-validate('예측 아님·Tier0 스크리닝 팩터 아님')·stale-mock fail-safe. **M11 Resend 제거**(Telegram+dashboard). **완료 범위 = mock-macro distill seam + wiring; 실 FRED/news source + Polymarket(선택) = USER 게이트/후속.** USER 활성 게이트 = `MACRO_CONTEXT_ENABLED`+실 거시 source. SoT spec 2026-06-26-g4-…. |
| **S7b M12a 뉴스 자동제외 (2026-06-27)** | **✅ shadow-first 빌드·dormant**. `lib/news/m12a/*`(verdict/brake/cashout/ledger/negative-news-context/telegram-text/evaluator/orchestrator/briefing-integration/auto-remove/run-result + config/flags/types). Core 11 뉴스평가→per-company thesis-break 구조화 판정→결정론 verdict→smart brake(per-company mass>3·track 70% floor·집중포트)→durable ledger(`news_event` 1 + 신규 `m12a_ticker_assessment` N, action_taken 3-layer 귀속)→shadow/mutate 게이트→텔레그램+`/admin` durable alert(news_critical/news_warning 재사용·enum 불변·이메일 0). **shadow-first**: `M12A_AUTO_REMOVE_ENABLED` off→mutation 0; `M12A_NEWS_EVAL_ENABLED` off→morning-briefing byte-identical. 자동 mutation=출시 후 fast-follow(throw-stub). 마이그 **0042 DORMANT(USER apply)** + PG smoke 10 assertions. go-live fail-closed step-0(AI_COST_LOG+CRON_SYSTEM_USER_ID UUID/getUserById, sibling parity). omxy R1~R3 + Claude 5-dim deep-review CONVERGED(6 fix). SoT spec 2026-06-26-m12a-…. USER 활성 게이트=§3. M12a 자동 mutation=출시 게이트 아님. |
...
| **풀 리포트 (production)** | `stock_reports` 2026-06 **30행 = B++ 새 30 (Tier2 포함, 2026-06-24 재생성·검증)** — section_0~8+appendix + committee_votes 330 core + 420 sector = 750 + partA 14 + partC.sector_aggregate + NULL-sector 0. **PR-T2a(Tier2 배선) ✅ CONVERGED + 마이그 0040/0041 applied**. **PR-T2c FE 렌더 ✅ 검증**(Part A 14 + sector_aggregate 렌더 OK, 30/30 modern-parse, fallback 0). **B-PARTB ✅ 완료 + MERGED→main + DEPLOYED**: Part B 쟁점 실제 추출(writer.ts `extractIssueDebates`, CONVERGED 2026-06-25, B-PARTB merge `ea27656`, 현 main HEAD = runtime verify `git rev-parse --short origin/main`, canary 4/4 200) + **기존 30 리포트 ₩0 in-place 백필 APPLIED ✅(AI 0원, 결함 인용 117→0, 30/30 clean, backup scripts/out/...)**. 완결성 체크 갭(pre-launch). report_batch_job 30 done. 2026-05-01 004150 1행(P1 잔존). |
...
| **Step 7 S7b** 뉴스 자동 제외(M12a) + 모닝 브리핑(M11) + **G4 거시/뉴스 AI 컨텍스트** | USER(Naver B-8 + Telegram B-9 + AI 키 + `MACRO_CONTEXT_ENABLED`+실 거시 source) + CLAUDE | B++ G1/G2 + go-live USER gates 완료 후 | **G4 ✅ 완료(2026-06-26, dormant·D33 §4)**: `lib/macro` distill → 리포트 writer/모닝 브리핑/**LIVE Tier1 선정 패널(R1/R2)** 컨텍스트 입력(`MACRO_CONTEXT_ENABLED` off→byte-identical). Tier0 factor 금지(allowlist 테스트 박제)·M12a 범주 분리·forward-validate·stale fail-safe. **M11 모닝 브리핑 Resend 제거**(Telegram+dashboard). omxy R1+R2 CONVERGED + Claude deep-review(live-path HIGH catch)·게이트 green. SoT `docs/superpowers/specs/2026-06-26-g4-macro-news-ai-context-layer-design.md`. **M12a AI-persona 뉴스 자동제외 ✅ shadow-first 빌드 완료(dormant, 2026-06-27)**: `lib/news/m12a/*` — Core 11 뉴스 평가 → per-company thesis-break 구조화 판정 → 결정론 verdict → smart brake(per-company mass>3/track 70% floor/집중포트) → durable ledger(`news_event` 1 + 신규 `m12a_ticker_assessment` N) → shadow/mutate 게이트 → 텔레그램/`/admin` durable alert. **shadow-first**(`M12A_AUTO_REMOVE_ENABLED` default false → mutation 0; `M12A_NEWS_EVAL_ENABLED` default false → morning-briefing byte-identical). 자동 제거 ON은 출시 후 fast-follow(throw-stub). 마이그 0042 DORMANT(USER apply) + PG smoke 10 assertions. go-live fail-closed step-0(AI_COST_LOG+CRON_SYSTEM_USER_ID). omxy R1~R3 + Claude deep-review CONVERGED·게이트 green. **M12a 자동 mutation = 출시 게이트 아님.** spec SoT = `ServicePlan-Admin §3.10 M12a` + `docs/superpowers/specs/2026-06-26-m12a-news-auto-remove-shadow-first.md`. 이메일/Resend 전역 미사용. **USER 활성 게이트 = M12A_NEWS_EVAL_ENABLED+AI_COST_LOG_REAL_INSERT_ENABLED+CRON_SYSTEM_USER_ID+0042 apply+Naver/Telegram/AI 키**(§3). **디자인 결합**: D3 신규 화면 final-style 동시구현, D1은 S7b UI 착수 전 필수. |
...
| **PR-K Reflection 자가학습 빌드 (D32 — 출시 전 승격) ✅ 완료(dormant, 2026-06-27)** | CLAUDE 빌드 ✅ + USER(flag/키/비용 = 활성화) | 빌드 완료 · S9 기간 중 실가동·검증 | **✅ 빌드 완료(dormant)**: 마이그 0043 `reflection_log`(DORMANT, USER apply) + track별(주1/월1, 65차 Q1) 회고 job(cron `/api/cron/reflection-job`) + 직전 사이클 평가군 실현 수익률(KRX·무비용) → 페르소나 적중률·가중수익 → **신규 `reflectionLearningContext` seam**으로 선정 패널 주입(off→byte-identical·**Q5 `reflectionContext` per-ticker와 별개 필드**). 회고지 예측 아님(retrospective CHECK + summarizer 예측 출력 필터). LLM 케이스 요약 = 별 flag + fail-closed(cost gate + atomic claim). 데이터 의존 → S9 1개월+ 운용으로 (단기/주간 트랙) 첫 회고 사이클 검증(중장기 월간은 S9 길이·첫 선정일 의존·no-op fail-soft). **빌드 완료 = S9 진입 선행조건(sequencing) 충족; Reflection 동작·품질 = soft launch criterion(§2.2 7-criteria 미변경, hard-block 아님)**. Claude 5-lens + omxy R1 CONVERGED. ⚠️ scheduling 비대칭(read=selection-worker cron / write=reflection-job 미스케줄) → flag flip + reflection-job schedule 페어 액션(§3). SoT spec = `docs/superpowers/specs/2026-06-27-reflection-prk-build.md`(빌드) + `2026-06-24-…-pre-launch-promotion.md`(D32). |
| **TradingAgents graft 출시 전 lane (G1~G4, D33 — G5 reject)** | CLAUDE 빌드 + USER(G1/G3 AI 키·비용 승인) | G1/G3 최소 빌드 = S7d 이후~S9 진입 전(G3는 D11 라이브 포트 후; G1/PR-K 계측 scaffold는 D11 직후 병렬 착수) · G2 = 섹터 비교 메뉴 내 · G4 = S7b 진입 시 | **per-ticker 토론 funnel 이식 금지(oversight/Reflection 패턴만)·NO-CONFIG-PASSES 유지(예측 claim 영구 금지)**. **G1** Tier0 Reflection Lab = 150/30 postmortem → B++ 가중치 champion/challenger **제안만→USER 승인**(diagnostic·자동 적용 금지·PR-K[D32]와 다른 층). **G2** Leader-miss why-excluded = **기존 Path-A Track-2 + `shadow_reconcile.py` 흡수**(신규 빌드 금지·hard-gate live 영구 금지·₩0). **G3** Risk 3자(공격/보존/중립) = Accept 전 포트 구성당 1회 위험 재판정 layer(포트 구성당 1회 shadow/advisory로 cost cap·**USER 출시후→출시전 이동**·Accept 게이트 substitute 아님·비용 중). **G4** 거시/뉴스/예측시장 = Tier1/리포트/브리핑 **컨텍스트 입력**(Tier0 factor 아님·M12a와 범주 분리·S7b 결합·₩0) — **§4 1차 구현 ✅ 2026-06-26·dormant**(mock-macro distill seam + 3-consumer wiring; 실 FRED source + Polymarket 후속). G1/G3 빌드 = sequencing deliverable, 동작·품질 = soft criterion(**§2.2 7-criteria 미변경**). **계측-먼저/완성-늦게**: G1/PR-K 계측 scaffold는 D11 직후 병렬 착수(S9 첫날 데이터 확보)·KIS 지연과 직렬 무관(병렬 lane). **Step 9(섹터메뉴+G1+G3+PR-K+D4) 병렬 마감으로 과밀 분산**(디자인 freeze·QA 병목 방지). **omxy 순서 교차검증 CONVERGED(2026-06-25)**. SoT = `docs/superpowers/specs/2026-06-25-tradingagents-graft-prelaunch-roadmap.md`. |
| **Step 15 S9 운용 → 🎉 출시** | USER 1개월+ + CLAUDE hotfix | Pre-launch 섹터 비교 메뉴 + Tier2 배선 + D4 freeze + PR-K Reflection 빌드 + D33 G1/G3 빌드 완료 후 | 어드민 3인 실 사용 1개월+ + 위 7 criteria 통과 = 출시 (★ S9 기간 중 PR-K Reflection + D33 G1/G3 동작·품질 soft 관찰). **디자인 결합**: D4 freeze(S7d 후 · S9 직전) 완료 후 진입 — 풀 리디자인 아님, `/gstack-design-review` QA + polish + 회귀 차단만. |
...
| **G4 거시 컨텍스트 활성** | Vercel `MACRO_CONTEXT_ENABLED=true` + 실 거시 source 연결(현 mock-macro·stale fail-safe로 flag만으론 ""→dormant) | G4 컨텍스트를 리포트/브리핑/Tier1 프롬프트에 실제 주입 시 (코드 ✅ 완료·flag off) |
| **M12a go-live 게이트** (shadow-first 빌드 ✅ dormant) | Vercel `M12A_NEWS_EVAL_ENABLED=true` + `AI_COST_LOG_REAL_INSERT_ENABLED=true` + `CRON_SYSTEM_USER_ID`(유효 UUID·auth.users 존재) + 마이그 **0042 apply**(`m12a_ticker_assessment`) + AI 비용 승인. (shadow 검증 후에만 `M12A_AUTO_REMOVE_ENABLED=true` — 출시 후 fast-follow.) 코드 ✅(flag off→byte-identical·미적용 안전). | USER (활성화 시 morning-briefing이 M12a shadow 평가 실행) |
| **PR-K Reflection go-live 게이트** (shadow-first 빌드 ✅ dormant, D32) | Vercel `REFLECTION_ENABLED=true` + 마이그 **0043 apply**(`reflection_log`) + `SELECTION_CRON_AUTO_ENABLED=true`(선정 가동) + `KRX_OPENAPI_KEY`(실현 수익률) + **reflection-job schedule 추가**(vercel.json 또는 외부 — write seam은 미스케줄, flag만 켜면 read가 빈 결과로 조용히 무동작이라 flag flip과 **페어**) + (선택)`REFLECTION_LLM_SUMMARY_ENABLED=true`+`AI_COST_LOG_REAL_INSERT_ENABLED`+`CRON_SYSTEM_USER_ID`+AI 비용 승인. 코드 ✅(flag off→byte-identical·미적용 안전). | USER (활성화 시 reflection-job이 회고 기록→다음 선정 prompt 주입) |
...
| **S7b M12a 뉴스 자동제외 shadow-first 빌드 spec (2026-06-27)** | `docs/superpowers/specs/2026-06-26-m12a-news-auto-remove-shadow-first.md` (모듈 레이아웃·verdict/brake/ledger/cashout 결정론·마이그 0042·shadow-first 게이트·가드레일 체크리스트) |
...
| **AI 자가 학습 Reflection / PR-K (D32) — 결정** | `docs/superpowers/specs/2026-06-24-reflection-prk-pre-launch-promotion.md` |
| **PR-K Reflection shadow-first 빌드 spec (2026-06-27, ✅ 완료·dormant)** | `docs/superpowers/specs/2026-06-27-reflection-prk-build.md` (모듈 레이아웃·metrics/context 결정론·마이그 0043·LLM fail-closed/atomic claim/예측 필터·§9 USER 게이트·§10 omxy R1 as-built) |
| **TradingAgents graft pre-launch 로드맵 (D33 — G1~G4 채택 + G5 reject)** | `docs/superpowers/specs/2026-06-25-tradingagents-graft-prelaunch-roadmap.md` |
