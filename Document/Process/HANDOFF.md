...
### 빌드 순서 = W0 ✅ → W2a ✅ → W2b ✅ → W1a ✅ → W1b ✅ → W3a ✅ → **W3b-1 ✅** → W3b-2 → W3b-3  (**69차: W3b-1(portfolio_proposal AI 클라이언트 + proposePortfolio 플러밍) MERGED PR #97, main `6b118bc` — 다음 = W3b-2(portfolio_proposal DB 영속 + Accept 통합)**)
...
| 영역 | 상태 |
|---|---|
| main HEAD | **runtime verify** `git rev-parse --short origin/main` (2026-06-05 68차 W1b PR #94+#95 후 `30107cc` 또는 자손). |
...
| 검증 게이트 (main, 2026-06-05 68차 PR #95 후 `30107cc`) | build 26 routes / lint 0 err 0 warn / **test:ci 1792 PASS + 2 skipped / 150+1 files** / tsc clean / **Python 95 tests PASS**. |
...
> ⚠️ **본 §6은 과거 세션 완료 기록(history).** 각 entry의 "다음: PR-G ⓑ …" 표현은 **해당 차수 당시 기준**이며, **현재 1순위는 상단 ⭐ MVP 엔진 W0✅→W2a✅→W2b✅→W1a✅→W1b✅→W3a✅→W3b 중 W3b** (PR-G ⓑ는 W0~W3로 superseded·코드 자산 재사용). 과거 행은 역사로 보존하며 개별 정정하지 않는다.

### W3a — entry_price 실배선 (KRX EOD 종가) (68차, 2026-06-05, PR #96 + 배선 LOW fix MERGED main `b9073a7`)
...
- **게이트**: build 26 routes / lint 0·0 / **test:ci 1812 PASS + 2 skipped / 152+1 files** / tsc clean / 마이그·admin-approvals·admin-snapshots·calendar diff 0 / resolveRealEntryPrice grep 0. behavior-neutral(flag-off=현 동작 1:1). canary 4/4.
- **다음**: **W3b(Q2 portfolio_proposal AI 자율 구성)** — 편입/총개수/단·중·장 분배/비중/현금 0~30% AI 판단(`portfolio` role Opus). 실 AI·USER 게이트. **USER 게이트 += `PORTFOLIO_REAL_ENTRY_PRICE_ENABLED=true` + `KRX_OPENAPI_KEY`**(실 Accept 가동 시).
