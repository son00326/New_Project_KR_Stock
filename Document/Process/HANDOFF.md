...
- **Mock cleanup**: Step 1 ✅ MERGED `1d2db08` (PR #32 5 commits, omxy R1~R4 CONVERGED) + **Step 2.1 OPEN PR #33** (admin alerts 3 routes → real `alert_event`/`news_event` SELECT, 3 commits `29222ab`+`e210b02`+`c498276` docs R3 sweep, omxy R1~R2 CONVERGED) + **Step 2.2 OPEN PR #34** (admin settings → real `admin_settings`/`ticker_alert_pref` SELECT + WRITE boundary, 1 commit `43c7886`, omxy R1 CONVERGED, 신규 helper 2종 + tests +24)
...
2. **58차 OPEN (사용자 머지 대기)**:
   - **PR #33** Mock cleanup Step 2.1 alerts (`MOCK_ADMIN_ALERTS` + `MOCK_ADMIN_NEWS` → `alert_event` + `news_event` 실 DB SELECT, 5 commits `29222ab` + `e210b02` + `0dcc83c` + `c498276` + `c2ee4b8` docs sweeps R3/R5, omxy R-debate 2 rounds CONVERGED + docs sweep R-debate R2~R6 CONTINUE catch 12건 → R7 fix 진행 중)
...
**운영 원칙**: 미래 지향. §6 완료 이력 = 직전 2 entry inline (58차 Mock cleanup Step 2.1 OPEN PR #33 + 58차 Mock cleanup Step 1 MERGED baseline)만. older historical = git log + spec/plan/REVIEW docs + ProgressDashboard 위임.
...
# OPEN: PR #33 (Step 2.1 alerts, READY, branch `feat/mock-cleanup-step-2-1-alerts` 5 commits `29222ab`+`e210b02`+`0dcc83c`+`c498276`+`c2ee4b8` docs sweep R3/R5) + PR #34 (Step 2.2 settings, READY, branch `feat/mock-cleanup-step-2-2-settings` 1 commit `43c7886`, omxy R1 CONVERGED).
...
| OPEN PRs | **#2** (format-error, CONFLICTING 보류) + **#33** (Step 2.1 alerts, READY, 3 commits `29222ab`+`e210b02`+`c498276` docs sweep + Vercel preview SUCCESS) + **#34** (Step 2.2 settings, omxy R1 CONVERGED READY, 1 commit `43c7886` + Vercel preview SUCCESS) — 58차 누적 MERGED: PR #30 (`3c09d6e`) + PR #31 (`838386e`) + 마이그 0025 production applied + PR #32 (`1d2db08`) |
...
| 검증 게이트 (PR #33 branch baseline, docs sweep R3 적용) | build 25 routes / lint 0 err 6 warn (pre-existing) / **test:ci 1186 PASS / 109 files** (+37 over 1149 main baseline) / tsc clean / 0 migrations (sweep docs only). PR #34 branch baseline: test:ci 1173 PASS / 109 files (+24 net new). |
...
- **PR #33** Step 2.1 alerts (admin user-visible 3 routes `MOCK_ADMIN_ALERTS` + `MOCK_ADMIN_NEWS` → `alert_event` / `news_event` 실 DB SELECT, omxy R1~R2 CONVERGED, 3 commits `29222ab`+`e210b02`+`c498276` docs sweep R3)
