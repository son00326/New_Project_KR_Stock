...
>
> **▶ 지금 당장 (owner별):**
> - **[진행 중 · CLAUDE 자율 · 비용 0] B+C full-factor 다중장세 4-config 검증 (USER가 option (a) 선택)**: B++ step-2 verdict(triple-gate FAIL, 아래 박제) 이후 **USER가 "(a) full-factor 깨끗한 verdict"를 선택** → 4 config(① trend+size ② +foreign ③ +earnings ④ +both)를 **다중 장세**(2022 약세·2023 회복·2024-25 강세)에서 검증해 어느 config가 150 깔때기를 가장 잘 만드는지 통계적으로 판정. 동결 결정규칙 = `docs/superpowers/tier0-4config-decision-rules.md` + `TestFrozenDecisionRules`(git-tag `tier0-multiregime-freeze`, run 후 임계 수정=p-hacking 금지). **현 상태**: branch `tier0-bpp-multiregime`. ① earnings PIT backfill 스크립트(`scripts/dart_backfill.py`) omxy BC-R1~R4 CONVERGED(`25833bd`, scripts 267 tests OK, omxy 2 code-review agents 0 issues). **DART PIT 백필 day-1 가동 중**(1772 corps × 25 periods = 44,300 fetches, ~2.3-4.7일 quota-bound[DART 무료 2만/일], `--max-calls 19000 --resume`, **production Supabase 미접촉·로컬 `scripts/out/dart_backfill.jsonl`만**). **resume 명령**: `scripts/.venv/bin/python scripts/dart_backfill.py --start-month 2022-01-01 --end-month 2025-12-01 --cache-dir scripts/out/pit_cache --out scripts/out/dart_backfill.jsonl --max-calls 19000 --resume` (익일 quota 리셋 후 반복, done 44,300까지). **다음**: 백필 완료 → configs 3·4 harvest(`--earnings --dart-backfill-path scripts/out/dart_backfill.jsonl` [+`--with-foreign`]) × 3 regime → 4-config×3-regime 비교 + 동결 결정규칙 적용 → omxy 동일 프로세스 검토 → docs verdict 갱신.
> - **[박제/참고]** **Tier0 스코어링 B++ step-2 harvest = ✅ 실행 완료 → triple-gate FAIL → Claude↔omxy CONVERGED → harness MERGED(PR #122, main `285339a`).** 실 19개월 PIT run(2024-06~2025-12, 순수 trend+size, 비용 0): **Gate A FAIL**(overall recall 0.108<0.20 · 단 **B++ 0.108>baseline 0.107**[복잡도 정당화] · largemid **0.431** · leaders **138/209≈7/11/mo**[73차 1/11]) · **Gate B ADJUDICATE**(IC IR 0.260<0.30 · large-IC **+0.08** · B++≫baseline −0.05) · **Gate C PASS**(60/60/30 19개월). **결론(합의)**: B++는 대형 주도주 retrieval 개선 실증 + naive baseline 상회하나, earnings/foreign 부재(순수 trend+size) + 절대 임계 미달 → **full 예측 thesis 미검증 = "leader-inclusive diagnostic candidate generator"까지만, --apply/Tier1/"상승 예측" claim 전부 금지.** (← USER가 위 (a) full-factor 검증 선택으로 진행 중.) [SoT: spec §5/§6 + review doc] — §6 상세.
...
> - 트랙: 메인 런북 #1~4 · **shortlist 정확성(현재 = B++ step-2 harvest 실행 완료 → triple-gate FAIL → diagnostic generator·USER 결정 대기)** · 토스 D0~D4. 메인 런북이 출시 critical path.
...
git rev-parse --short HEAD          # 2026-06-12 77차: PR #122(B++ step-2 harvest) merge `285339a` 후 자손 (runtime verify)
...
| **스코어링 방법론** | B++ step-2 harvest **실행 완료**(PR #122, 77차 후속): 실 19개월 PIT(순수 trend+size) → **triple-gate FAIL**(Gate A 0.108<0.20 단 B++>baseline + largemid 0.431 + leaders 7/11 · Gate B IC IR 0.26<0.30 large-IC +0.08 · Gate C PASS). Claude↔omxy CONVERGED. **B++ = 대형 retrieval 개선 실증 + baseline 상회하나 절대 예측 임계 미달 → diagnostic leader-inclusive generator, --apply/Tier1/"상승 예측" claim 금지** — "robust, factor-informed, leader-inclusive candidate shortlist"까지만. full-factor verdict = USER-gated DART rcept_dt+foreign backfill rerun. SoT spec §5/§6. |
...
| ✅→🤔 **Tier0 스코어링 B++ step-2 harvest — 실행 완료(PR #122) → triple-gate FAIL → USER 결정 대기** | harness + 실 19개월 PIT harvest ✅ MERGED, Claude↔omxy CONVERGED. **verdict = FAIL**(B++ baseline 상회 + 대형 retrieval 개선 실증하나 절대 예측 임계 미달, earnings/foreign 부재) → diagnostic generator, **--apply/Tier1/"상승 예측" 전부 금지·미실행**. **USER 결정**: (a) full-factor 깨끗한 verdict 원하면 DART rcept_dt 스키마+backfill + foreign backfill → unchanged 게이트 rerun(스키마 변경+비용 = USER-gated), 또는 (b) B++ diagnostic generator 유지. **둘 다 USER 선택 전까지 추가 작업 없음.** | USER 결정 (backfill rerun vs keep-diagnostic) [SoT: spec §5/§6 + review doc] |
