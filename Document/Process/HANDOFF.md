...
| **B66 quality/trust blocker** | `short_list_30` 30 rows sector="코스닥"/"코스피" placeholder. **Approach lock-in = C 하이브리드**: DART `induty_code` → canonical 14 일반화 매퍼를 `scripts/screen_shortlist_tier0.py::fetch_universe` seed pipeline에 영구 심고, DART 모호/오분류 종목은 override fallback. 구현은 다음 세션 R-debate/TDD; production write는 USER Supabase re-auth/write gate. |
...
| **다음 세션 1순위** | ⭐ CLAUDE 작업 = **Task 5 B66 C 하이브리드 R-debate 진입**(DART `induty_code` 일반화 매퍼 + seed pipeline 영구 심기 + override fallback). PR #54 §5.3/canary는 USER 권장 잔여(비차단), Task 7 Smoke Stage 2 비용 승인은 필수 USER gate. Step 2.7b.3은 PR #54로 완료. |
...
| 검증 게이트 (code baseline `50cb94a`; docs-sync 자손 허용) | build 25 routes / lint 0 err 5 warn (pre-existing) / **test:ci 1317 PASS / 117 files** (PR #54 +21 net) / tsc clean. |
| omxy debate 상태 | 최신 PR #54 Step 2.7b.3까지 CONVERGED. HANDOFF에는 current 결론/W-ticket만 유지하고, 라운드 수·catch 수·fix commit 세부 chain은 git log + PR body + cmux transcript 위임. |
...
### §2.1 Step matrix (main docs-sync `cb43cb5` 또는 B66-docs 자손; code baseline PR #54 Step 2.7b.3 ✅ — **PR5 진입 = Task 5~7 PASS 후**)

**현재 위치 = 60차 PR #54 종료 post-merge baseline (code baseline PR #54 Step 2.7b.3 ✅ — cron INSERT mock cleanup Step 2 전체 완료)**:
...
**누적 박제**: 58차 PR #30~#34 + 마이그 0025 + 59차 PR #39~#51 + 60차 PR #53/#54 모두 MERGED, main docs-sync `cb43cb5` 또는 B66-docs 자손. 라운드/commit 상세는 git log + PR body 위임.

**다음 1순위** = **(CLAUDE) Task 5 B66 C 하이브리드 R-debate 진입** / **(USER 권장) PR #54 §5.3 production schema preflight + 인증 세션 canary** / **(USER 필수) Task 7 Smoke Stage 2 비용 승인**. B66 C 하이브리드 = DART `induty_code` 일반화 매퍼를 seed pipeline에 영구 심고 override fallback으로 2026-05 backfill + 미래 PR5 cron 월간 batch 모두 canonical 14 부여. production write는 USER Supabase re-auth/write gate. Mock cleanup Step 2.7b.2/2.7b.3은 완료. Vercel env는 이미 true; 실 AI trigger는 Task 7 승인 전 금지.
...
- **USER 잔여 액션 (omxy R6 caution)**: PR #54 §5.3 production schema preflight (Supabase re-auth 후 alert_event CHECK + briefing_log date UNIQUE 검증) 권장 잔여. schema 0006/0010 long-applied (구조 변경 0) → drift 위험 매우 낮음. PR #53 §5.3은 완료(drift 0).
...
- **HANDOFF.md 다음 세션 자동 진행 가능 조건**: §0 + §1 + §2 + §9 모두 stale 0. 현재 baseline = main docs-sync `cb43cb5` 또는 B66-docs 자손 / code-deploy `50cb94a` PR #54, OPEN PR #2 only, build/lint/test/tsc green, Vercel env true 완료, last-known production audit rows 0. **USER 잔여 액션 = PR #54 §5.3 preflight(권장) + 인증 세션 canary verify(권장) + Smoke Stage 2 시점 1회 비용 승인(Task 7)**. 다음 CLAUDE 작업 = Task 5 B66 C 하이브리드 R-debate/impl plan 진입(구현은 다음 세션, production write USER-gated).
...
**B66 결정 (2026-05-28, 구현 0 / 결정 박제 only)**: **C 하이브리드**. A(매월 수동 큐레이션 맵) 단독은 PR5 cron 월간 batch마다 재발하고, B(DART 단독)는 소형주 모호/오분류 risk가 있어 채택하지 않음. 다음 세션은 동일 R-debate workflow(plan writing-plans + native critic subagents → impl TDD → post-merge R7/R8 + Claude Trust 3-step)로 진입.
...
- **Primary**: DART `induty_code`(한국표준산업분류/KSIC) → canonical 14 crosswalk. `seed_dart_corp_codes.py`의 corp_code seed(2766 corp) 재활용 + DART company.json API로 30 ticker corp_code lookup. 현재 `induty_code`는 어디에도 fetch 안 되므로 다음 구현에서 신규 통합.
...
**B89 unknown ticker 처리 옵션 R-debate (다음 세션 확정)**:
- (i) **Block**: short_list_30 INSERT 차단 — 안전, 종목 수 감소 risk
- (ii) **Manual review / override**: override map 또는 review queue로 수동 매핑 — 운영 부하 있지만 품질 안정
- (iii) **Backfill exclude**: 매핑된 ticker만 UPDATE, 나머지는 명시 warn/unknown_pending — 점진적 보정

**Production write gate**: CLAUDE는 script + crosswalk + override map + tests + R-debate까지 수행. `short_list_30` production backfill apply는 USER Supabase re-auth + write 권한이 필요한 USER-gated 작업. 본 60차 문서 변경은 결정 박제 only, 코드/script/DB 변경 0.
