...
### 빌드 순서 = W0 ✅ → W2a ✅ → W2b ✅ → W1a ✅ → W1b → W3  (**68차: W1a(D28 mix + R2 반박) MERGED PR #92+#93 — 다음 = W1b(judge/dual-judge)**)
...
- ~~즉 **다음 세션 1순위 = W0**~~ **[68차: W0 ✅ → W2a ✅ → W2b ✅ → W1a ✅ MERGED(PR #92+#93) → 다음 세션 1순위 = W1b(judge/dual-judge)]**. 아래 §한눈에·§다음 액션 큐의 "PR-G ⓑ 1순위" 표현은 본 섹션으로 대체된다(코드/DB/게이트 현 상태 서술은 유효).
...
> ⚠️ **다음 세션 1순위 = 상단 ⭐ 65차 MVP 엔진 섹션 — W0 ✅ → W2a ✅ → W2b ✅ → W1a ✅ MERGED(68차 PR #92+#93) → W1b → W3.** 아래 "지금 어디 / 다음 액션 큐"의 **PR-G ⓑ 1순위 표현은 W0~W3로 superseded**(코드 자산은 토대 재사용). 현 게이트 수치 = 68차 기준(test:ci 1789+2skip, PR #93 후).
...
**main HEAD**: `git rev-parse --short origin/main` (2026-06-05 68차 W1a PR #92+#93 후 = `d032034` 또는 자손).
**OPEN PR**: **#2**(format-error, CONFLICTING 보류) only.
**검증 게이트**: build 26 routes / lint 0 err 0 warn / **test:ci 1789 PASS + 2 skipped(smoke 의도) / 150+1 files** / tsc clean / Python 95.
...
> ⚠️ **superseded by 상단 ⭐ 65차 MVP 엔진 섹션.** 68차 W1a ✅ MERGED(PR #92+#93) 이후 다음 세션 1순위 = **W1b** (빌드 순서 W0 ✅ → W2a ✅ → W2b ✅ → W1a ✅ → W1b → W3). 아래 PR-G ⓑ 큐는 코드 자산(청크워커/큐/run-mutex)을 W0~W3 토대로 재사용하는 형태로 흡수됨 — 역사 기록으로 보존.
...
#   main 기준 기대 (2026-06-05 68차 PR #93 후, main `d032034`): build 26 routes / lint 0 err 0 warn / test:ci 1789 PASS + 2 skipped(w0 smoke 의도) / 150+1 files / tsc clean / Python 95 tests PASS
...
> **[→ 65차 supersede · 68차 갱신] 다음 세션 1순위 = 상단 ⭐ MVP 엔진 W1b** (W0 ✅ → W2a ✅ → W2b ✅ → W1a ✅ MERGED PR #92+#93 · 구: §다음 액션 큐 1번 PR-G ⓑ — 코드 자산은 W0~W3 토대 재사용). PR-G ⓐ는 이미 ✅ MERGED(`d15da47`/PR #68). **KRX 공식 API 전환(PR #80) + 150 시드(`tier0_candidates_150` 150 rows) + 선정 청크 워커(PR #82, 구 0031 written-not-applied dormant)는 63차 ✅ 빌드 완료(67차 W2a에서 0031 재작성본으로 superseded).** ⓑ 잔여 = **[USER] `SELECTION_CRON_AUTO_ENABLED=true` + AI 키/비용 승인 + 주간 tier0 producer 자동화 → 워커 수동 트리거 ~55분 → 배지/cost_log/카드 검증** (동기 단발은 NON-VIABLE이라 청크 워커 단일화). 본 §0.A는 그와 **독립된 PR5(report-only cron) go-live** 트랙 — 혼동 금지. ⓑ가 ① 재시드/④ 비용으로 막히면 보고 후 다음 unblocked CLAUDE step으로 진행하고, PR5는 별도 USER-gate 트랙으로 유지한다.
...
| 영역 | 상태 |
|---|---|
| main HEAD | **runtime verify** `git rev-parse --short origin/main` (2026-06-05 68차 W1a PR #92+#93 후 `d032034` 또는 자손). |
...
| 검증 게이트 (main, 2026-06-05 68차 PR #93 후 `d032034`) | build 26 routes / lint 0 err 0 warn / **test:ci 1789 PASS + 2 skipped / 150+1 files** / tsc clean / **Python 95 tests PASS**. |
...
| **W1a (68차)** | ✅ **MERGED** (PR #92 rebase FF + 배선감사 fix PR #93, main `d032034`). D28 mix(Sonnet×6+GPT×5 interleave + per-slot binding + cost_log per-slot model) + R2 선택적 반박 라운드(computeR2Targets 경계±5∪분산>0 20% → round=2 jobs → makeCallDebatePanel → pickFinalPanels R2 우선) + **마이그 0032 dormant**(USER apply 게이트) + transient retry classifier + worst-slot reservation. 0031 RPC·reader·consensus·W2b 모듈 무변경. omxy plan R6 + impl R7 + blind R8 CONVERGED. **다음 = W1b(judge/dual-judge).** |
...
### §2.1 Step matrix — 8-row (legacy B65/B66/PR5 Task 구조 — 역사 기록. **현 1순위 = 상단 ⭐ MVP 엔진 W1b** [W0 ✅ → W2a ✅ → W2b ✅ → W1a ✅ MERGED · 구: PR-G])

**현재 위치** = **실데이터+실AI e2e 트랙** (PR-A~F + PR-G ⓐ MERGED + 마이그 0028~0030 applied). **[→ 65차 supersede · 68차: W0 ✅ → W2a ✅ → W2b ✅ → W1a ✅ MERGED] 현 1순위 = 상단 ⭐ MVP 엔진 W1b** (구: PR-G ⓑ 실 AI 첫 30선정 — 코드 자산은 W0~W3 토대 재사용). 아래 8-row Task matrix는 **legacy(B65/B66/PR5 Task 구조) 역사 기록** — 현 진행은 e2e 11-PR 로드맵(ADR §4)이 SoT. PR5 go-live는 별도 트랙(USER 게이트, dormant). **상세 박제는 git log + PR body + ADR/plan docs 위임.**
...
> ⚠️ **본 §6은 과거 세션 완료 기록(history).** 각 entry의 "다음: PR-G ⓑ …" 표현은 **해당 차수 당시 기준**이며, **현재 1순위는 상단 ⭐ MVP 엔진 W0✅→W2a✅→W2b✅→W1a✅→W1b→W3 중 W1b** (PR-G ⓑ는 W0~W3로 superseded·코드 자산 재사용). 과거 행은 역사로 보존하며 개별 정정하지 않는다.
...
- **다음**: **W1b** — D28 ③ debate_judge(Opus 4.8 per-ticker) + 경계 ±2 GPT 최고급 dual-judge(불일치 시 Opus 최종) + 단발 orchestrator/`upsertShortList30` 정리 + trackOfPeriod 처분 + calibration 보정. **USER 게이트 += 마이그 0032 apply**(R2 반박 라운드 가동 전 — 미적용 상태는 R1-only 동작이 아니라 `selection_round_schema_missing`으로 명시 중단).
