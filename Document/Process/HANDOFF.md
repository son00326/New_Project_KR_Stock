...
### 빌드 순서 = W0 ✅ → W2a ✅ → W2b ✅ → W1a ✅ → W1b ✅ → W3a ✅ → W3b-1 ✅ → **W3b-2a ✅** → W3b-2b → W3b-3  (**69차: W3b-2a(portfolio_proposal DB 영속 — dormant 마이그 0034 + upsert RPC + persist 게이트) MERGED PR #98, main `c1ee610` — 다음 = W3b-2b(Accept 통합, money-path)**)
...
**W3 — Q2 AI 자율 포트 구성** (분할: **W3a ✅**[entry_price KRX EOD, AI-key-free] / **W3b-1 ✅**[AI 제안 클라이언트+proposePortfolio 플러밍] / **W3b-2a ✅**[DB 영속, dormant 0034] / **W3b-2b = 다음 1순위**[Accept 통합, money-path] / **W3b-3**[admin UI])
...
- **W3b-2b (다음)**: Accept(buildInitialSnapshots)가 영속된 proposal의 positions/weight/cash로 snapshot 구성 — **money-path 통합**(getProposalByMonth consumer + stale 재검증 + cash row). **W3b-3**: admin UI 버튼·표시 + selection finalize 연결.
...
> ⚠️ **다음 세션 1순위 = W3b-2b(Accept가 영속 proposal의 positions/weight/cash로 snapshot 구성 — money-path).** 완료 경로 = 상단 ⭐ 65차 MVP 엔진 W0 ✅ → W2a ✅ → W2b ✅ → W1a ✅ → W1b ✅ → W3a ✅ → W3b-1 ✅ → W3b-2a ✅ MERGED(69차 PR #98). 아래 "지금 어디 / 다음 액션 큐"의 **PR-G ⓑ 1순위 표현은 W0~W3로 superseded**(코드 자산은 토대 재사용). 현 게이트 수치 = 69차 기준(test:ci 1866+2skip, W3b-2a 후).
...
| 영역 | 상태 |
|---|---|
| main HEAD | **runtime verify** `git rev-parse --short origin/main` (2026-06-05 69차 W3b-1 PR #97 docs-sync `89a4f18` 또는 자손). |
...
| 검증 게이트 (main, 2026-06-05 69차 W3b-1 PR #97 docs-sync `89a4f18`) | build 26 routes / lint 0 err 0 warn / **test:ci 1847 PASS + 2 skipped / 153+1 files** / tsc clean. |
...
> ⚠️ **본 §6은 과거 세션 완료 기록(history).** 각 entry의 "다음: PR-G ⓑ …" 표현은 **해당 차수 당시 기준**이며, **현재 1순위는 상단 ⭐ MVP 엔진 W0✅→W2a✅→W2b✅→W1a✅→W1b✅→W3a✅→W3b-1✅→W3b-2 중 W3b-2** (PR-G ⓑ는 W0~W3로 superseded·코드 자산 재사용). 과거 행은 역사로 보존하며 개별 정정하지 않는다.
