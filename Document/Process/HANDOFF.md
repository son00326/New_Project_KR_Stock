...
| **선정 흐름 (production)** | `short_list_30` 2026-06-01 = **30 AI 배지/ai_score**(🟣20/🟢7/🟡2/🔵1, 10/10/10) · 2026-05-01 = 30 Tier0 incumbents. **production 리스트는 여전히 73차 선정(2026-06-09)** — 76차 DART fix + 77차 재시드는 아직 production 미반영(Tier0 dry-run만, Tier1 재선정은 B+ 검증 후 보류). 메인 path = short 주간 + mid·long 월간 rolling. Tier1 재선정은 **B++ 삼중 게이트 통과 후 보류**(D30). **DART quarterly 캐시는 무효화+재populate됨**(fixed 파서). |
...
| 🔬 **Tier0 스코어링 B++ + 삼중 게이트 검증** (= shortlist 정확성 트랙 현 1순위, CLAUDE) | 2차 토론 CONVERGED(Claude 퀀트+omxy 2 독립, 실증 후 B+ REJECT→B++). 다음 세션: ①survivorship feasibility 선결 → ②B++ 구현(size sleeve+유동성 플로어+모멘텀 재설계+winsorize/rank+결측 tiering+foreign ADV+sector quality+rank ensemble) → ③recall+IC 하버스트(`validate_tier0_ic.py`) + 파라미터 train-lock-OOS → ④omxy 적대검토 → **삼중 게이트 ALL PASS 시에만** Tier0 재screen→apply→**Tier1(~₩25k, USER 승인)**. 재시드 dry-run 150은 구 스코어링이라 B++로 supersede. | CLAUDE 구현 → (Tier1 비용 USER 승인됨) [SoT: spec 2026-06-12 B++] |
...
| **Tier0 스코어링 B++ + 삼중 게이트 검증 (다음 세션 실행 스펙)** | **`docs/superpowers/specs/2026-06-12-tier0-scoring-bplus-validation.md`** (2차 토론 CONVERGED + B++ 설계[size sleeve·모멘텀 재설계·rank ensemble] + recall/IC/size 삼중 게이트 + survivorship 선결 + 재시드 상태) |
