...
9. **Pre-launch 섹터 비교 메뉴 + Tier2 리포트 배선 잔여(PR-T2c 렌더) + TradingAgents graft(G1~G4) + 디자인 freeze(S9/출시 전).**
   ...
   - **TradingAgents graft 출시 전 lane (D33, 2026-06-25 USER 확정 · Claude↔omxy CONVERGED · 코드 빌드 0·로드맵만)** — per-ticker 토론 funnel 직접 이식 금지(oversight/Reflection 패턴만 차용) + NO-CONFIG-PASSES 유지(예측 claim 영구 금지·자동 교체 없음):
     - **G1 Tier0 Reflection Lab** (신규 pre-launch 빌드 + S9 관찰): 과거 150/30 + 실현 수익률 postmortem → 다음 달 B++ funnel 가중치를 champion/challenger **제안만 → USER 승인**. **diagnostic only·자동 적용/예측 claim 금지·PR-K(D32)와 다른 층**(funnel 가중치 vs prompt 주입). 비용 낮음(데이터 DB 상주).
     ...
     - **G3 Risk 3자 토론(공격/보존/중립)** (신규 pre-launch 빌드 · **USER가 출시 후→출시 전 이동**): portfolio Accept 전 1회, 30선정+포트(Q2 자율)를 risk 3-debator로 위험도 재판정(통과/조건부/거절) **컨텍스트 layer**. **거래별 아님(포트 구성별 1회)·Accept 게이트 substitute 아님**. 비용 중.
