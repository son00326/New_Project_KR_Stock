...
| **3** | **CLAUDE** | Step 1 merge 전이면 stacked branch (1세션+ 작업이라 진입 의사 1회 확인 후 자동) | 진입 의사 1회 확인 → 사용자 OK 시 자동 진행 | stacked branch base = feat/s7a-anthropic-wrapper. Step 1 merge 후 rebase 필요 가능. push/PR create = §9 SHARED. merge는 USER. | Tier 2 Sector Board canonical 14 sectors × 14 personas overlay plan verbatim 구현 (D21 52차 정정) + tests + commit + push branch + PR create | Step 4 stacked |
