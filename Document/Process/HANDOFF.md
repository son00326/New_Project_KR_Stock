...
>
> **[섹터-우선 후속 · 2026-06-20 · Track1+Track2 스펙 + §12 빌드 파라미터 전부 Claude↔omxy CONVERGED(design-only) · 다음=PR-B1 직행]** 사용자 "layer 구성이 틀렸나 — 섹터 먼저 거르면?"(PRISM `dragon1086/prism-insight` 참조) → **경로 A forward-shadow 섹터 레이어 = Track 1 + Track 2 스펙 둘 다 Claude↔omxy CONVERGED**(Track 1: omxy R1→Claude R2→omxy R2→Claude R3→omxy CONVERGED; 연결포인트 검증 PASS·production 무접촉·코드 0·0038 미생성). **하드게이트(universe를 LLM섹터로 절단) production 직행 = REJECT**(R2 게이트오류 비가시성: 위원회·사람 전 절단 → 놓친 winner 안 보여 어드민 3인 안전망 무력화. 참고: PRISM도 universe 하드게이트 안 씀(부분슬롯+regime-off, 실측)). 대신 **2-트랙**: **Track 1**=Tier1 post-persist in-pool 30-reranking shadow(default-off·money-path 무결합·**150-recall 미측정 명시**) / **Track 2**=generator-shadow(Python Tier0 옆 sector-aware shadow 150 → `tier0_candidates_150_shadow`, forward로 shadow-150 vs production-150 recall 비교 = **유일하게 사용자 원질문[150-recall] 답**, 별도 spec). 백테스트 원천 불가→forward-only. 30=단/중/장10·B++150 불변. **USER 결정 = (c) 둘 다 빌드.** **Track 2 generator-shadow 스펙 = Claude↔omxy CONVERGED (2026-06-20, 본 세션)**: Claude dynamic-workflow 드래프트(5-ground+5-critique+revise) → omxy R1 direct-edit(6 fix, SIGNAL CONTINUE) → Claude 적대검토(5 code-grounded lens, 27 findings 전부 valid) 4 HIGH+MED fix → omxy R2 direct-edit(2 HIGH[finalize RPC run-level·hypothesis content-binding]+2 MED residual)+SIGNAL CONVERGED → Claude 코드근거 최종검증 clean. 연결포인트 PASS(B++ seam 실존[`run_bpp_candidates`→`build_stock_raw_list`/`select_bpp_candidates`/`build_bpp_candidate_rows`, `--apply` hard-block 유지·production write 구조적 미도달], canonical-14 Python==TS==SQL, finalize RPC atomicity[단일 `upsert_tier0_shadow_run` 1-tx·shared run_id·no-orphan], parity=selection-identity[ticker/bucket/rank+round score, sector 제외]). **핵심**: production-mirror(B++) vs sector-tilt/hard-gate arm forward paired recall. **2 metric 분리** = PRIMARY(arm-vs-mirror lift, period 1부터 forward-valid, production 무의존) / SECONDARY(vs LIVE persisted-150, **★ #1 B++ production 적용 후 + monthly grain만 유효**, 그 전엔 INCOMPLETE_RUN=정직·버그아님). hard-gate=diagnostic-only(full-universe snapshot으로 잘린 리더 가시화·R2 해소). design-only·production 무접촉·코드 0·migration 0039 미생성(0038=Track1 예약). **§12 빌드 파라미터 = Claude↔omxy CONVERGED (2026-06-20, 3 rounds · recommended default, USER override 가능) → 다음 세션 결정 단계 없이 PR-B1 직행 가능**: **D0** Track2 빌드는 ★#1에 블록 안 됨 — PR-B1~B3 즉시, 첫 eligible period부터 **PRIMARY(mirror+soft-tilt)-only 로깅 시작(★#1 무관)**, SECONDARY(vs persisted-150)만 ★#1 후 unlock(과결합 제거 — forward clock 안 버림). **D1** PR-B1~B3에서 mirror+soft-tilt+hard-gate 전부 구현; mirror-only plumbing은 **dry-run/same-period health check**(forward period 안 태움); **첫 persisted forward period부터 mirror+soft-tilt paired 동시 로깅이 default**; hard-gate는 gate_eligible_size/underfill 통과 시만 active(실패=그 arm만 diagnostic INCOMPLETE_RUN, soft-tilt 안 막음). **D2** hypothesis source = manual_pre_registered 먼저(LLM sector_advisor=별도 spec). **D3** counterfactual = 별도 `tier0_shadow_universe_snapshot`. **D4** forward-only 수용; **n_periods≥6은 real-hypothesis logged period만 카운트**(absent/plumbing 제외); manual hypothesis 첫 cutoff 전 미등록 → ESCALATE; PR-B3 후 ★#1 1 period라도 지연 시 **PRIMARY-only 자동 전환**; 첫 verdict 시도 = short~6주/midlong~6개월(USER 최종 확인). **D5** PIT-mechanical sanity probe 추가 안 함(look-ahead). **D6** unresolved = 단일 finalize RPC `unresolved_rows`. **D7** `soft_tilt_v1` 사전등록 default = **multiplier-only ×1.10**(leading-sector ticker B++ ordering score; PR-B1이 score 비음수 확인, ≤0면 동일효과 additive +10pp로 PR-B1에서 1회 확정·동결) + **cap ≤3 sectors** + **freeze horizon n≥6**(변경=새 tilt_version+clock reset) + immutable params jsonb(finalize RPC content-binding) + **첫 real non-mirror forward logging 전 USER 1회 sign-off**(미승인+non-mirror arm 요청=ESCALATE). → **다음 1순위 = PR-B1**(pure `compute_shadow_selections` + tests, production 무접촉). [SoT: Track2 `docs/superpowers/specs/2026-06-20-pathA-track2-generator-shadow.md`(961줄) + Track1 `docs/superpowers/specs/2026-06-19-pathA-forward-shadow-sector-layer.md` · memory `project_pathA_forward_shadow_sector_spec`] (B++ production 적용[★ #1]과 독립 레이어 — B++=funnel, 섹터레이어=그 위 판단/관측층; 단 Track2 SECONDARY metric은 ★#1 종속)
