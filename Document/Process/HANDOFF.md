...
1. **PR-G ⓑ 실 AI 첫 30선정** (다음 1순위 — ⓐ ✅ MERGED `d15da47`/PR #68. **63차(2026-06-02) 결정: ❶ 실행 = B 청크 워커**(Vercel cron, PR5 report-worker 패턴 복제 — 로컬 러너 폐기) **/ ❷ 데이터 = 하이브리드**(KRX 공식 Open API[시세·거래량·universe] + pykrx[S3 외국인] + DART[S4·S5])):
   ...
   - **다음 세션 절차**: (a) ✅ DONE `screen_shortlist_tier0.py` KRX 공식 API 전환(PR #80 MERGED main `30cb3b4`, 41 tests + e2e + FE 연결 CONVERGED) (b) **[USER] 150 시드**(`KRX_OPENAPI_KEY` env + `--emit-candidates`, KRX 공식 API 경로) (c) [CLAUDE] 선정 청크 워커 PR 빌드(§2.0a — mock+flag-off는 cost 0) (d) Anthropic 키+비용 승인 후 워커 수동 트리거 ~55분 (e) short_list_30 AI컬럼/cost_log/배지 검증 → 카드 실 배지 활성. **첫 검증·매달 자동 모두 동일 청크 워커로 단일화.**
...
- **다음**: [USER] 150 시드(`KRX_OPENAPI_KEY` env + `--emit-candidates`) → [CLAUDE] 선정 청크 워커 PR → PR-G ⓑ 실 AI 첫 30선정. **선택 follow-up**: `trend_score = momentum` 복제(pre-existing 데이터품질, 카드 T축=M축 동일, KRX 전환·연결 무관).
