...
status: ⚪ 대기 (🎉 출시 후 단독 진입 — 2026-06-01 재배치 · 구 D18 'S7d 후 단독 진입' supersede)
...
> **⚠️ 2026-06-01 진입 시점 재배치 (사용자 결정)**: S8 자동매매는 **🎉 출시 후** 개발한다. 출시 = 자동매매 없는 "AI 추천 + 가상 포트 + 알림" 도구(S7 + S9 운용 검증). 자동매매는 어드민 3인이 실운용하며 보면서 개발. 본 문서의 "S7d 후 단독 진입" · "4조건 중 3번째" 표현은 구 D18 기준이며, **현재 순서 = S7d → S9 운용 검증 → 출시 → S8**. SoT: `HANDOFF.md §2.2` + `CLAUDE.md` 진행순서.
...
S8은 "어드민 내부 도구 완성"의 마지막 단계 — **🎉 출시(자동매매 제외) 후** 추가한다 (2026-06-01 재배치). 순서 = Mock → 실데이터 → 운용 검증 → 출시 → **자동매매**.
...
- **(2026-06-01 현재 선행) = S7a~S7d 완료 + S9 어드민 1개월+ 운용 검증 + 🎉 출시 완료** (S8 자동매매는 출시 후 — 위 banner). 이하 (D18 기준) 설명은 출시 전까지의 선행 슬라이스 흐름:
...
### Phase S8-Scaffold (세션 1~2, **🎉 출시 후 단독 진입** — 2026-06-01 재배치 · 순서 = S7d → S9 운용 검증 → 출시 → S8)
