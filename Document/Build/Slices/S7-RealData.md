...
> **[2026-06-28] S7c·S7d ✅ 빌드 완료(dormant, shadow-first)** — branch `tier0-bpp-multiregime`. **S7c**: 장중 모니터(`lib/intraday/{flags,monitor,exit-outcome,run-monitor}`) + M15 Exit(`lib/notify/{exit-signal,run-exit-eval}`) + exit-dispatch 이메일 제거 + cron `exit-outcome`/`exit-signal`(dormant) + 마이그 0044/0045 + unread badge + recordExitDecision RPC 배선. **S7d**: silent-health 이메일/Resend 제거(72차 완결) + M19 heartbeat 카드. shadow-first flag off→byte-identical. USER 게이트(0044/0045 apply + INTRADAY_MONITOR/EXIT_SIGNAL/EXIT_OUTCOME_ENABLED + KIS/Telegram/KRX 키 + cron schedule) = HANDOFF §3. SoT = `docs/superpowers/specs/2026-06-27-s7c-s7d-alerts-monitoring.md`. 아래 S7c/S7d Phase Tasks는 당시 스켈레톤(빌드는 위 spec/git 기준).
>
> **[73차 현황 2026-06-09]** 본 슬라이스 파일은 history. **현 production: `short_list_30` 2026-06-01 = 실 AI 선정 30**(consensus_badge/ai_score populated, 🟣20/🟢7/🟡2/🔵1) — 풀 P3 실행 완료(PR #109 main `179d745`, cost_log 2026-06 2611행 ₩24,655.64, 양 run succeeded; 2026-05-01 30 Tier0 보존). 아래 53/54차 박제의 "53/54차 fallback·PR1 후속" 등은 **역사**(당시 기준). 라이브 SoT = `Document/Process/HANDOFF.md` ⭐ 73차 / §0.
>
> **[77차 D30 supersede 2026-06-12]** 아래 T7e.8의 Tier 0 스코어링(`scripts/screen_shortlist_tier0.py` 5-Signal z정규화 → 단/중/장 top50=150)은 **예측력 미검증 + 구조적 retrieval 실패**(실증: production 150 대형 주도주 10/11 누락·소형 급등주 독식·long 압축; 지속추세 시그널 부재 + 소형주 구성편향) → **"B++ (size-sleeve recall-first) + recall/IC/size 삼중 게이트" 채택(D30, B+ 단독 REJECT, 구현 다음 세션)**. T7e.8/T7a.7의 스코어링·가중치·정규화 기술은 **현 코드 상태(검증 전)** history이며, 다음 세션 재설계 SoT = `docs/superpowers/specs/2026-06-12-tier0-scoring-bplus-validation.md`(B++ 전문) + `ServicePlan-Admin.md §1A.5 D30`. **삼중 게이트 통과 전 후보 150을 "향후 상승 예측"으로 제시 금지.**
>
...
>
> **54차 §3 박제 (2026-05-22)**: **PR #12 (PR3a Group H stock_reports schema drift fix Critical Hard gate) MERGED in main `0813a41`** — admin-reports.ts zod validation + `ValidatedStockReport` + `parseSectionSafe`/`parseReportSection8` onError 콜백 + page.tsx null guard + Section 8 dual-shape renderer + `partCToCommitteeAgg` helper. omxy R1~R12 + R7 Codex `/review` GATE PASS + gsd + gstack testing + red-team 다중 review CONVERGED. **Group H Hard gate ✅ 해소**. canonical 5-PR: PR2 ✅ → PR3a ✅ → PR1 → PR3b → PR4. Step 3c caller wiring = PARTIAL — dangling server action (PR1 cron wire + PR4 UI wire에서 해소 예정). 당시 T7e.8은 Tier 0 기반 30 rows fallback 박제(현 73차 supersede). 메인 path = Tier 1 AI 30 선정(73차 PR #109에서 가동 완료). spec doc: `docs/superpowers/specs/2026-05-21-shortlist-report-flow-correction.md` + plan/REVIEW: `docs/superpowers/{plans,reviews}/2026-05-22-pr3a-group-h-schema-drift{,-review}.md`.

> **53차 §5 정정 박제 (2026-05-21, R6 추가 fix 반영, 54차 §3 PR3a로 Hard gate 해소됨)**: Step 3c caller wiring = **PARTIAL — dangling server action** (PR1+PR4에서 해소). T7e.8 박제 정정: 당시 Tier 0 기반 30 rows fallback(현 73차 supersede). 메인 path = Tier 1 AI 30 선정(73차 PR #109에서 가동 완료). canonical PR 순서 = PR2 → PR3a (Group H schema drift fix Hard gate) → PR1 → PR3b → PR4. omxy R1~R7 7 rounds CONVERGED-track **32 BLOCKERS catch & fix**. spec doc: `docs/superpowers/specs/2026-05-21-shortlist-report-flow-correction.md`.
...
### S7b · 뉴스 기반 자동 제외 (M12a) + 모닝 브리핑 (M11) + G4 거시/뉴스 AI 컨텍스트
> **(72차 재정의 + 2026-06-26 G4 + 2026-06-27 M12a)** S7b = (a) **뉴스 기반 자동 제외**(AI 페르소나, **✅ shadow-first 빌드·dormant 2026-06-27** — `lib/news/m12a/*` + 마이그 0042 DORMANT; flag off→byte-identical·mutation 0; 실 Naver/AI 호출=USER 게이트) + (b) M11 모닝 브리핑 + (c) **G4 거시/뉴스 → AI 컨텍스트 ✅ 완료(dormant, D33 §4)**. 구 "수동 브리핑 요약 + 규칙기반 3등급 분류" supersede. spec SoT = `ServicePlan-Admin.md §3.10` + `docs/superpowers/specs/2026-06-26-m12a-news-auto-remove-shadow-first.md` + `docs/superpowers/specs/2026-06-26-g4-macro-news-ai-context-layer-design.md`.
> **G4 ✅ 1차 (2026-06-26)**: `lib/macro/{context,source}` distill → 리포트 writer·모닝 브리핑(M11)·**LIVE Tier1 선정 패널(R1/R2)** 3-consumer 컨텍스트 입력(flag `MACRO_CONTEXT_ENABLED` off→byte-identical). **완료 범위 = mock-macro source distill seam + wiring(dormant); 실 FRED/news source + (선택)Polymarket = USER 게이트/후속.** Tier0 factor 금지(allowlist 테스트)·M12a 범주 분리·forward-validate·stale fail-safe. **M11 Resend 제거**(Telegram+dashboard). omxy R1+R2 CONVERGED + Claude deep-review(live-path HIGH catch)·게이트 green.
> **M12a ✅ shadow-first 빌드 (2026-06-27)**: `lib/news/m12a/*`(Core 11 뉴스평가→per-company thesis-break→verdict→smart brake→durable ledger[news_event 1 + 신규 `m12a_ticker_assessment` N, 마이그 0042 DORMANT]→shadow/mutate 게이트→텔레그램+`/admin` durable alert) + go-live fail-closed step-0. **shadow-first**: `M12A_NEWS_EVAL_ENABLED`/`M12A_AUTO_REMOVE_ENABLED` default off → morning-briefing byte-identical·mutation 0. 자동 mutation=출시 후 fast-follow(throw-stub). omxy R1~R3 + Claude 5-dim deep-review CONVERGED·게이트 green(test:ci 2246)·PG smoke 10 assertions. 실 Naver 수집·실 AI 호출·자동 제거=USER 게이트(아래 Task).
- **포함 Must**: M11 (모닝 브리핑) · **M12a (뉴스 기반 자동 제외 — AI 페르소나)**
- **외부 의존**: Naver News API · Anthropic(+멀티프로바이더) · Telegram. **(Resend/이메일 알림 전역 제거 — S7b 뉴스·브리핑, S7c Exit, S7d health 모두 텔레그램 + `/admin` 웹 알림)**
- **선행 BL-KRIT**: BL-KRIT-1(AI 키) · BL-KRIT-3(Naver) · BL-KRIT-5(Telegram) — ~~BL-KRIT-4(Resend)~~
- **예상 세션**: 2.5 (뉴스 AI 분석 + 자동 제외 배선 추가)
- **핵심 산출**: (a) `src/lib/news/naver-api.ts`·`scraper.ts` 실 호출(수집) + **뉴스 AI 페르소나 분석(Core 11, `lib/news/m12a/evaluator.ts`) → scope는 메타데이터로 보존 + 각 보유/리스트 종목별 thesis-break 판정 → auto_remove/alert_only/hold_for_review** + **shadow-first 홈 리스트/가상포트 제외 게이트**(빼기만·freed→현금 KRX EOD virtual_eod; `M12A_AUTO_REMOVE_ENABLED` off→mutation 0, would-remove만 기록; 실 mutation=출시 후 fast-follow throw-stub) + **smart brake**(per-company unique 4건↑[>3]/track 70% floor/집중포트) + **durable ledger 기존 `news_event` 1건 + 신규 `m12a_ticker_assessment` N건**(마이그 0042 DORMANT) + 다음 선정 candidate-level negative-news context 주입(fresh∪incumbent, selection-worker seam·off→byte-identical) + 텔레그램 + `/admin` 웹 알림(news_critical/news_warning 재사용·이메일 0) · (b) `src/lib/briefing/compose.ts` Anthropic 실 브리핑(유지). 08:00 KST Cron 실 실행 검증(실 Naver/AI=USER 게이트). **빌드 ✅ shadow-first dormant(2026-06-27); flag/키/0042 apply=USER 게이트.**

### S7c · 장중 + Exit 텔레그램+/admin 2-layer 실 연결
- **포함 Must**: M13 (장중 이상 감지) · M14 (커스텀 임계치) · M15 (Exit 시그널 텔레그램+/admin 2-layer)
- **외부 의존**: KIS WebSocket · Telegram (`/admin` 웹 알림은 내부 UI/DB 경로)
- **선행 BL-KRIT**: BL-KRIT-2 · BL-KRIT-5 (~~BL-KRIT-4 Resend~~ 72차 사용자 override로 폐기)
- **예상 세션**: 2
- **핵심 산출**: `src/lib/intraday/kis-websocket.ts` 실 구독 · `src/lib/notify/telegram.ts`·`exit-dispatch.ts` 실 발송 + D10 재정의(텔레그램 best-effort + `/admin/alerts` durable event + unread badge catch-up) 검증.

### S7d · Silent Health 실 INSERT + override UI
- **포함 Must**: M18 (파이프라인 헬스체크) · M19 (Silent Health 하트비트) + BL-17 B 구현
- **외부 의존**: Supabase · Telegram (`/admin` 웹 알림은 내부 UI/DB 경로)
- **선행 BL-KRIT**: BL-KRIT-5 (~~BL-KRIT-4 Resend~~ 72차 사용자 override로 폐기)
- **예상 세션**: 1
...
|---|---|---|---|
| Anthropic `/messages` | 리포트·브리핑·뉴스 AI 평가/자동 제외 | Anti-Metric 50만원 hardcap (65차 상향, 구 40만) | usage 파싱으로 실시간 측정 |
| Supabase PostgreSQL | 전 엔티티 실 I/O | 무료 티어 | 프로젝트 한도 |
| Naver News Open API | M12a 뉴스 수집 | 무료 | 일 25,000건 |
| pykrx (로컬 Python) | 영업일·시세 seed | 0 | — |
| KIS REST / WebSocket | 주식 시세·주문 | 계좌당 무료 | 초 20건 |
| ~~Resend~~ | ~~이메일 발송~~ — **72차 사용자 override로 알림 이메일/Resend 전역 제거(S7b·S7c·S7d·M15 모두 미사용)** | 0 | — |
...
> **(65차 Q3 supersede)** 단일 Anthropic 키 전제는 변경됨 — 멀티프로바이더(Claude+GPT), provider availability auto-detect(GPT키 없으면 Claude-only), 모델 하드코딩 제거+설정값화 + 모델 레지스트리, 역할별 모델 차등(토론=저가 / judge·리포트=고가). 'AI 키 발급 여부' 단일 게이트 어휘는 W0 추상화로 재해석. (HANDOFF.md ⭐ 65차 MVP 엔진 섹션 참조)
>
...
- [ ] T7a.2 `src/lib/ai/client.ts` — AI provider wrapper: 모델 하드코딩 제거+설정값화 + 멀티프로바이더(Claude+GPT, provider availability auto-detect, 모델 레지스트리) + prompt cache 옵션 (65차 Q3 W0; HANDOFF.md ⭐ 65차 MVP 엔진 섹션 참조 — 구현은 W0 scope)
...
- [ ] **T7a.7 (D19 Tier 1) Core 11 페르소나 평가**: T7e.8 Tier 0 후보 150 → 시간대별 페르소나 가중치(단기 Druckenmiller↑·중기 Lynch↑·장기 Buffett·Munger·Fisher↑) → 단/중/장 각 10 = 30 선정. `src/lib/screening/persona-eval.ts`. 출력: `🤖 AI 점수`(0~100) per (종목, 시간대) → `short_list_30.ai_score` 컬럼 (**마이그 0029 applied** — consensus_badge/ai_score/weighted_score_×3/winning_timeframe/conviction/ai_comment_kr 8종 nullable; "0010 추가 검토"는 41차 당시 표기로 supersede됨). **(65차 Q4 supersede)** Core 11 병렬 독립 채점 → 실시간 멀티라운드 AI 반박 토론 loop로 확장(합의 점수는 선택), 토론 참가=저가 모델 / 최종 judge=고가, 빌드 순서 W1. PR-G ⓑ(150×11 단발선정)는 W0~W3로 superseded(코드자산 재사용). (HANDOFF.md ⭐ 65차 MVP 엔진 섹션 참조)
- [ ] **T7a.8 (D19 합의 에이전트) 4종 배지 산출**: 🔢 vs 🤖 비교 → 🟢 강한 합의 / 🔵 숫자 우세 / 🟣 AI 우세 / ⚪ AI 분석 대기. `src/lib/screening/consensus.ts`. **(65차 Q4)** 결정론적 합의 에이전트는 실시간 토론 loop 도입 시 합의 점수가 '선택'으로 격하 — 배지/합의 산출은 토론 결과의 부가 산출로 재배치 가능(W1). (HANDOFF.md ⭐ 65차 MVP 엔진 섹션 참조)
- [ ] **T7a.9 (D19 Tier 2) Sector Board 활성화 시점 — 30종목만**: `ReportFramework.md §7` 14섹터별 10명 박제 중 **각 종목의 섹터 14명만** 활성화 (전체 140명 X). 종목당 Core 11 + Sector 14 = 25명. M17 hardcap 50만원 내 통제 (65차, 구 40만 · 월 30 × 25 ≈ 750 LLM call/월; Q4 실시간 토론 loop는 멀티라운드라 호출수 재추정 W1 scope)
- [x] **T7a.10 (D19/D32 Reflection) `reflection_log` ✅ shadow-first 빌드 완료 (2026-06-27, dormant)**: `tudal/src/lib/reflection/*`(pure core + DI orchestrator) + `data/admin-reflection.ts` + 마이그 **0043 `reflection_log` ✅ production applied (2026-06-27, `20260627120308`)** + cron `/api/cron/reflection-job` + `scripts/pg_smoke_0043.sh`. 직전 finalize 사이클 평가 후보 풀 실현 수익률(KRX·무비용) + 페르소나별 적중률·conviction-가중 수익률 → reflection_log → 다음 선정 W1 패널 prompt에 **신규 `reflectionLearningContext` 필드** 주입(`REFLECTION_ENABLED` off→byte-identical). **(65차 D27 주의)** 본 PR-K Reflection(track별 회고 — **D32 2026-06-24 USER로 출시 전 빌드+S9 검증으로 승격**)은 **신규 `reflectionLearningContext` 필드**, **Q5 incumbent thesis 재점검(MVP, 선정 시점 per-incumbent — W2 union + W1 주입)은 기존 `reflectionContext` per-ticker seam** — **별개 필드·혼동 금지**. 회고지 예측 아님(reflection_kind='retrospective' CHECK + summarizer 예측 출력 필터). Claude 5-lens(4 HIGH) + omxy R1(7 fix) CONVERGED. SoT: `docs/superpowers/specs/2026-06-27-reflection-prk-build.md` + `2026-06-24-reflection-prk-pre-launch-promotion.md`(D32). 실 가동·S9 검증 = USER 게이트(§3 HANDOFF).
...
- [x] **T7e.8 (D19, 2026-05-08 35차 · 2026-05-21 53차 §5 정정) Tier 0 인디케이터 자동 스크리닝 — AI 키 불필요**:
  - **53차 §5 박제**: 당시 `short_list_30` 2026-05-01은 AI 미평가 30 rows였고, **73차 supersede**로 풀 P3 PR #109(main `179d745`)가 `short_list_30` 2026-06-01 실 AI 30(🟣20/🟢7/🟡2/🔵1, consensus_badge/ai_score populated)을 생성 완료. PR-G ⓑ 단발 계획은 W0→W2→W1→W3 엔진으로 대체되어 실행 완료(코드자산 재사용). (HANDOFF.md ⭐ 73차 참조)
...
### Phase S7b (세션 4~5) — 뉴스 기반 자동 제외(M12a) + 모닝 브리핑(M11) *(spec SoT: `ServicePlan-Admin.md §3.10 M12a` · **M12a ✅ shadow-first 빌드 완료·dormant 2026-06-27** — `lib/news/m12a/*` + 마이그 0042 DORMANT(USER apply) + PG smoke 10 assertions; flag off→byte-identical·mutation 0; 자동 mutation=출시 후 fast-follow. 빌드 SoT `docs/superpowers/specs/2026-06-26-m12a-news-auto-remove-shadow-first.md`. 실 Naver 수집·실 AI 호출=USER 게이트 잔여)*
- [ ] T7b.1 Naver News API 실 호출 경로 전환(뉴스 수집) — *news-sweep이 실 선정 유니버스(getActiveShortList active short_list_30) 기반 수집(2026-06-29 cfb1bde, 구 3종 하드코딩 제거 → M12a/모닝 브리핑과 동일 범위 정합); 실 키 발급(B-8, `NAVER_CLIENT_ID`+`NAVER_CLIENT_SECRET`) USER 게이트. (잔여 follow-up: M12a의 전역 `getRecentNewsEvents({limit:50})`는 뉴스 많은 종목이 윈도를 잠식해 per-ticker 커버리지가 부분적 — universe-aware read는 별도 작업.)*
- [x] T7b.2a **M12a 뉴스 AI 페르소나 분석** ✅ — Core 11 병렬 평가(`evaluator.ts` `makeM12aNewsEvaluator` + `parseNewsPersonaJudgment` + `aggregateNewsAssessment` 전체 panelSize 과반 consensus) → 구조화 판정(severity/scope/confidence/affected_tickers/materiality/directness/thesis_break_reason/`recommended_action`). `decideRecommendedAction`(verdict.ts) 결정론. **빌드 위치 = `src/lib/news/m12a/*`**(평면 `auto-remove.ts`는 orchestrator mutation helper로 분리됨). Sector 14 = MVP 제외(후순위). 실 AI 호출=USER 비용 게이트(테스트 mock).
- [x] T7b.2b **자동 제외 실행 + smart brake** ✅ — `scope`=메타데이터(비-gate, verdict). auto_remove 조건 충족 시 홈 리스트+가상포트 제외(빼기만·freed→현금 KRX EOD `virtual_eod` fail-closed) — **shadow-first: `M12A_AUTO_REMOVE_ENABLED` off→mutation 0(throw-stub), would-remove만 기록**. brake(`applySmartBrake`: per-company unique mass>3 / track 70% floor short<7·midlong<14·full<21 / 집중포트 N<10 2건↑ → whole-run hold_for_review). 텔레그램 + `/admin` durable alert(news_critical/news_warning 재사용·이메일 0).
- [x] T7b.2c **durable ledger + 재진입 context** ✅ — 기존 `news_event` 1건 + **신규 `m12a_ticker_assessment` N건**(마이그 0042, action_taken=shadowed|held_by_brake|removed = GAP1 3-layer 귀속·hold_for_review는 mutation 0) + 다음 선정 negative-news context seam(`selection-worker`→`renderUserPrompt`, fresh∪incumbent uniform digest·D27 seam과 별개·additive·off→byte-identical). **`alert_event.alert_type` enum 불변**(removal 타입 추가 X). 마이그 0042 DORMANT(USER apply).
- [ ] T7b.3 M11 Anthropic 실 브리핑 생성 (08:00 KST Cron 실 검증, 기존 유지) — *M12a attentionTickers seam 배선 완료(flag off→[])*
- [ ] ~~T7b.2 Resend 도메인 인증~~ **폐기 — 72차 사용자 override: 이메일/Resend 알림 전역 제거. S7b 뉴스·브리핑, M15/S7c Exit, S7d health 모두 텔레그램 + `/admin` 웹 알림.**
- [ ] T7b.5 `briefing_log`·`news_event`·removal ledger 실 INSERT 확인
- [ ] **AI 키 게이트**: M12a 뉴스 AI 분석/자동 제외는 AI 키(멀티프로바이더 Claude+GPT, auto-detect) 필요. **AI 미발급 시 자동 제외 비활성**(Naver 뉴스 수집·요약만, M11 브리핑은 뉴스 요약 부분만 mock), 키 발급 시 plug-in. (65차 Q3 — HANDOFF ⭐ 65차 MVP 엔진 섹션)

### ★ S7b 완료 후 게이트 — D11 AI 가상 포트 1차 가동 (D18, 2026-05-08)

> **D18 박제**: S7b 완료 시점에 KRX/pykrx/DART/네이버 + 실 Anthropic + 실 Supabase로 D11 AI 가상 포트가 **KIS 0개로 작동 가능**한 상태 도달. S7c 진입 전 **어드민 3인이 며칠~1주 운용 검증** — 가상 포트 의사결정 품질·승인 워크플로우·재생성 cap·M12a 뉴스 자동 제외/검토 판정·알림 정확도 등 검증 후 S7c 진입. 자동매매(S8)는 **🎉 출시 후** 단독 진입 (2026-06-01 재배치 · 구 'D11/D18 운용 검증 후'; 현재 순서 = S7d → S9 운용 → 출시 → S8).

운용 검증 체크리스트:
> **(65차 supersede)** Q1: 선정주기 단기 주1회 / 중장기 월1회 분리 → '단10/중10/장10 동시 30' 고정 가정 재해석(주간 lane=단기, 월간 lane=중장기). Q2: 30 전체 운용 고정 → AI 자율 포트(운용여부·총개수·종목·단중장분배·비중·현금0~30% 전부 AI 자율)이므로 '항상 30 운용'·'승인 워크플로우'(Accept/Reject만)는 AI 자율 산출 검증으로 변경. Q4: 합의 배지 4종은 토론 loop 부가 산출(합의 점수 선택)로 재배치. (HANDOFF.md ⭐ 65차 MVP 엔진 섹션 참조)
...
- [ ] 모닝 브리핑 실 발송 + 뉴스 기반 자동 제외 정확도 체감 (M11·M12a) — AI 키 ✅ 조건부
...
- [ ] T7c.4 M15 Exit 텔레그램+/admin 2-layer + D10 durable catch-up 실 동작
...
- [ ] T7d.4 텔레그램 best-effort + `/admin` durable event/unread badge catch-up 실 검증
...
- **BL-KRIT-1~3, 5, 7** — HANDOFF §4 참조 (Anthropic·KIS·Naver·Telegram 외부 계정 발급 + 마이그레이션 **0010**; ~~BL-KRIT-4 Resend~~ 72차 사용자 override로 폐기)
...
- **R-S7-1** Anthropic 비용 폭주 (실 AI 호출 첫 도입 시점)
  - 완화: S7a에서 dry-run 견적 먼저 돌리고 한도 override 없이 35만 경보 / 50만 hardcap 실 동작 확인 (65차: hardcap 50만으로 상향, 구 40만 · 35만 경보값은 현재 코드값/기존 경보값 — 65차 W0에서 재산정)
...
- **R-S7-4** Telegram Bot/chat_id 등록 지연
  - 완화: `/admin/alerts` durable event + 대시보드 unread badge가 catch-up 안전망을 맡고, Telegram은 토큰·chat_id 확보 즉시 best-effort 발송으로 활성화
