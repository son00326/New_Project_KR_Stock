...
>
> **[77차 D30 supersede 2026-06-12]** 아래 T7e.8의 Tier 0 스코어링(`scripts/screen_shortlist_tier0.py` 5-Signal cross-section z정규화 `(z+3)/6×100` → 시간대 가중 → 단/중/장 top50=150)은 **예측력 미검증 + 실행 결함 다수**(결측 0주입·z fat-tail·외국인 size bias·가중치 IC 미검증)로 진단됨 → **"B+ 형태수정 + 경량 IC 검증" 채택(D30, 구현 다음 세션)**. T7e.8/T7a.7의 스코어링·가중치·정규화 기술은 **현 코드 상태(검증 전)** 기준 history이며, 다음 세션 재설계 SoT = `docs/superpowers/specs/2026-06-12-tier0-scoring-bplus-validation.md` + `ServicePlan-Admin.md §1A.5 D30`. **검증 통과 전 후보 150을 "향후 상승 예측"으로 제시 금지.**
>
