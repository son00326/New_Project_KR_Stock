...
- **외부 의존**: Naver News API · Anthropic(+멀티프로바이더) · Telegram. **(Resend 제거 — S7b 뉴스 알림은 텔레그램+앱만. 비-S7b[M15/S7c Exit·S7d health] 이메일은 별도·불변)**
- **선행 BL-KRIT**: BL-KRIT-1(AI 키) · BL-KRIT-3(Naver) · BL-KRIT-5(Telegram) — ~~BL-KRIT-4(Resend)~~
- **예상 세션**: 2.5 (뉴스 AI 분석 + 자동 제외 배선 추가)
- **핵심 산출**: (a) `src/lib/news/naver-api.ts`·`scraper.ts` 실 호출(수집) + **뉴스 AI 페르소나 분석(Core 11) → 구조화 판정 → auto_remove/alert_only/hold_for_review** + 홈 리스트/가상포트 자동 제외(빼기만·freed→현금) + **smart brake**(4건↑[>3]/track 70% floor) + **durable removal ledger**(news_event linked) + 다음 선정 candidate-level negative-news context 주입(fresh∪incumbent) + 텔레그램/앱 알림 · (b) `src/lib/briefing/compose.ts` Anthropic 실 브리핑(유지). 08:00 KST Cron 실 실행 검증.
...
| Resend | 이메일 발송 — **S7b 뉴스/브리핑 미사용(텔레그램+앱). 비-S7b 경로(M15/S7c Exit·D10·S7d health/catch-up 등)는 보존** | ~$20/월 (3K emails) | — |
...
- [ ] T7b.2b **자동 제외 실행 + smart brake** — auto_remove 조건(direct·material·high-conf·회사고유|thesis 직접 깨는 sector) 충족 시 홈 리스트 + 가상포트 제외(빼기만·freed→현금) + brake(`max_auto_removals_per_run`=3 초과[4건↑] / track 70% floor 위반 → 그 run 전체 hold_for_review; 집중포트 N<10 = 1건 자동·2건↑ 보류). 텔레그램+앱 알림(이메일 미발송).
- [ ] T7b.2c **durable removal ledger + 재진입 context** — `news_event`-linked ledger 기록(홈 리스트 row 삭제와 무관 영속) + 다음 선정 시 모든 Tier1 후보(fresh∪incumbent)에 negative-news context 주입(D27 incumbent-only seam과 별개·additive). **`alert_event.alert_type` enum 불변**(removal 타입 추가 X — build-time migration defer).
- [ ] T7b.3 M11 Anthropic 실 브리핑 생성 (08:00 KST Cron 실 검증, 기존 유지)
- [ ] ~~T7b.2 Resend 도메인 인증~~ **폐기 — S7b 뉴스/브리핑 알림 = 텔레그램+앱. M15/S7c Exit 이메일(Resend·D10)은 별도 트랙·유지**
