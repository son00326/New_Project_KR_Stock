...
> **(72차 재정의 + 2026-06-26 G4)** S7b = (a) **뉴스 기반 자동 제외**(AI 페르소나, planned·미구현) + (b) M11 모닝 브리핑 + (c) **G4 거시/뉴스 → AI 컨텍스트 ✅ 완료(dormant, D33 §4)**. 구 "수동 브리핑 요약 + 규칙기반 3등급 분류" supersede. spec SoT = `ServicePlan-Admin.md §3.10` + `docs/superpowers/specs/2026-06-26-g4-macro-news-ai-context-layer-design.md`.
> **G4 ✅ 1차 (2026-06-26)**: `lib/macro/{context,source}` distill → 리포트 writer·모닝 브리핑(M11)·**LIVE Tier1 선정 패널(R1/R2)** 3-consumer 컨텍스트 입력(flag `MACRO_CONTEXT_ENABLED` off→byte-identical). **완료 범위 = mock-macro source distill seam + wiring(dormant); 실 FRED/news source + (선택)Polymarket = USER 게이트/후속.** Tier0 factor 금지(allowlist 테스트)·M12a 범주 분리·forward-validate·stale fail-safe. **M11 Resend 제거**(Telegram+dashboard). omxy R1+R2 CONVERGED + Claude deep-review(live-path HIGH catch)·게이트 green. **AI 페르소나·자동 제외(M12a) 로직은 여전히 planned(미구현) — 별도 후속.**
...
- **핵심 산출**: (a) `src/lib/news/naver-api.ts`·`scraper.ts` 실 호출(수집) + **뉴스 AI 페르소나 분석(Core 11) → scope는 메타데이터로 보존 + 각 보유/리스트 종목별 thesis-break 판정 → auto_remove/alert_only/hold_for_review** + 홈 리스트/가상포트 자동 제외(빼기만·freed→현금) + **smart brake**(4건↑[>3]/track 70% floor) + **news_event 1건 + per_ticker_assessments N건 durable ledger** + 다음 선정 candidate-level negative-news context 주입(fresh∪incumbent) + 텔레그램 + `/admin` 웹 알림 · (b) `src/lib/briefing/compose.ts` Anthropic 실 브리핑(유지). 08:00 KST Cron 실 실행 검증.
...
### Phase S7b (세션 4~5) — 뉴스 기반 자동 제외(M12a) + 모닝 브리핑(M11) *(spec SoT: `ServicePlan-Admin.md §3.10 M12a` · planned — 미구현)*
- [ ] T7b.1 Naver News API 실 호출 경로 전환(뉴스 수집)
- [ ] T7b.2a **M12a 뉴스 AI 페르소나 분석** — Core 11 병렬 평가 → 구조화 판정(severity/scope/confidence/affected_tickers/materiality/directness/thesis_break_reason/`recommended_action`). 신규 `src/lib/news/auto-remove.ts`. Sector 14 = MVP 제외(후순위).
- [ ] T7b.2b **자동 제외 실행 + smart brake** — `scope`는 gate가 아닌 메타데이터로 보존하고, 회사/섹터/거시 뉴스 모두 각 보유·리스트 종목별 thesis-break를 평가한다. auto_remove 조건(direct·material·high-conf·affected_ticker per-company thesis-break) 충족 시 홈 리스트 + 가상포트 제외(빼기만·freed→현금) + brake(`max_auto_removals_per_run`=3 초과[4건↑] / track 70% floor 위반 → 그 run 전체 hold_for_review; 집중포트 N<10 = 1건 자동·2건↑ 보류). 텔레그램 + `/admin` 웹 알림.
- [ ] T7b.2c **durable ledger + 재진입 context** — `news_event` 1건 + `per_ticker_assessments` N건 기록(홈 리스트 row 삭제와 무관 영속; hold_for_review는 proposed_action/reason 보존·mutation 금지) + 다음 선정 시 모든 Tier1 후보(fresh∪incumbent)에 negative-news context 주입(D27 incumbent-only seam과 별개·additive). **`alert_event.alert_type` enum 불변**(removal 타입 추가 X — build-time migration defer).
- [ ] T7b.3 M11 Anthropic 실 브리핑 생성 (08:00 KST Cron 실 검증, 기존 유지)
