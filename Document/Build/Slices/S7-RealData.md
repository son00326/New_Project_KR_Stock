...
> **[73차 현황 2026-06-09]** 본 슬라이스 파일은 history. **현 production: `short_list_30` 2026-06-01 = 실 AI 선정 30**(consensus_badge/ai_score populated, 🟣20/🟢7/🟡2/🔵1) — 풀 P3 실행 완료(PR #109, cost ₩24,655.64). 아래 53/54차 박제의 "Tier 0 단독 fallback"·"다음 1순위 = PR1" 등은 **역사**(당시 기준). 라이브 SoT = `Document/Process/HANDOFF.md` ⭐ 73차 / §0.
>
...
- [x] **T7e.8 (D19, 2026-05-08 35차 · 2026-05-21 53차 §5 정정) Tier 0 인디케이터 자동 스크리닝 — AI 키 불필요**:
  - **53차 §5 박제**: Tier 0 단독 30 rows production 적용 상태 = **fallback** (메인 path 아님). 메인 path = Tier 1 AI 30 선정. **(64차 현황 supersede) Tier1 선정 엔진/청크 워커 빌드 ✅ 완료** (PR2 `f85fb69` + PR-E `492cd46` 마이그 0029 applied + 선정 청크 워커 PR #82 마이그 0031 written-not-applied dormant) — 실 AI 첫 30선정 = **PR-G ⓑ** (USER 마이그 0031 apply + `SELECTION_CRON_AUTO_ENABLED=true` + Anthropic 키 + 비용 게이트). canonical 5-PR(PR2→PR3a→PR1→PR3b→PR3c→PR4) 전체 MERGED. **(65차 supersede)** PR-G ⓑ(150×11 단발선정)는 W0(모델/프로바이더 추상화+hardcap 50만+cheap smoke)→W2(주간/월간 split+rolling composite writer)→W1(토론 loop)→W3(자율 포트)로 superseded(코드자산 재사용). 'Anthropic 키' 단일 게이트 → 멀티프로바이더 레지스트리+auto-detect(Q3). Tier 1 평가 = 토론 loop(W1)로 확장(Q4). (HANDOFF.md ⭐ 65차 MVP 엔진 섹션 참조)
