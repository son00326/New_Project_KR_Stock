...
Last updated: 2026-05-12 (T7e.8 follow-up — **DART Signal 4·5 production 적용 ✅** · `short_list_30` 2026-05-01 30 rows UPSERT · S7e 7/8 sub-task, T7e.7 RLS QA 잔여)
...
**진행률 (정확)**: Mock 동작 **19/19** · **DQ-7 구현 ~97% (Session 1·2 ✅ + 30차 Vercel + 32차 Supabase 마이그 + 33차 Smoke #1·#2·#6 ✅ — Smoke #4·#5 잔여 · Smoke #3은 S8까지 유예)** · **S7e 7/8 (T7e.1·T7e.2·T7e.3·T7e.4·T7e.5·T7e.6 ✅ + T7e.8 DART 실 시드 ✅), T7e.7 RLS QA 잔여** · 실데이터 **1+/19** (`short_list_30` 30 rows production 적용 · Signal 4·5 DART 실데이터 반영) · 실 AI 호출 **0** · 자동매매 프레임 **0%** · 실 운용 검증 **0일** · **Vercel Production ✅ (25 routes, son00326 신 Supabase 연결)** · 검증 게이트 **build 25 routes · lint 0 · test:ci 384 pass / 49 files**
