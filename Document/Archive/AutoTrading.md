...
> **⚠️ ARCHIVE — D11(2026-04-15) 이전 "자동매매 독립 트랙" 가정 기반 리서치 원자료.**
> 현 SoT = `../Build/Slices/S8-AutoTrading.md` (2026-04-21 D16 S8 단일 슬라이스 통합).
> 본 문서의 "완전 독립"·"별도 종목 풀"·"별도 성과 추적" 전제는 폐기됨. S8 AI 어댑터 drop-in 시 참조 원자료 가치로 보존. **2026-04-22 Archive 이관 (28차 docs cleanup)**.

---

Last updated: 2026-04-15 (D11 위상 재정의)
Status: 레퍼런스 6종 분석 완료 / 데이터 소스 팩트체크 완료 / 한국 전환 매핑 완료 / **D11 재정의 — 어드민 서브시스템 (미확정)**

---

> **[D11 위상 재정의 — 2026-04-15]** 자동매매는 **어드민 서비스 내 서브시스템(경로 2)** 으로 재정의됨. 구 "완전 독립 트랙" 가정은 폐기.
> - **상위 모델**: `BusinessPlan §Q10 레이어 B` + `§10.5 3경로 집행` + `§10.8 자동매매 트랙과의 관계`. 주픽 어드민의 AI 가상 포트(레이어 A, §10.5)는 매뉴얼·자동매매가 **공통 참조**. 가상 포트·Track Record·NSM은 단일 SoT.
> - **실제 집행**: E9 `BrokerageConnection.scope=auto` 레코드 사용. 어드민별·전략별 복수 API 허용(D12).
> - **범위**: 주식/코인 모두 **미확정**. Stage 2~3 이전 별도 결정.
> - **본 리서치 문서의 역할**: 자동매매 구현 옵션 조사 원자료. 본 문서의 "완전 독립"·"별도 종목 풀"·"별도 성과 추적" 등 기존 전제는 D11 이후 폐기. 이후 설계·구현은 ServicePlan-Admin v1.0 기준으로 전환.
...
> **[기준선 단순화 → D11 재정의 후 필요 항목만 잔류 예정. 현 서술은 리서치 원자료]**
> 본 문서는 자동매매 트랙 ~~(리포트 트랙과 독립)~~ → **어드민 서비스 내 서브시스템(경로 2)** 의 외부 레퍼런스 조사 기록이다. D11 재정의(2026-04-15) 이후:
> - "리포트 트랙과 독립" → **폐기**. AI 분석·가상 포트폴리오(§10.5 레이어 A)는 매뉴얼·자동매매가 **공통 사용**.
> - "독립 트랙"·"별도 종목 풀"·"별도 성과 추적" → **폐기**. 가상 포트·Track Record·NSM은 단일 SoT(ServicePlan-Admin §1A.0).
> - 본 문서의 스킬·도구·데이터 소스는 D11 모델(가상 포트 공유 + 집행 레이어 분리) 관점에서 재검토한 뒤 필요 항목만 잔류시킨다.
