...
- **41~45차 (2026-05-09~12)**: **T7e.8 Tier 0 DART Signal 4·5 production 적용 ✅** + **D20 Section 8 위원 전원 표 박제 ✅**. 0012 short_list_30 name/sector + 0013 dart_corp_codes + 0014 dart_financial_cache 마이그 → `seed_dart_corp_codes.py` 2,766 rows + `scripts/screen_shortlist_tier0.py` 실 standalone/quality 기반 점수로 `short_list_30` 2026-05-01 30 rows production UPSERT (short=모멘텀 10 / mid=실적 모멘텀 8 + 모멘텀 2 / long=퀄리티 10). D20 = Section 8 정적 표 4종(Sector 14명 + Core 11명 전원 + 쟁점 인용 3~5건 + 최종 합의 패널) — `ServicePlan-Admin.md §3.7 R3.7-6/7/8` + `§6 D20` + `ReportFramework §8 Step 2 v2.3` 박제. test:ci 384/49. 다음 1순위 = S7a (AI 키 B-6 트리거).
- **46차 (2026-05-13)**: **QA audit → P0·P1 실 실행 + production hotfix push ✅** (omxy cmux pair-debate 7 rounds 모두 CONVERGED — R1·R2 P0.1 / R3·R4 hotfix push / R5·R6 P1.1 / R7 P2.1). **P0.1** = 마이그 0015a `definer_execute_lockdown` apply → SECURITY DEFINER 5 함수 PUBLIC + anon EXECUTE 회수 + 활성 3종만 authenticated 유지 (omxy least-privilege 정정으로 미사용 2종도 회수). advisor anon WARN **5→0** / authenticated **5→3**. **P0.3** = `DQ7-Credentials.md §6.10` 운영 SOP — 0009 rollback production 금지 + 3 조건 체크리스트. **P0.2** = HIBP 토글(B-2A 사용자 dashboard 잔여). **P1.1** = `src/lib/admin/format-error.ts` 헬퍼 신규 + 5 panel(portfolio/regenerate/exit-decision/settings/credential forms) ad-hoc switch 제거 + credentials lib raw DB passthrough wrap(`Invalid id format`/`pending-s8`/DB error.message → 한국어). +40 tests. **P1.3** = `(admin)/loading.tsx` + `error.tsx` + `not-found.tsx` 한국어 boundary. **P1.4** = reportLinksEnabled/actionsEnabled/actionsDisabledMessage 4 prop + 분기 일괄 제거 + canLinkDeltaReport 단일 인자화. **Production hotfix** = HANDOFF B-15 ahead 39 commits + 46차 P0 batch `git push origin main` → Vercel auto-deploy → `real_persistence_not_configured` 에러 해소. **검증 게이트**: build 25 routes / lint 0 / **test:ci 429/50** (+45 vs 384/49) / `tsc --noEmit` clean. **Git**: commits `9661037` (P0.1+P0.3+P1.2) + `4c6eea7` (P1.1+P1.3+P1.4) + 본 P2.1 SoT commit. 다음 1순위 = §7 P2.2~P2.4 + P3 잔여 + **S7a (B-6 AI 키 발급 트리거)**.
- **47차 (2026-05-13)**: **§7 P2.2~P2.4 SoT cleanup 완료 ✅** (cmux pair-debate omxy 4 rounds 모두 CONVERGED). HANDOFF "403"→"401" + news-sweep daily 주석 + .env.example DART/KRX 3 키 superset + CodebaseStatus 체크리스트 14 항목 정정 + record-view TODO 정정 + FixPlan-46 자기 경로 정정. 7 파일 +46/-24. test:ci 429/50 유지. **Git**: commit `03d9bc7` (push 보류 — 48차 commit과 함께 일괄 push 대기 B-16).
- **49차 (2026-05-19)**: **S7a Anthropic wrapper 코드 구현 진입 ✅** (`feat/s7a-anthropic-wrapper` branch 신설, 8 commits ahead of main, push 보류 — billing 미충전 상태에서 mock 100% 구축). **brainstorming → writing-plans → subagent-driven-development** 흐름. **omxy debate 누적 25 rounds CONVERGED** (21 brainstorm/plan + 1 R5 + 3 code-review). 사용자 명시 = "omxy랑 토론해서 정말 완벽하게 검토" + "사용자 승인 게이트 제거" (omxy CONVERGED = 사용자 승인 등가). **Q1~Q6 + Q5b 합의**: 범위 B / cost_log flag toggle / section_8 schema SoT = ServicePlan-Admin §4 / Core 11 TS const registry + version YYYY-MM-DD / 임계값 hard-coded TOP_PERCENTILE_THRESHOLD=0.30 / **5종 배지 (🟢 강한 합의·🔵 숫자 우세·🟣 AI 우세·🟡 관망(신규)·⚪ AI 분석 대기)** / prompt cache flag + persona-major warm-first. **Design R4 합의**: monthly_batch_runs lock + preflight upper-bound × 30 + cost_log·writer 트랜잭션 독립 + cron route mock dry-run only + admin server action trigger. **Plan R4 합의**: acquire_batch_lock RPC(p_admin_id 제거, 내부 auth.uid()) + ticker 추적 + Tier1 score+bucket rank+isTopTier + stock_reports.consensus_badge 컬럼 + commit_persona_eval/commit_badge_only 분리 + CORE_USER_PROMPT_TEMPLATE shared. **Task 4/17 완료**: Task 1 마이그 0017 (3 RPC + RLS + UNIQUE + stock_reports.consensus_badge) / Task 2 section-8-schema.ts zod canonical contract / Task 3 pricing.ts (R5 정정으로 anthropic-pricing.ts wrapper) / Task 4 persona registry 11 + shared template + render. **omxy 적대적 코드 검토 R1~R3 = 3 critical BLOCKERS catch + fix**: (1) PostgreSQL `IF <null>` pass-through → `is null or ... is distinct from ...` + `coalesce(v->>'vote','')` (c14fb2e) (2) Q3 section_8 partA `z.array()` 1~13 통과 → `.refine(len === 0 || len === 14)` (c14fb2e) (3) **committee_votes.vote 기존 check enum(`approve/reject/abstain` 마이그 0003) vs RPC `BUY/HOLD/SELL` 23514 충돌** — plan R1~R4도 못 catch한 spec gap — RPC INSERT case 매핑(BUY→approve / HOLD→abstain / SELL→reject) + `invalid_vote_row` guard 추가 (a2d2c04). section_8.partD.vote = BUY/HOLD/SELL 유지 (writer 산출물 의미), DB 저장 = 운영 의사결정 의미로 매핑. **마이그 0017 production apply 보류** (Task 17 후 사용자 트리거 B-17). **다음 1순위 = Task 5 cost-logger → ... → Task 17 검증 게이트** (HANDOFF §2.A + plan verbatim). **Lesson**: subagent self-review만으로 critical blocker 놓침 — omxy 적대적 코드 검토 매 task 강제 (HANDOFF §7 패턴).
- **48차 (2026-05-13)**: **§7 P3.2 + P3.4 완료 + 마이그 0016 production apply + origin push ✅** (cmux pair-debate omxy 3 rounds 모두 CONVERGED + 사용자 트리거 후속). **P3.2** = 마이그 `0016_accept_shortlist_rpc.sql` + rollback — `accept_shortlist_with_snapshots(p_month text, p_shortlist_generated_at timestamptz, p_snapshots jsonb) returns jsonb` SECURITY DEFINER + `set search_path = public, pg_temp`. plpgsql 단일 txn(auto-rollback). auth 순서 omxy 정정: `auth.uid() null → auth_unavailable` 먼저 → `is_admin() → admin_required`. `jsonb_typeof array` guard. portfolio_approval INSERT + portfolio_snapshot bulk INSERT (`jsonb_array_elements`). EXCEPTION unique_violation → `get stacked diagnostics constraint_name` 분기 (`portfolio_approval_final_month_uniq` → `already_finalized` return / 기타 → re-raise). **권한**: `revoke from public + revoke from anon + grant to authenticated` (0015a 마이그 적용 후 추가된 신규 SECURITY DEFINER 함수는 Supabase default grant로 anon에 X 자동 부여됨 → 명시 revoke 필요. 적용 직후 advisor 신호로 발견하여 follow-up 마이그로 hotfix + 0016 SoT 파일에도 라인 추가). `admin-approvals.ts`에 `acceptShortlistRpc()` wrapper 신규. `actions.ts:267-302` acceptShortList → RPC 단일 호출 (orphan G-1 차단). `isUniqueViolation` defensive catch 잔존(snapshot-side re-raise 매핑). **P3.4** = G-cron-auth +12 (4 cron × 3 unauth) + G-wrapper-error +8 (5 wrapper characterization tests) + G-FE-map +9 (5 specific + 2 prefix edge + 2 dev console.warn) + acceptShortlistRpc +4 = **+33 tests**. **검증 게이트**: build 25 routes / lint 0 / **test:ci 463/50** (+34 vs 429) / `tsc --noEmit` clean. **마이그 0016 production apply 완료 ✅** (사용자 트리거 직후 + anon revoke hotfix). advisor anon WARN 0건 유지 / authenticated WARN 3→4 (accept_shortlist_with_snapshots intended). Apply order: 0010 → 0012~0014 → 0015a → **0016** → (0011 = S8 reserve). **Git**: 본 commit 2건 + 47차 `03d9bc7` 모두 `git push origin main` 완료 ✅, Vercel auto-deploy 트리거. **B-16 해소**. 다음 1순위 = **S7a Anthropic wrapper (B-6 AI 키 발급)** 또는 P3.1 D20 컴포넌트(S7a 시드 후) + P3.3 error taxonomy(사용자 결정). **Lesson**: 향후 SECURITY DEFINER 마이그는 반드시 `revoke from public + revoke from anon + grant to authenticated` 3종 세트.
