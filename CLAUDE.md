...
- **48차 (2026-05-13)**: **§7 P3.2 + P3.4 완료 ✅** (cmux pair-debate omxy 3 rounds 모두 CONVERGED). **P3.2** = 마이그 `0016_accept_shortlist_rpc.sql` + rollback 신규 — `accept_shortlist_with_snapshots(p_month text, p_shortlist_generated_at timestamptz, p_snapshots jsonb) returns jsonb` SECURITY DEFINER + `set search_path = public, pg_temp`. plpgsql 단일 txn(auto-rollback). auth 순서 omxy 정정: `auth.uid() null → auth_unavailable` 먼저 → `is_admin() → admin_required`. `jsonb_typeof array` guard. portfolio_approval INSERT + portfolio_snapshot bulk INSERT (`jsonb_array_elements`). EXCEPTION unique_violation → `get stacked diagnostics constraint_name` 분기 (`portfolio_approval_final_month_uniq` → `already_finalized` return / 기타 → re-raise). `revoke from public + grant to authenticated`. `admin-approvals.ts`에 `acceptShortlistRpc()` wrapper 신규. `actions.ts:267-302` acceptShortList → RPC 단일 호출 (orphan G-1 차단). `isUniqueViolation` defensive catch 잔존(snapshot-side re-raise 매핑). **P3.4** = G-cron-auth +12 (4 cron × 3 unauth) + G-wrapper-error +8 (5 wrapper characterization tests) + G-FE-map +9 (5 specific + 2 prefix edge + 2 dev console.warn) + acceptShortlistRpc +4 = **+33 tests**. **검증 게이트**: build 25 routes / lint 0 / **test:ci 463/50** (+34 vs 429) / `tsc --noEmit` clean. **마이그 0016 apply 보류** (B-16 큐, 사용자 트리거 대기). Apply order: 0010 → 0012~0014 → 0015a → **0016** → (0011 = S8 reserve). **Git**: 본 commit + 47차 stale `03d9bc7` 모두 origin push 대기 (omxy R1·R3 권고). 다음 1순위 = **S7a Anthropic wrapper (B-6 AI 키 발급)** 또는 P3.1 D20 컴포넌트(S7a 시드 후) + P3.3 error taxonomy(사용자 결정).
