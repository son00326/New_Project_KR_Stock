...
**현 진행 순서 (현 진행 상태 live SoT = `Document/Process/HANDOFF.md` · `Document/Build/ProgressDashboard.md §2` — W3b-3 merge main `7c50dd0` + docs-sync main `726497d` 기준 canonical 5-PR + e2e engine PR-D~PR-G ⓐ + launch-readiness PR #79(A~E) + **66차 W0 ✅ MERGED(PR #86 — 모델/프로바이더 추상화 LlmProvider+model-registry 7 role + openai@6.42 + hardcap 50만 + D28 비용가드 3종 + projection 350,167≤50만 + cost_hardcap_exceeded rename)**, **67차 W2a(주간/월간 split + rolling composite) ✅ MERGED(PR #89)**, **68차 W2b ✅ MERGED(PR #91)** + **68차 W1a(Q4 토론 loop ① — D28 mix Sonnet×6+GPT×5 + R2 선택적 반박 라운드 + 마이그 0032 dormant) ✅ MERGED(PR #92 + 배선감사 fix #93, main `d032034`)** + **68차 W1b(Q4 토론 loop ② — judge/dual-judge + 단발 경로 정리) ✅ MERGED(PR #94 + 배선감사 fix #95, main `30107cc`)** + **68차 W3a(entry_price KRX EOD — AI 키 불필요·이중 게이트+누락 fail-closed) ✅ MERGED(PR #96 + 배선감사 LOW fix)** + **69차 W3b-1(portfolio_proposal AI 제안 플러밍 — Opus `portfolio` role + proposePortfolio 반환-only) ✅ MERGED(PR #97, code-merge `6b118bc`, docs-sync `89a4f18`)** + **69차 W3b-2a(portfolio_proposal DB 영속 — dormant 0034 + upsert RPC + persist 게이트) ✅ MERGED(PR #98, main `c1ee610`)** + **69차 W3b-2b(Accept proposal weight — money-path, cash implicit) ✅ MERGED(PR #99, main `96c91b5`)** + **69차 W3b-3(admin UI: proposePortfolio 버튼 + proposal 표시) ✅ MERGED(PR #100, main `7c50dd0`)** → 다음 1순위 = **W3b-2c(명시 cash row + proposal status lifecycle, 선택 refinement)** 또는 **go-live USER 게이트** 또는 **S7b** · 빌드 순서 W0✅→W2a✅→W2b✅→W1a✅→W1b✅→W3a✅→W3b-1✅→W3b-2a✅→W3b-2b✅→W3b-3✅ (USER 게이트 = `SELECTION_CRON_AUTO_ENABLED`+키/비용 + 주간 tier0 producer + 0032·0033 apply(R2/judge 가동 전) + 0034 apply(영속 가동 전) + (W3a 선택) `PORTFOLIO_REAL_ENTRY_PRICE_ENABLED`+`KRX_OPENAPI_KEY` + (W3b-1 실 Opus) `PORTFOLIO_AI_PROPOSAL_ENABLED`+`ANTHROPIC_API_KEY`+`AI_COST_LOG_REAL_INSERT_ENABLED` + (W3b-2a 영속) `PORTFOLIO_PROPOSAL_PERSIST_ENABLED` + (W3b-2b Accept weight) `PORTFOLIO_USE_PROPOSAL_ENABLED`), 상세 HANDOFF ⭐ 65차 MVP 엔진)**:
```
Mock Skeleton ✅
  ...
  → **Tier 2 D21 (52차)** ── ✅ **scaffold + production schema MERGED in main** (PR #4 + PR #5 + PR #6 historical, 마이그 0018·0019 production applied)
       ...
       · **출시 Runbook 진입점 = `Document/Process/HANDOFF.md §2`** (현재 위치·다음 1순위 live SoT = HANDOFF) · W3b-3 merge main `7c50dd0` + docs-sync main `726497d` 기준 canonical 5-PR + e2e engine PR-D~PR-G ⓐ + launch-readiness PR #79(A~E) + 67차 W2a ✅(PR #89) + 68차 W2b ✅(PR #91) + 68차 W1a ✅(PR #92+#93) + 68차 W1b ✅(PR #94+#95) + 68차 W3a ✅ MERGED(PR #96) + 69차 W3b-1 ✅ MERGED(PR #97, code-merge `6b118bc`) + 69차 W3b-2a ✅ MERGED(PR #98, main `c1ee610`) + 69차 W3b-2b ✅ MERGED(PR #99, main `96c91b5`) + 69차 W3b-3 ✅ MERGED(PR #100, main `7c50dd0`) → **다음 1순위 = `W3b-2c`(선택 refinement) 또는 go-live USER 게이트 또는 S7b** (65차 supersede: PR-G ⓑ → W0✅→W2a✅→W2b✅→W1a✅→W1b✅→W3a✅→W3b-1✅→W3b-2a✅→W3b-2b✅→W3b-3✅로 분해 · USER 게이트 = `SELECTION_CRON_AUTO_ENABLED`+키/비용 + 주간 tier0 producer + 0032·0033 apply + (W3a) PORTFOLIO_REAL_ENTRY_PRICE_ENABLED+KRX_OPENAPI_KEY + (W3b-1 실 Opus) PORTFOLIO_AI_PROPOSAL_ENABLED+ANTHROPIC_API_KEY+AI_COST_LOG_REAL_INSERT_ENABLED + (W3b-2a 영속) PORTFOLIO_PROPOSAL_PERSIST_ENABLED+마이그 0034 apply + (W3b-2b Accept weight) PORTFOLIO_USE_PROPOSAL_ENABLED, 상세 HANDOFF §2 ⭐ 65차 MVP 엔진)
       · 다음 1순위 = **canonical 5-PR 전체 MERGED (PR2 `f85fb69` / PR3a `0813a41` / PR1 `4aa3130` / PR3b `cf68731` / PR3c `b2a902a` / PR4 `7de9696`) + e2e engine PR-D~PR-G ⓐ + launch-readiness PR #79(A~E) + **67차 W2a ✅(PR #89) + 68차 W2b ✅(PR #91) + 68차 W1a ✅(PR #92+#93) + 68차 W1b ✅(PR #94+#95) + 68차 W3a ✅ MERGED(PR #96) + 69차 W3b-1 ✅ MERGED(PR #97, code-merge `6b118bc`) + 69차 W3b-2a ✅ MERGED(PR #98, main `c1ee610`) + 69차 W3b-2b ✅ MERGED(PR #99, main `96c91b5`) + 69차 W3b-3 ✅ MERGED(PR #100, main `7c50dd0`)** → **다음 1순위 = `W3b-2c`(선택 refinement) 또는 go-live USER 게이트 또는 S7b** (빌드 순서 W0✅→W2a✅→W2b✅→W1a✅→W1b✅→W3a✅→W3b-1✅→W3b-2a✅→W3b-2b✅→W3b-3✅ · 65차 supersede: PR-G ⓑ 단발 → 분해)** · Reflection/PR-K = **출시 전 빌드 + S9 운용 검증 중 가동·검증 (D32, 2026-06-24 USER 승격 — 구 "출시 후 defer" supersede)** · (65차 MVP 재정의: 출시 핵심 = ①30리스트 ②포트폴리오 ③30리포트 정확. W0~W3가 MVP 경로, 고도화 아님) · 상세·중간 단계 live SoT = HANDOFF §2
       · **🔑 키 구조 (live · 65차 Q3 + D28, D35 supersede)**: AI 프로바이더 키 = 공통(shared) — `OPENROUTER_API_KEY`(GLM 5.2 primary) + `ANTHROPIC_API_KEY`(Claude fallback) + `OPENAI_API_KEY`(GPT 역할/secondary). provider availability auto-detect — OpenRouter 키 없고 Anthropic 키가 있으면 Claude fallback, 둘 다 없으면 AI role fail-closed; GPT-only는 여전히 미지원. 모델 하드코딩 제거 + 역할별 차등은 D28 품질 목적함수(예측 적중률+리포트 정확성, hardcap=제약) + D35 GLM primary 레지스트리 기반. "Anthropic 유일 공통" / "Claude 필수 primary" / "토론=저가" 어휘 stale (상세 HANDOFF ⭐ 65차 MVP 엔진) / KIS·Binance = per-admin(DQ-7 암호화 저장) · KIS 현재 3명 중 1명만 보유(다른 1명 모의 키) → **사용자 포함 2명 발급 필요** (S7c read-only 시세는 1개 충분, S8 자동매매는 3명 each)
       · T7e.7 RLS 브라우저 수동 QA는 D11 운용 검증 직전 마무리 (1시간 안짝 수동)
  → [65차 supersede → 상단 W0✅→W2a✅→W2b✅→W1a✅→W1b✅→W3a✅→W3b-1✅→W3b-2a✅→W3b-2b✅→W3b-3✅ · 상세 HANDOFF ⭐ 65차 MVP 엔진] ~~PR-G ⓑ 실 AI 첫 30선정 → PR-H → PR-I·PR5b Tier2 → PR-J~~ (구 D11-전 hard gate 경로 — 코드자산은 W0~W3 토대 재사용)
  → S7b (뉴스+브리핑)
  ...
  → (S7d 후·S9 진입 전) PR-K Reflection 자가학습 빌드 [D32 — 출시 전 승격] → S9 운용 검증 (어드민 3인 1개월+ · §2.2 7 criteria · ★ S9 중 Reflection 실가동·검증) → 🎉 출시 (자동매매 없는 "AI 추천 + 가상 포트 + 알림" 내부 도구 · 단기/주간 트랙 자가학습 작동 — 중장기는 spec §4 타이밍)
...
### Supabase migrations
`tudal/supabase/migrations/` 0001~0037 production applied (0001 RLS sketch → 0037 `claim_skip_locked_cte_fix` — claim RPC over-claim CTE fix, 74차). DQ-7부터 도입된 컨벤션: 위험한 마이그레이션은 `NNNN_name.sql` + `NNNN_name.rollback.sql` 짝으로 작성 (예: `0009_dq7_credentials.{sql,rollback.sql}`). MCP `mcp__supabase__apply_migration`로 원격 적용.
