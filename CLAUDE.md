...
## ⭐ 프로젝트 재정의 (2026-04-21 D16 · 2026-05-08 D18 시퀀스 v3 · 2026-05-08 D19 AI 강화 v3.1 · 2026-05-20 D21·D22 Tier 2 14×14 overlay + Kevin v3.1 quality target v3.2 · 2026-05-21 D23 53차 §5 정정 v3.3 · 2026-05-22 D24 54차 §3 PR3a MERGED + Group H Hard gate 해소 v3.4 · 2026-05-22 D25 PR1 MERGED v3.5 · 2026-06-02 e2e engine PR-D~PR-G ⓐ + 비용0/FE-audit + launch-readiness PR #79(A~E) MERGED, main `532a0a5` · **2026-06-04 D26 65차 7결정 + D27 Q5 LOCKED — Q1 선정주기 분리(단기 주1회/중장기 월1회) · Q2 AI 자율 포트구성 · Q3 멀티프로바이더(Claude+GPT) 모델 추상화 · Q4 실시간 반박 토론 loop · hardcap 50만원 · 역할별 모델 차등(토론 저가/judge·리포트 고가) · MVP=30리스트+포트폴리오+30리포트 정확. 빌드 순서 W0→W2→W1→W3(PR-G ⓑ superseded) · Q5(D27, 같은 65차 후속) = 펀드식 incumbent thesis 재점검 — 모든 트랙(단기 주1회 포함) 재선정 시 기존 종목은 직전 리포트·논거+실현 성과 컨텍스트로 재평가(유지=신규와 top10 랭킹 경쟁 · W2 후보풀=fresh Tier0 ∪ incumbents + W1 주입 · PR-K Reflection과 별개). 상세 = HANDOFF.md ⭐ 65차 MVP 엔진 섹션 — 현 진행 상태 live SoT = `Document/Process/HANDOFF.md`**)
...
**현 진행 순서 (현 진행 상태 live SoT = `Document/Process/HANDOFF.md` · `Document/Build/ProgressDashboard.md §2` — main `532a0a5` 기준 canonical 5-PR + e2e engine PR-D~PR-G ⓐ + launch-readiness PR #79(A~E) MERGED, 다음 1순위 = W0(모델/프로바이더 추상화+hardcap 50만+cheap smoke) — 65차 supersede: PR-G ⓑ 단발 30선정 → W0→W2→W1→W3로 분해, 상세 HANDOFF ⭐ 65차 MVP 엔진)**:
```
Mock Skeleton ✅
  ...
  → S7a (Anthropic wrapper) ✅ 완료 (PR #1 `61653d22`) · 실 AI 첫 30선정 진입 대기 — 65차 supersede: PR-G ⓑ 단발 → W0→W2→W1→W3로 분해, 코드자산 재사용 (상세 HANDOFF §2 ⭐ 65차 MVP 엔진)
  → **Tier 2 D21 (52차)** ── ✅ **scaffold + production schema MERGED in main** (PR #4 + PR #5 + PR #6 historical, 마이그 0018·0019 production applied)
       ...
       · **출시 Runbook 진입점 = `Document/Process/HANDOFF.md §2`** (현재 위치·다음 1순위 live SoT = HANDOFF) · main `532a0a5` 기준 canonical 5-PR + e2e engine PR-D~PR-G ⓐ + launch-readiness PR #79(A~E) MERGED → **다음 1순위 = W0(모델/프로바이더 추상화+hardcap 50만+cheap smoke)** (65차 supersede: PR-G ⓑ → W0→W2→W1→W3로 분해 · AI 키 발급 + USER 비용 승인 게이트는 W0 진입 시점, 상세 HANDOFF §2 ⭐ 65차 MVP 엔진)
       · 다음 1순위 = **canonical 5-PR 전체 MERGED (PR2 `f85fb69` / PR3a `0813a41` / PR1 `4aa3130` / PR3b `cf68731` / PR3c `b2a902a` / PR4 `7de9696`) + e2e engine PR-D~PR-G ⓐ + launch-readiness PR #79(A~E, main `532a0a5`) MERGED → W0→W2→W1→W3 (65차 supersede: PR-G ⓑ 단발 → 4단계 분해)** · Reflection/PR-K = 출시 게이트 아님 (S9/go-live 후 defer) · (65차 MVP 재정의: 출시 핵심 = ①30리스트 ②포트폴리오 ③30리포트 정확. W0~W3가 MVP 경로, 고도화 아님) · 상세·중간 단계 live SoT = HANDOFF §2
       · **🔑 키 구조 (live · 65차 Q3 supersede)**: AI 프로바이더 키 = 공통(shared) — 멀티프로바이더 `ANTHROPIC_API_KEY`(Claude) + `OPENAI_API_KEY`(GPT, optional). provider availability auto-detect — GPT 키 없으면 Claude-only fallback. 모델 하드코딩 제거 + 역할별 차등(토론=저가 · judge/리포트=고가) + 레지스트리 기반. "Anthropic 유일 공통" 어휘 stale (상세 HANDOFF ⭐ 65차 MVP 엔진) / KIS·Binance = per-admin(DQ-7 암호화 저장) · KIS 현재 3명 중 1명만 보유(다른 1명 모의 키) → **사용자 포함 2명 발급 필요** (S7c read-only 시세는 1개 충분, S8 자동매매는 3명 each)
       · T7e.7 RLS 브라우저 수동 QA는 D11 운용 검증 직전 마무리 (1시간 안짝 수동)
  → [65차 supersede → 상단 W0→W2→W1→W3 · 상세 HANDOFF ⭐ 65차 MVP 엔진] ~~PR-G ⓑ 실 AI 첫 30선정 → PR-H → PR-I·PR5b Tier2 → PR-J~~ (구 D11-전 hard gate 경로 — 코드자산은 W0~W3 토대 재사용)
  → S7b (뉴스+브리핑)
