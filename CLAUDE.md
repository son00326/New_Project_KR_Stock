...
`tudal/src/app/api/cron/`에 cron 라우트 + `tudal/vercel.json` 스케줄 **9개(전부 daily·Vercel Hobby 호환·9/100)**: `monthly-batch/report-worker` `0 1`(PR5_CRON_AUTO_ENABLED gated·dormant) · `monthly-batch/selection-worker` `0 2`(SELECTION_CRON_AUTO_ENABLED gated·dormant) · `morning-briefing` `0 23` · `news-sweep` `0 0`(Naver 주입 후 live) · `silent-health` `0 15` · `reflection-job` `30 1`(REFLECTION_ENABLED gated·dormant·#129) · `exit-signal` `30 15`(EXIT_SIGNAL_ENABLED gated·dormant·#129) · `exit-outcome` `0 16`(EXIT_OUTCOME_ENABLED gated·dormant·#129) · `funnel-reflection` `0 3`(FUNNEL_REFLECTION_ENABLED gated·dormant·#131). `monthly-batch` 부모 라우트는 no-op(deprecated). flag-gated dormant cron은 flag OFF면 byte-identical. Hobby는 minute 아닌 hour 정밀도. 변경은 운영 영향 있으므로 배포 조율 필요. **비-Vercel 자동화(#131)**: `.github/workflows/tier0-weekly-producer.yml`(주간 tier0 producer — GH vars 게이트 dormant) + `tudal/scripts/worker/intraday-ws-worker.ts`(장중 연속 WS 워커 — 외부 tsx 프로세스, CONFIRM env 게이트).

### Supabase migrations
`tudal/supabase/migrations/` 0001~0049 production applied (0001 RLS sketch → 0049 `reset_sector_board_eligible_jobs`) + **0050 `tier0_candidates_150.factor_ranks` ✅ production applied(2026-07-04, version `20260704120509` — Claude MCP apply·omxy mig R1~R3 CONVERGED·dormant NULL)**. DQ-7부터 도입된 컨벤션: 위험한 마이그레이션은 `NNNN_name.sql` + `NNNN_name.rollback.sql` 짝으로 작성 (예: `0009_dq7_credentials.{sql,rollback.sql}`). MCP `mcp__supabase__apply_migration`로 원격 적용.
