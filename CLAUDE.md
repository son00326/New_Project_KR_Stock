...
### Supabase migrations
`tudal/supabase/migrations/` 0001~0049 production applied (0001 RLS sketch → 0049 `reset_sector_board_eligible_jobs`) + **0050 `tier0_candidates_150.factor_ranks`(#131 빌드·pg_smoke PASS — DORMANT·USER apply 대기)**. DQ-7부터 도입된 컨벤션: 위험한 마이그레이션은 `NNNN_name.sql` + `NNNN_name.rollback.sql` 짝으로 작성 (예: `0009_dq7_credentials.{sql,rollback.sql}`). MCP `mcp__supabase__apply_migration`로 원격 적용.
