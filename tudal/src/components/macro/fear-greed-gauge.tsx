...
function getZoneInfo(value: number) {
  if (value <= 20) return { label: "극단적 공포", color: "text-blue-700", bg: "bg-blue-600" };
  if (value <= 40) return { label: "공포", color: "text-blue-600", bg: "bg-blue-500" };
  if (value <= 60) return { label: "중립", color: "text-yellow-600", bg: "bg-yellow-500" };
  if (value <= 80) return { label: "탐욕", color: "text-red-500", bg: "bg-red-500" };
  return { label: "극단적 탐욕", color: "text-red-700", bg: "bg-red-600" };
}
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="text-center mt-2">
        <span className={`text-4xl font-bold ${zone.color}`}>{value}</span>
