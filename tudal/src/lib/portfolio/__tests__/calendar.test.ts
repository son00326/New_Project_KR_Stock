...
describe("영업일/주말/공휴일 불변식", () => {
  ...
  it("공휴일 맵 18종 전수 일치 + 그 외 날짜는 holidayName=null (R32: exact 고정, 누락/추가 회귀 탐지)", () => {
    // 0004 §4 2026 블록과 100% 동기화 — money-path 게이팅 입력이라 정확히 박제(범위 아님).
    // 77차: 2026-05-01 근로자의날 + 2026-07-17 제헌절(둘 다 KRX 휴장, 금요일) 추가 (16종→18종).
    ...
    // (2) holidayName 보유 날짜 집합 = 정확히 18종 (추가/유령 공휴일 회귀 탐지)
    const named = cal.filter((d) => d.holidayName !== null).map((d) => d.date).sort();
    expect(named).toEqual(Object.keys(HOLIDAYS_2026).sort());
  });

  it("영업일 수 정확히 244 (R32: exact 고정 — 365 − 주말 104 − 평일공휴일 17)", () => {
    // 77차: 2026-05-01 근로자의날 + 2026-07-17 제헌절(둘 다 금요일=평일) 추가로 평일공휴일 15→17, 영업일 246→244.
    expect(cal.filter((d) => d.isBusinessDay).length).toBe(244);
  });
});
