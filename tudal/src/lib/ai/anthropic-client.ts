...
export interface CallPersonaInput {
  ...
  // PR-K Reflection (D32): 직전 사이클 회고 컨텍스트(컨텍스트 입력 only). 미지정/"" → byte-identical(dormant).
  //   D27 Q5 reflectionContext(per-ticker)와 별개 필드. renderUserPrompt가 macro/negative 뒤 조건부 append.
...
export async function callPersona(input: CallPersonaInput): Promise<CallPersonaResult> {
  ...
  const userPrompt = renderUserPrompt(input.userPromptTemplate ?? persona.userPromptTemplate, {
    ...
    reflectionLearningContext: input.reflectionLearningContext,
  });
