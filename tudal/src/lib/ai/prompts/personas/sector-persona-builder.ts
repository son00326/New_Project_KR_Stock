...
export const SECTOR_PHILOSOPHIES: Record<CanonicalSector, string> = {
  "바이오": "바이오는 임상 결과 1건이 시총을 2배로 만들거나 80% 날려버리는 binary 산업입니다. 신약 1개 개발에 10년·1조원이 들지만 성공 시 글로벌 매출 5조원이 가능합니다. 핵심 판단: 파이프라인 단계(전임상→3상)·FDA/식약처 일정·매출 다각화·현금 소진 속도. **재무 인용 anchor**: 매출 / R&D 비용 / 현금성 자산 / 영업현금흐름 / 라이센싱 마일스톤 수령액. **peer 비교 anchor**: 글로벌 바이오텍 (동일 modality·시총 상위 그룹) + 국내 빅바이오 그룹. **밸류 가정 패턴**: 파이프라인 NPV (단계별 할인율 적용) + 로열티 NPV + 빅파마 라이센싱 deal multiples. 가정 노출 시 단계 성공률·할인율·peak sales 명시. **일상 비유 anchor**: 신약 개발 = '실험실에서 10년 키운 묘목 1그루가 갑자기 글로벌 매대에서 팔리거나 폐기되는 사건'.",
  "반도체": "반도체는 4년 주기 사이클(호황 2년·불황 2년)을 반복하는 자본 게임입니다. 신규 공정 leap이 시장 점유율을 5년간 결정합니다. 핵심 판단: DRAM/NAND 가격 흐름·CAPEX 규모·고객사 다변화·재고 사이클 단계. **재무 인용 anchor**: 매출 mix(DRAM·NAND·파운드리)·영업이익률·CAPEX·재고일수·R&D 비중. **peer 비교 anchor**: 글로벌 메모리/파운드리 peers (해외 상위 1~3개) + 국내 동종 그룹. **밸류 가정 패턴**: EV/EBITDA 사이클 보정 (호황 멀티플·불황 멀티플 분리) + P/B 다운사이드. 가정 노출 시 사이클 위치 명시. **일상 비유 anchor**: 반도체 사이클 = '여름에 폭주하다 겨울에 비는 호텔 사업 — 객실 증설 타이밍이 5년 수익을 가른다'.",
  "건설": "건설은 수주(2~3년 전) → 매출 → 손익으로 시차가 큰 산업입니다. PF(프로젝트 파이낸싱) 부실 한 건이 자본을 잠식할 수 있습니다. 핵심 판단: 미분양 비율·PF 잔액·해외 수주 마진·원자재(시멘트·철근) 가격 전가력. **재무 인용 anchor**: 매출 / 수주잔고 / PF 우발채무 / 영업이익률 / 미분양 호수 / 운전자본 회전. **peer 비교 anchor**: 국내 대형 시공사 (시총 상위 그룹) + 글로벌 EPC peers. **밸류 가정 패턴**: PBR + 수주잔고/매출 + 미분양 cycle adjusted EV/EBITDA. 가정 노출 시 PF 부실률·분양률 명시. **일상 비유 anchor**: 건설은 '식당이 손님 예약(수주)을 2년 전에 받고, 그 사이 식재료 값(원자재) 오르면 마진이 통째로 증발하는 사업'.",
  "금융": "금융은 NIM(순이자마진)·자산 건전성·자기자본비율 3축으로 평가합니다. 금리 1% 변화가 은행 순이익을 10~20% 흔듭니다. 핵심 판단: 연체율 추세·BIS 비율·디지털 전환 속도·핵심 예금 비중. **재무 인용 anchor**: NIM / 대손충당금 / NPL 비율 / CIR (비용효율) / BIS 비율 / 핵심예금 비중. **peer 비교 anchor**: 국내 4대 금융지주 + 글로벌 대형 은행 (시총 상위 그룹). **밸류 가정 패턴**: P/B (ROE 디스카운트/프리미엄 적용) + 배당수익률 + Gordon Growth. 가정 노출 시 금리·NPL 가정 명시. **일상 비유 anchor**: 은행은 '예금이라는 저금통 1조 모아서 1.05조에 빌려주는 사업 — 빌려준 사람 5%가 갚지 않으면 본전이 사라지는 구조'.",
  "2차전지": "2차전지는 LFP(저가)·NCM(고에너지) 화학 노선과 EV 보급 곡선이 엮인 산업입니다. 1개 메이저 OEM 수주 변경이 매출 30% 좌우합니다. 핵심 판단: 셀 제조 mix·OEM 고객 분산·원자재 원가·차세대 R&D 진척. **재무 인용 anchor**: 매출 / 영업이익률 / CAPEX / 재고 / 셀당 평균 판가 / OEM 고객별 매출 mix. **peer 비교 anchor**: 글로벌 셀 메이커 상위 3 + 국내 동종 그룹. **밸류 가정 패턴**: EV/EBITDA + 수주 잔고 NPV (단가·물량·고객 분산). 가정 노출 시 EV 보급률·OEM 수주 가정 명시. **일상 비유 anchor**: 배터리는 '카페가 메뉴 한 종류만 팔다가 갑자기 전 매장 메뉴를 동시에 바꾸는 격변기 — 어떤 레시피(화학)에 베팅하느냐가 5년 매출을 결정'.",
  "자동차": "자동차는 ICE→EV 전환과 ADAS·자율주행 기술 leap이 동시 진행되는 transition 산업입니다. 글로벌 OEM 1개 sourcing 변경이 부품사 매출 50%를 흔듭니다. 핵심 판단: EV 비중·자율주행 SW 역량·중국 시장 노출·공급망 회복도. **재무 인용 anchor**: 매출 / 영업이익률 / EV 매출 비중 / R&D 비중 / 글로벌 판매대수 / 평균 판매단가. **peer 비교 anchor**: 글로벌 OEM (시총 상위 그룹) + 국내 동종 그룹. **밸류 가정 패턴**: EV/Sales (EV transition premium 분리) + sum-of-parts (ICE + EV + 자율주행 SW). 가정 노출 시 EV mix·OEM 시장 점유 가정 명시. **일상 비유 anchor**: 완성차는 '20년 운영하던 휘발유 식당이 전기 식당으로 동시에 메뉴를 바꾸는 사업 — 손님(시장)과 주방(생산라인) 둘 다 새로 학습'.",
  "IT/SW": "IT/SW는 SaaS·클라우드 인프라가 성장의 핵심이며 retention(고객 유지율)·NRR(순매출 유지율) 100% 초과가 quality 시그널입니다. 핵심 판단: ARR 성장률·net dollar retention·R&D 효율·플랫폼 락인 vs commodity 위험. **재무 인용 anchor**: ARR / NRR / 매출 / 영업현금흐름 / Rule of 40 (성장률 + 영업이익률) / 고객 acquisition cost. **peer 비교 anchor**: 글로벌 SaaS peers (시총 상위 그룹) + 국내 동종 그룹. **밸류 가정 패턴**: EV/ARR (성장률 mapped multiple) + Rule of 40 score + DCF (장기 retention). 가정 노출 시 ARR 성장률·NRR 가정 명시. **일상 비유 anchor**: SaaS는 '넷플릭스처럼 매달 구독료를 받되, 고객이 사용량을 늘리면 자동으로 더 많이 내는 사업 — 신규 모객보다 기존 고객 확장이 핵심'.",
  "유통/소비재": "유통/소비재는 옴니채널(온라인+오프라인) 통합과 DTC(direct-to-consumer) 전환이 마진 구조를 바꿉니다. 1개 핵심 brand 트렌드 변화가 영업이익을 30% 흔듭니다. 핵심 판단: 매장당 매출·이커머스 비중·재고 회전·소비자 신뢰지수. **재무 인용 anchor**: 매출 / 매장당 평균 매출 / 이커머스 매출 비중 / 재고 회전일수 / 영업이익률 / GMV (총거래액). **peer 비교 anchor**: 글로벌 retail/소비재 peers (시총 상위 그룹) + 국내 동종 그룹. **밸류 가정 패턴**: EV/Sales (성장률 + 마진 mix) + EV/EBITDA + 브랜드 가치 옵션. 가정 노출 시 매장당 매출·DTC 비중 가정 명시. **일상 비유 anchor**: 소비재는 '동네 카페 1개가 전국 100개 매장으로 늘 때, 매장당 매출이 같이 늘어야 진짜 성장 — 점포 수만 늘면 영업이익이 떨어진다'.",
  "에너지": "에너지는 신재생(태양광·풍력·수소) 비중 확대와 전력시장 design 변화가 장기 가치를 결정합니다. 정책 1건이 사업성을 reset할 수 있습니다. 핵심 판단: 신재생 발전 capacity·전력 가격 hedging·정책 노출도·CAPEX 회수 기간. **재무 인용 anchor**: 매출 / 발전 capacity (MW) / SMP (전력가) / 영업이익률 / CAPEX 회수율 / 정책 보조금 수령액. **peer 비교 anchor**: 글로벌 신재생 utilities (시총 상위 그룹) + 국내 발전사 그룹. **밸류 가정 패턴**: DCF (capacity factor + LCOE 가정) + EV/EBITDA + 보조금 NPV. 가정 노출 시 capacity factor·전력가격 가정 명시. **일상 비유 anchor**: 발전사업은 '한번 지으면 25년 임대료(전력 판매)를 받는 건물 — 단, 임대료 시세(정책·전력가)가 매년 바뀐다'.",
  "엔터/미디어": "엔터/미디어는 OTT 글로벌 라이센싱과 K-콘텐츠(드라마·K-팝·게임 IP) 수출이 매출 다각화의 핵심입니다. 1개 hit IP가 시총을 2배로 만들 수 있습니다. 핵심 판단: 글로벌 라이센스 매출 비중·IP 포트폴리오 다양성·아티스트/창작자 계약 안정성. **재무 인용 anchor**: 매출 / 글로벌 라이센스 매출 비중 / 영업이익률 / IP 매출 mix / 아티스트 계약 잔여기간. **peer 비교 anchor**: 글로벌 미디어/엔터 peers (OTT·음악·게임 시총 상위) + 국내 동종 그룹. **밸류 가정 패턴**: EV/Sales (IP 가치 옵션) + sum-of-parts (음악·드라마·게임 분리) + DCF (IP 라이프사이클 가정). 가정 노출 시 IP hit ratio·계약 갱신율 가정 명시. **일상 비유 anchor**: 엔터는 '맛집 1곳 떴을 때 전국 프랜차이즈로 확장하는 사업 — 한 IP의 글로벌 침투가 매출을 2배 만든다'.",
  "통신": "통신은 5G·6G CAPEX 사이클과 ARPU(가입자당 매출) 안정성이 cash flow를 결정합니다. 정부 주파수 배정 1건이 5년 경쟁 구도를 결정합니다. 핵심 판단: 5G/6G 투자 회수 단계·기업 B2B 매출 비중·MVNO 위협 정도·해외 진출. **재무 인용 anchor**: 매출 / ARPU / 5G 가입자 비중 / CAPEX / 영업이익률 / B2B 매출 비중. **peer 비교 anchor**: 국내 통신 3사 + 글로벌 통신사 peers (시총 상위 그룹). **밸류 가정 패턴**: EV/EBITDA + 배당수익률 + DCF (5G CAPEX 회수 가정). 가정 노출 시 ARPU 성장률·5G 침투율 가정 명시. **일상 비유 anchor**: 통신은 '아파트 전기·수도처럼 매달 안정적으로 받는 사업 — 단, 5년에 한 번 큰 공사(5G·6G) 비용이 한꺼번에 나간다'.",
  "철강/소재": "철강/소재는 글로벌 수급(중국 수출·인프라 수요)·원자재(철광석·니켈) 가격·스프레드(판매가-원가) 3축 게임입니다. 1년 사이클이 영업이익을 -50%~+50% 흔듭니다. 핵심 판단: 글로벌 강재 수급·중국 정책·에너지 전환 수혜·고부가가치 비중. **재무 인용 anchor**: 매출 / 톤당 판매단가 / 톤당 원가 / 스프레드 / 영업이익률 / 고부가가치 제품 매출 비중. **peer 비교 anchor**: 글로벌 철강/소재 peers (시총 상위 그룹) + 국내 동종 그룹. **밸류 가정 패턴**: PBR (cycle low/mid/high 분리) + EV/EBITDA (사이클 평균 가정). 가정 노출 시 스프레드·중국 수출 가정 명시. **일상 비유 anchor**: 철강은 '재료비(원자재)와 판매가가 둘 다 거의 매일 바뀌는 식당 — 두 가격 사이 간격(스프레드)이 영업이익의 전부'.",
  "운송/물류": "운송/물류는 글로벌 무역 흐름·BDI(벌크 운임)·항공/해운 운임이 매출을 직격합니다. 코로나·운하 사고 같은 외부 충격이 운임을 10배 만들 수 있습니다. 핵심 판단: 운임 사이클 단계·선대(船隊) 규모·연료비 hedging·환율 노출. **재무 인용 anchor**: 매출 / 평균 운임 / 적재율 / 선대 capacity (TEU·DWT) / 영업이익률 / 연료비 비중. **peer 비교 anchor**: 글로벌 해운/항공 peers (시총 상위 그룹) + 국내 동종 그룹. **밸류 가정 패턴**: EV/EBITDA (사이클 평균 멀티플) + P/B (선대 가치). 가정 노출 시 운임·연료비 가정 명시. **일상 비유 anchor**: 해운은 '택시 운임이 매년 5배 올랐다 다음 해 절반이 되는 사업 — 운전기사(선원)와 차량(선대)은 5년 전에 미리 갖춰야 한다'.",
  "보험/증권": "보험/증권은 운용자산 규모·운용수익률·신계약 ARPU 3축으로 평가합니다. 보험은 actuarial(보험수리) 정확도, 증권은 시장 사이클이 핵심입니다. 핵심 판단: 운용자산 성장률·계약유지율·자기자본수익률·디지털 채널 전환. **재무 인용 anchor**: 매출 (보험료/수수료) / 운용자산 (AUM) / 운용수익률 / 신계약 ARPU / 영업이익률 / 계약유지율. **peer 비교 anchor**: 국내 보험/증권 peers (시총 상위 그룹) + 글로벌 동종 그룹. **밸류 가정 패턴**: P/B (ROE 적용) + 임베디드 가치 (보험) + EV/AUM (증권·자산운용). 가정 노출 시 AUM 성장률·운용수익률 가정 명시. **일상 비유 anchor**: 보험은 '매달 회비 1만원 모은 1만 명의 돈을 100억 운용해서 5년 뒤 사고 난 1000명에게 100만원씩 돌려주는 사업 — 운용 수익률이 1% 차이나면 회사 손익이 통째로 바뀐다'.",
};
