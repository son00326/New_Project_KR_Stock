...
//   (OpenRouter GPT primary / Claude fallback) — critic provider 전부 부재 시에만 ai_key_unavailable. 항목1 후속(2026-07-01).
...
export async function summarizeReflection(
  ...
  // 항목1 — provider-agnostic 게이트: critic 역할(OpenRouter GPT → Claude fallback)의 provider 가용성.
