...
function makeFakeClient(opts: {
  ...
  // B-SEL-CRON (finding 16) — finalize stale-guard: 같은 트랙 더 최신 finalized period 조회 결과.
  newerFinalized?: Array<{ period_key: string }>;
  newerFinalizedError?: { code?: string } | null;
}) {
    ...
    from: vi.fn((table: string) => {
      ...
      chain.select = vi.fn(
        (_cols: string, selOpts?: { count?: string; head?: boolean }) => {
          ...
          // 비-count SELECT 3형 (terminal 메서드로 구분):
          //   (a) finalize 전체 rows: await .eq('period_key',..) → {data: allRows} (then)
          //   (b) run row 구분(B-SEL-CRON): .eq('period_key',..).maybeSingle() → {data: runRow}
          //   (c) stale-guard(finding 16): .eq('track',..).gt('period_key',..).not(..).limit(1)
          //         → {data: newerFinalized}
          const q: Record<string, unknown> = {};
          q.gt = vi.fn(() => q);
          q.not = vi.fn(() => q);
          q.limit = vi.fn(async () => ({
            data: opts.newerFinalized ?? [],
            error: opts.newerFinalizedError ?? null,
          }));
...
describe("runTier1SelectionChunk finalize (nonTerminal===0 && terminal>0 → 30선정 + persist)", () => {
  ...
  // B-SEL-CRON (finding 16) — stale 재개 가드: 같은 트랙 더 최신 finalized period 존재 시 persist skip.
  it("같은 트랙 더 최신 finalized period 존재 → persist skip + finalized:false + mark_finalized로 종착", async () => {
    const r1RowsFull = makeTrackCandidates("midlong").map((c) => ({
      ticker: c.ticker,
      status: "done",
      panel_result: makePanel(),
    ...
    const { client, rpcCalls } = makeFakeClient({
      ...
      terminalCount: 100,
      allRows,
      // 더 최신 period가 이미 finalize됨 (역순 수동 재개 시나리오)
      newerFinalized: [{ period_key: "m:2026-07" }],
    });
    const deps = makeDeps("midlong");
    ...
    expect(res.result?.finalized).toBe(false);
    // stale 선정으로 덮어쓰지 않음 (money-path shortlist 보존)
    expect(deps.persist).not.toHaveBeenCalled();
    expect(deps.runScreening).not.toHaveBeenCalled();
    // 그래도 종착 마킹 → always-due 일일 재시도 정지
  ...
  it("더 최신 finalized period 없음(정상 전진) → 정상 persist (가드 무영향)", async () => {
    const r1RowsFull = makeTrackCandidates("midlong").map((c) => ({
      ticker: c.ticker,
      status: "done",
      panel_result: makePanel(),
