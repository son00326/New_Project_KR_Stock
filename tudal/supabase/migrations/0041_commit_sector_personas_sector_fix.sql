...
-- purpose: 0040 live-catch(2026-06-24) corrective를 repo migration으로 공식화 + 0019(admin)·0040(cron) 양
--          함수의 sector 미설정 잠복 버그 동시 수정 (omxy R7 HIGH/MED + R8 MED self-contained).
--   본 마이그는 admin(commit_sector_personas)과 cron(commit_sector_personas_cron)을 **둘 다** idempotent
--   CREATE OR REPLACE(sector=p_sector) 한다 → buggy-0040만 적용한 env가 0041만 받아도 양 함수 정합(drift 0).
...
-- ── cron RPC (0040) — idempotent re-affirm (omxy R8 MED: self-contained corrective) ──
-- buggy-0040만 적용하고 prod one-off는 못 받은 env가 0041만 받아도 cron이 정합되도록 본 마이그가 cron도
-- CREATE OR REPLACE(sector=p_sector). 0040.sql(이미 수정)과 동일 정의 → 어떤 적용 순서든 drift 0.
